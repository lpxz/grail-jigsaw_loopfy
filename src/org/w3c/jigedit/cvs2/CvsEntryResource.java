...
public class CvsEntryResource extends ContainerResource {
    ...
    static byte startAnchor[] = { (byte) '<', (byte) 'a', (byte) ' ', (byte) 'h', (byte) 'r', (byte) 'e', (byte) 'f', (byte) '=', (byte) '"' };

    static byte midAnchor[] = { (byte) '"', (byte) '>' };

    static byte endAnchor[] = { (byte) '<', (byte) '/', (byte) 'a', (byte) '>' };

    static byte edittext[] = { (byte) 'e', (byte) 'd', (byte) 'i', (byte) 't', (byte) ' ', (byte) 'l', (byte) 'o', (byte) 'g' };

    static byte pattern[] = { (byte) 'r', (byte) 'e', (byte) 'v', (byte) 'i', (byte) 's', (byte) 'i', (byte) 'o', (byte) 'n' };

    static byte increments[] = new byte[128];

    static {
        edu.hkust.clap.monitor.Monitor.loopBegin(1083);
for (int i = 0; i < 128; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1083);
{
            increments[i] = 8;
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1083);
    ...
    class RevisionResource extends FramedResource {
        ...
        public ResourceReference getResourceReference() {
            if (rr == null) rr = new DummyResourceReference(this);
            return rr;
        }
    ...
    class RevisionFrame extends HTTPFrame {
            ...
            try {
                ...
                try {
                    ...
                    if (parent instanceof ContainerResource) {
                        ...
                        if (rr_res == null) {
                            this.setValue(ATTR_CONTENT_TYPE, org.w3c.www.mime.Utils.getMimeType(name));
                            return;
                        }
                        try {
                            res = rr_res.lock();
                            if (res instanceof FileResource) {
                                HTTPFrame httpFrame = (HTTPFrame) res.getFrame(Class.forName("org.w3c.jigsaw.frames.HTTPFrame"));
        ...
        public Reply createDefaultReply(Request request, int status) {
            ...
            HttpInteger contentlength = HttpFactory.makeInteger(getInt(ATTR_CONTENT_LENGTH, -1));
        ...
        public Reply get(Request request) throws ProtocolException {
            try {
                try {
                    ...
                    error.setContent("Bad revision number : <b>" + ex.getMessage() + "</b>");
                    return error;
                }
                ...
                try {
                    ...
                    this.setValue(ATTR_CONTENT_LENGTH, new Integer(content.length));
                ...
                Reply error = request.makeReply(HTTP.INTERNAL_SERVER_ERROR);
                error.setContent("Cvs operation failed : <b>" + ex.getMessage() + "</b>");
                return error;
            }
        }
    ...
    class CvsEntryFrame extends PostableFrame {
        ...
        protected Reply dolog(Request request) throws ProtocolException {
            ...
            try {
                ...
                g.append("<p>The CVS <strong>log</strong> command failed " + " on " + name + " with the following error message: " + "<em>" + cvs_ex.getMessage() + "</em>" + "<hr> from class: " + this.getClass().getName());
                error.setStream(g);
                throw new HTTPException(error);
            } catch (InvalidResourceException ex) {
                Reply error = request.makeReply(HTTP.INTERNAL_SERVER_ERROR);
                HtmlGenerator g = getHtmlGenerator("CVS log command failed");
                g.append("<p>The CVS <strong>log</strong> command failed " + " on " + name + " with the following error message: " + "<em>" + ex.getMessage() + "</em>" + "<hr> from class: " + this.getClass().getName());
                error.setStream(g);
                throw new HTTPException(error);
            } finally {
                rr_cvsframe.unlock();
        ...
        protected Reply dodiff(Request request) throws HTTPException {
            ...
            try {
                ...
                g.append("<p>The CVS <strong>diff</strong> command failed " + " on " + name + " with the following error message: " + "<em>" + cvs_ex.getMessage() + "</em>" + "<hr> from class: " + this.getClass().getName());
                error.setStream(g);
                throw new HTTPException(error);
            } catch (InvalidResourceException ex) {
                Reply error = request.makeReply(HTTP.INTERNAL_SERVER_ERROR);
                HtmlGenerator g = getHtmlGenerator("CVS diff command failed");
                g.append("<p>The CVS <strong>diff</strong> command failed " + " on " + name + " with the following error message: " + "<em>" + ex.getMessage() + "</em>" + "<hr> from class: " + this.getClass().getName());
                error.setStream(g);
                throw new HTTPException(error);
            } finally {
                rr_cvsframe.unlock();
            ...
            if ((diff == null) || (diff.length() == 0)) {
                ...
                g.append("<P>No differences between " + name + " and the repository</P>");
                reply.setStream(g);
            } else {
                HtmlGenerator g = getHtmlGenerator("Diff result");
                g.append("<center>");
                g.append(" [ <a href=\"../\">Up to directory</A> ] &middot;");
                g.append(" [ <a href=\"" + getCvsURL(), "\">Back to CVS</A> ] \n");
                g.append("</center>");
                g.append("<hr noshade width=\"80%\"><p>");
                g.append("<span class=\"title\"> CVS diff of ", name, "</span>\n");
        ...
        protected Reply doEditRev(Request request, String revision) throws HTTPException {
            HtmlGenerator g = getHtmlGenerator("Edit log");
            g.append("<center>");
            g.append(" [ <a href=\"../\">Up to directory</A> ] &middot; ");
            g.append(" [ <a href=\"" + getCvsURL() + "\">Back to CVS</A> ] &middot; ");
            g.append(" [ <a href=\"./", name + "?log\">Back to log</A> ] ");
            g.append("<hr noshade width=\"80%\"><p>");
            g.append("<span class=\"title\"> Edit log comment of ", name, " (revision " + revision + ")</span>\n");
            g.append("<form  method=\"post\">\n");
            g.append("<input type=\"hidden\" name=\"revision\" value=\"", revision, "\">\n");
            ...
            g.append("<input type=\"submit\" name=\"submit\" " + "value=\" Save Comment \">");
            g.append("</td></tr>");
	 ...
	 */
        public Reply get(Request request) throws ProtocolException {
            ...
            if (cmd.equalsIgnoreCase("log")) {
                ...
                if (rev != null) return doEditRev(request, rev); else {
                    Reply error = request.makeReply(HTTP.INTERNAL_SERVER_ERROR);
        ...
        public Reply handle(Request request, URLDecoder data) throws ProtocolException {
            ...
            try {
                ...
                Reply error = request.makeReply(HTTP.INTERNAL_SERVER_ERROR);
                HtmlGenerator g = getHtmlGenerator("CVS admin command failed");
                g.append("<p>The CVS <strong>admin</strong> command failed " + " on " + name + " with the following error message: " + "<em>" + ex.getMessage() + "</em>");
                error.setStream(g);
                throw new HTTPException(error);
    ...
    protected void checkRevisionNumber(String rev) throws RevisionNumberException, CvsException {
        try {
            String revision = getCvsManager().revision(name);
            if (revision == null) return;
            int minor = getMinor(revision);
            int minor2 = getMinor(rev);
            if ((minor2 > minor) || (minor2 <= 0)) throw new RevisionNumberException(rev);
        } catch (InvalidResourceException inv_ex) {
     ...
     */
    static final boolean byteArrayNEquals(byte[] ba1, int off1, byte[] ba2, int off2, int n) {
        int corr = off2 - off1;
        int max = n + off1;
...
edu.hkust.clap.monitor.Monitor.loopEnd(42);
    ...
    protected String parseDiff(String diff) {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(1084);
while ((idx = diff.indexOf('<')) != -1) { 
edu.hkust.clap.monitor.Monitor.loopInc(1084);
{
            newdiff.append(diff.substring(0, idx));
            newdiff.append("&lt;");
            diff = diff.substring(idx + 1);
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1084);
    ...
    protected HtmlGenerator parseLog(String log) {
        ...
        String head = "[ <a href=\"./../\">Up to directory</a> ] &middot; " + "[ <a href=\"" + CVSURL + "\">Back to CVS</a> ]";
        ...
        byteIdx += 7;
        do {
            edu.hkust.clap.monitor.Monitor.loopBegin(1085);
while (byteIdx < unparsed.length) { 
edu.hkust.clap.monitor.Monitor.loopInc(1085);
{
                if ((ch = unparsed[byteIdx]) == (byte) 'n') {
                    if (byteArrayNEquals(unparsed, byteIdx - 7, pattern, 0, 7)) {
                        break;
                    }
                }
                byteIdx += increments[ch >= 0 ? ch : 0];
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1085);

            if (++byteIdx >= unparsed.length) break;
            edu.hkust.clap.monitor.Monitor.loopBegin(1086);
while ((byteIdx <= unparsed.length) && isSpace(unparsed[byteIdx])) { 
edu.hkust.clap.monitor.Monitor.loopInc(1086);
{
                byteIdx++;
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1086);

            out.write(unparsed, startIdx, byteIdx - startIdx);
            startIdx = byteIdx;
            edu.hkust.clap.monitor.Monitor.loopBegin(1087);
while ((byteIdx <= unparsed.length) && isDigitOrPoint(unparsed[byteIdx])) { 
edu.hkust.clap.monitor.Monitor.loopInc(1087);
{
                byteIdx++;
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1087);

            if (byteIdx - 1 > startIdx) {
                String rev = new String(unparsed, startIdx, byteIdx - startIdx);
                try {
        ...
        if (length > 0) out.write(unparsed, startIdx, length);
    ...
    public synchronized ResourceReference getResourceReference() {
        if (self == null) self = new DummyResourceReference(this);
        return self;
    }
