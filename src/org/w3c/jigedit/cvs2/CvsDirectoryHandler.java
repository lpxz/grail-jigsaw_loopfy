...
public class CvsDirectoryHandler implements CvsHandlerInterface {
    ...
    protected Reply badAction(Request request, String action) {
        ...
        g.append("<p>Your command " + "<strong>" + action + "</strong>" + " wasn't undesrtood.<p>");
     ...
     */
    public void perform(Request request, String action, String regexp, String comment) throws ProtocolException {
        try {
            ...
            if (action.equals("add")) {
                ...
                if (comment != null) {
                    ...
                    jcomment = ((u != null) ? "(" + u + ") changed through Jigsaw, no comments" : "changed through Jigsaw, no comments");
                }
     ...
     * @exception ProtocolException If running the action failed.
     */
    public void perform(Request request, String action, String regexp) throws ProtocolException {
     ...
     * @exception ProtocolException If running the action failed.
     */
    public void perform(Request request, String action, String names[], String revs[]) throws ProtocolException {
     ...
     * @exception ProtocolException If running the action failed.
     */
    public void perform(Request request, String action, String names[], String revs[], String comment) throws ProtocolException {
        String u = (String) request.getState(AuthFilter.STATE_AUTHUSER);
        String env[] = { "USER=" + u, "LOGNAME=" + u };
        String jcomment = null;
        if (comment != null) {
        ...
        if (debug) edu.hkust.clap.monitor.Monitor.loopBegin(796);
for (int i = 0; i < names.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(796);
System.out.println("*** perform " + action + " on " + names[i]);} 
edu.hkust.clap.monitor.Monitor.loopEnd(796);
        ...
        try {
            if (action.equals("add")) {
                ...
                edu.hkust.clap.monitor.Monitor.loopBegin(797);
for (int i = 0; i < names.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(797);
cvs.revert(names[i], revs[i], msg, env);} 
edu.hkust.clap.monitor.Monitor.loopEnd(797);
