...
public class PutFilter extends ResourceFilter {
    ...
    static {
        ...
        a = new StringAttribute("put-list", null, Attribute.EDITABLE | Attribute.MANDATORY);
    ...
    protected synchronized ResourceReference resolvePutListResource() {
        ...
        if (r_target != null) {
            try {
                ...
                if (!(target instanceof PutListResource)) r_target = null; else list = r_target;
    ...
    public ReplyInterface ingoingFilter(RequestInterface req) {
        ...
        if (request.getMethod().equals("DELETE") && (request.getTargetResource() != null)) {
            ...
            if (!done) {
                ...
                s.errlog(getClass().getName() + ": unable to resolve companion PutListResource at " + getPutListURL());
            }
        }
        return null;
    }
    ...
    public ReplyInterface outgoingFilter(RequestInterface req, ReplyInterface rep) {
        ...
        boolean put = false;
        if (((put = request.getMethod().equals("PUT")) || request.getMethod().equals("DELETE")) && ((reply.getStatus() / 100) == 2)) {
            ...
            synchronized (this) {
                ...
                if (rr != null) {
                    try {
                        l = (PutListResource) rr.lock();
                        if (l != null) {
                            if (put) status = l.registerRequest(request); else status = l.confirmDelete(request);
                            done = true;
                        }
                    } catch (InvalidResourceException ex) {
                        done = false;
            ...
            switch(status) {
                ...
                case PutListResource.FILE_MG:
                    ...
                    g.append("<H1>Warning</H1> The file on publish space has " + "been modified directly but attempting to merge " + "has succeed.<p>" + "You should update the file before editing " + "it again.");
                    msg.setStream(g);
                    return msg;
