...
public class PutListResource extends FramedResource {
    ...
    static {
        ...
        a = new FileAttribute("file", null, Attribute.EDITABLE | Attribute.MANDATORY);
        ATTR_FILE = AttributeRegistry.registerAttribute(c, a);
        a = new FileAttribute("space", null, Attribute.EDITABLE | Attribute.MANDATORY);
        ATTR_SPACE = AttributeRegistry.registerAttribute(c, a);
        a = new FileAttribute("root", null, Attribute.EDITABLE | Attribute.MANDATORY);
        ATTR_ROOT = AttributeRegistry.registerAttribute(c, a);
        a = new BooleanAttribute("auto-publish", Boolean.FALSE, Attribute.EDITABLE);
        ATTR_AUTO_PUBLISH = AttributeRegistry.registerAttribute(c, a);
        a = new BooleanAttribute("auto-delete", Boolean.FALSE, Attribute.EDITABLE);
        ATTR_AUTO_DELETE = AttributeRegistry.registerAttribute(c, a);
        a = new IntegerAttribute("max-published", new Integer(10), Attribute.EDITABLE);
    ...
    protected void removeOldestPublishedEntry() {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(788);
while (penum.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(788);
{
            current = (PutedEntry) penum.nextElement();
            if (current.getTime() < oldest.getTime()) oldest = current;
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(788);
    ...
    protected synchronized void addPubEntry(PutedEntry e) {
        edu.hkust.clap.monitor.Monitor.loopBegin(789);
while (published.size() >= getMaxPublishedEntryStored()) { 
edu.hkust.clap.monitor.Monitor.loopInc(789);
removeOldestPublishedEntry();} 
edu.hkust.clap.monitor.Monitor.loopEnd(789);
    ...
    protected File getServerFile(File file) {
        ...
        if (!fpath.startsWith(fspace)) return null;
    ...
    public File getPubFile() {
        File file = getFile();
        if (file != null) return new File(file + ".pub"); else return null;
    }
    ...
    public File getDelFile() {
        File file = getFile();
        if (file != null) return new File(file + ".del"); else return null;
    }
    ...
    protected synchronized void write(File file, Enumeration genum) {
        ...
        try {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(790);
while (genum.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(790);
{
                v.addElement(genum.nextElement());
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(790);
    ...
    protected synchronized void removeUnconfirmedDelEntries() {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(791);
while (denum.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(791);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(791);
    ...
    protected synchronized void readList() {
        ...
        try {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(792);
for (int i = 0; i < holders.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(792);
addEntry((PutedEntry) holders[i]);} 
edu.hkust.clap.monitor.Monitor.loopEnd(792);
    ...
    protected synchronized void readPubList() {
        ...
        try {
            Reader reader = new BufferedReader(new FileReader(file));
            AttributeHolder holders[] = serializer.readAttributeHolders(reader);
            edu.hkust.clap.monitor.Monitor.loopBegin(793);
for (int i = 0; i < holders.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(793);
addPubEntry((PutedEntry) holders[i]);} 
edu.hkust.clap.monitor.Monitor.loopEnd(793);
    ...
    protected synchronized void readDelList() {
        ...
        try {
            Reader reader = new BufferedReader(new FileReader(file));
            AttributeHolder holders[] = serializer.readAttributeHolders(reader);
            edu.hkust.clap.monitor.Monitor.loopBegin(794);
for (int i = 0; i < holders.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(794);
{
                DeletedEntry e = (DeletedEntry) holders[i];
                e.confirm();
                addDelEntry(e);
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(794);
    ...
    protected PutedEntry lookupEntry(Request request) {
        ...
        if (rr != null) {
            try {
                r = rr.lock();
                if (r instanceof FileResource) k = ((FileResource) r).getFile().getAbsolutePath().toString();
    ...
    public synchronized int confirmDelete(Request request) {
        ...
        if (e != null) {
            e.confirm();
            if (getAutoDeleteFlag()) return delete(e);
        }
        return FILE_UC;
    }
    ...
    protected int delete(DeletedEntry de) {
        ...
        if (sfile.exists()) {
            if (debug) System.out.println("Deleting : " + sfile);
            sfile.delete();
            removeDelEntry(de.getKey());
            return FILE_DEL;
        } else {
            if (debug) System.out.println("Nothing to delete : " + sfile);
            removeDelEntry(de.getKey());
            return FILE_UC;
        }
    }
    ...
    protected int publish(PutedEntry pe) {
        ...
        try {
            ...
            if (c.status(file.getName()) == CVS.FILE_M) {
                ...
                String env[] = { "USER=" + author, "LOGNAME=" + author };
                String msg = ((author != null) ? "Published by " + author + " through Jigsaw" : "Published through Jigsaw");
                c.commit(file.getName(), msg, env);
            } else if (debug) {
                System.out.println("PutList: no commit needed on " + file.getAbsolutePath() + " st=" + c.status(file.getName()));
            }
            ...
            try {
                ...
                if (debug) {
                    System.out.println("publishing " + CvsDirectory.statusToString(cvs_status) + " file : " + filename);
                }
                if (cvs_status == CVS.FILE_C) {
                    ...
                    try {
                        ...
                        if (cvs_status == CVS.FILE_M) {
                            String author = pe.getAuthor();
                            String env[] = { "USER=" + author, "LOGNAME=" + author };
                            String msg = ((author != null) ? "Merged by " + author + " through Jigsaw" : "Merged through Jigsaw");
                    ...
                    System.out.println("PutList: no update needed on " + sfile.getAbsolutePath() + " st=" + CvsDirectory.statusToString(cvs_status));
                }
            ...
            pe.setValue(PutedEntry.ATTR_TIME, new Long(System.currentTimeMillis()));
            addPubEntry(pe);
    ...
    public void initialize(Object values[]) {
        ...
        File file = getFile();
        if ((file != null) && file.exists()) readList();
        File pub = getPubFile();
        if ((pub != null) && pub.exists()) readPubList();
        File del = getDelFile();
        if ((del != null) && del.exists()) readDelList();
