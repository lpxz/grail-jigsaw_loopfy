...
public class AutoLookupDirectory extends CvsRootDirectory {
    ...
    static {
        ...
        a = new BooleanAttribute("cvs-extensible", Boolean.FALSE, Attribute.EDITABLE);
    ...
    protected synchronized CvsDirectory getCvsManager() throws CvsException {
        if (cvs == null) {
            cvs = CvsModule.getCvsManager(getDirectory(), getContext(), getServer().getProperties());
        }
        return cvs;
    }
    ...
    public ResourceReference lookup(String name) {
        ...
        if (rr == null) {
            try {
                CvsDirectory cvs = getCvsManager();
                if (cvs.getDirectoryStatus(name) == CVS.DIR_NCO) cvs.updateDirectory(name);
            } catch (CvsException ex) {
