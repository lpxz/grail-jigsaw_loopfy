...
public class CvsDAVFileFrame extends DAVFrame {
    ...
    static {
        ...
        Class cls = null;
        try {
            cls = Class.forName("org.w3c.jigedit.webdav.frames.CvsDAVFileFrame");
        } catch (Exception ex) {
            ex.printStackTrace();
            System.exit(1);
        }
        a = new BooleanAttribute("autoupdate", Boolean.FALSE, Attribute.EDITABLE);
        ATTR_AUTOUPDATE = AttributeRegistry.registerAttribute(cls, a);
    }

    protected static Reply error(Request request, int status, String title, String msg) {
    ...
    protected synchronized File getResourceDirectory() {
        if (resDirectory == null) {
            ...
            Resource p = null;
            edu.hkust.clap.monitor.Monitor.loopBegin(246);
while (true) { 
edu.hkust.clap.monitor.Monitor.loopInc(246);
{
                try {
...
edu.hkust.clap.monitor.Monitor.loopEnd(246);
    ...
    public String computeETag() {
        ...
        if (fresource != null) {
            ...
            if (lstamp >= 0L) {
                etag_s = Long.toHexString(lsize) + "-" + Long.toHexString(lstamp);
            }
        }
        return etag_s;
    }
    ...
    protected void add(Request request) throws CvsException {
        ...
        if (mtype != null && (mtype.match(MimeType.TEXT) != MimeType.MATCH_SUBTYPE) && (mtype.match(MimeType.APPLICATION_XHTML_XML) != MimeType.MATCH_SPECIFIC_SUBTYPE)) {
            names = new String[2];
            names[0] = "-kb";
     ...
     * @exception CvsException if the CVS process failed
     */
    protected void commit(Request request, String msg) throws CvsException {
        String u = (String) request.getState(AuthFilter.STATE_AUTHUSER);
        String env[] = { "USER=" + u, "LOGNAME=" + u };
    ...
    protected Reply putFileResource(Request request) throws ProtocolException, ResourceException {
        if (debug) System.out.println("Put on " + getResource().getIdentifier() + "...");
        if (getResourceDirectory() != null) {
            if (checkCvsManager()) {
                ...
                if (debug) System.out.println("... with a cvs status equals to : " + statusToString(cvs_status));
                if (cvs_status == CVS.FILE_C) {
                    String msg = "Conflict between working " + "revision and repository revision.<br> " + "Please update the file manually or use the CVS form.";
                    return error(request, HTTP.CONFLICT, "Conflict", msg);
                } else if (cvs_status == CVS.FILE_NCO) {
                    String msg = "File already in the repository " + "(added independently by second party).<br>" + "Please update the file manually or use the CVS form.";
                    return error(request, HTTP.CONFLICT, "Conflict", msg);
                    ...
                    try {
                        if (cvs_status == CVS.FILE_OK) {
                            ...
                            String msg = "File is not up to date, " + "please update the file manually " + "or use the CVS form.";
                            return error(request, HTTP.CONFLICT, "Error", msg);
                        }
                    } catch (UpToDateCheckFailedException utd_ex) {
                        resfile.delete();
                        backup.renameTo(resfile);
                        String msg = utd_ex.getFilename() + " is not up to date, " + "please update the file manually " + "or use the CVS form.";
                        return error(request, HTTP.CONFLICT, "Error", msg);
                    } catch (CvsException ex) {
                        backup.delete();
                        String msg = "CvsException : " + ex.getMessage();
                        getServer().errlog(getIdentifier() + " : " + ex.getMessage());
                        return error(request, HTTP.SERVICE_UNAVAILABLE, "Error", msg);
                    }
                } else {
                    Reply r = super.putFileResource(request);
                    try {
                        ...
                        String msg = "Cvs add failed : <br>'" + ex.getMessage() + "'<br>please update the file manually " + "or use the CVS form";
                        return error(request, HTTP.SERVICE_UNAVAILABLE, "Error", msg);
                    } catch (CvsException ex) {
                        ex.printStackTrace();
                        String msg = "Problem during cvs process : " + ex.getMessage();
                        getServer().errlog(getIdentifier() + " : " + msg);
                        return error(request, HTTP.INTERNAL_SERVER_ERROR, "Internal Server Error", msg);
                    }
     ...
     */
    protected synchronized Reply deleteFileResource(Request request) throws ProtocolException, ResourceException {
        if (fresource == null) {
            String msg = "Can't delete resource: " + resource.getIdentifier() + " is not associated with a FILE. Try again later.";
            Reply error = error(request, HTTP.FORBIDDEN, "Error", msg);
        ...
        if (names[0] == null) names[0] = fresource.getIdentifier();
        try {
            if (getCvsManager().status(names[0]) != CVS.FILE_Q) {
                getCvsManager().remove(names, "Deleted via HTTP delete method.", env);
            }
        ...
        try {
            ...
            String msg = "Can't delete resource: " + resource.getIdentifier() + " is locked. Try again later.";
            Reply error = error(request, HTTP.FORBIDDEN, "Error", msg);
            throw new HTTPException(error);
        }
    ...
    public ReplyInterface perform(RequestInterface req) throws ProtocolException, ResourceException {
        ...
        if ((method.equals("GET") || method.equals("HEAD") || method.equals("POST") || method.equals("PUT")) && isAutoUpdatable() && (fresource != null)) {
            try {
                update();
            } catch (CvsException ex) {
                String msg = "cvs update \"" + getFileResource().getFile().getName() + "\" failed.";
                getServer().errlog(this, msg);
            }
        }
