...
public class Slider extends Canvas {
    ...
    class PointerClickListener extends MouseAdapter {
        ...
        public void mouseClicked(MouseEvent e) {
            ...
            if (target == slider) slider.movePointerTo(e.getX(), e.getY());
        }
    ...
    class Pointer {
        ...
        public void fill3DPointer(Graphics g, int x, int y, int width, int height, boolean raised) {
        ...
        public void paint(Graphics g) {
            fill3DPointer(g, getLocation().x, getLocation().y, width, height, true);
        }
        ...
        public Point getNewLocation(int x, int y) {
            return new Point(getLocation().x + x, getLocation().y + y);
        }
    ...
    class Graduation {
        ...
        public String getStringValue() {
            if (isDouble) return String.valueOf((float) value); else return String.valueOf((long) value);
        }
    ...
    protected void updateCurrentGraduation() {
        if (pposition.x > (width - marginx - rect_margin_x)) {
            ...
            int i = 0;
            edu.hkust.clap.monitor.Monitor.loopBegin(1152);
while (i < graduations.length) { 
edu.hkust.clap.monitor.Monitor.loopInc(1152);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(1152);
    ...
    protected int getGoodX(int x) {
        if (x > (width - marginx - rect_margin_x)) {
            if (graduations[graduations.length - 1].x1 != pointer.getCenterX()) {
                currentGraduation = graduations[graduations.length - 1];
            ...
            int mindx = getGraduationLength() + 50;
            int i = 0;
            edu.hkust.clap.monitor.Monitor.loopBegin(1153);
while (i < graduations.length) { 
edu.hkust.clap.monitor.Monitor.loopInc(1153);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(1153);
    ...
    protected void updatePointerPosition(double value) {
        ...
        if (value > graduations[graduations.length - 1].value) setMax(value + 10 * step);
        edu.hkust.clap.monitor.Monitor.loopBegin(1154);
for (int i = 1; i < graduations.length - 2; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1154);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(1154);
    ...
    public void paint(Graphics g) {
        ...
        dbg.fillRect(marginx, marginy, width - (2 * marginx), height - (2 * marginy));
        if (border) {
            dbg.setColor(color.darker());
            dbg.drawRect(marginx, marginy, width - (2 * marginx), height - (2 * marginy));
        }
        dbg.setColor(Color.white);
        dbg.fill3DRect(marginx + rect_margin_x, marginy + rect_margin_y - guideHeight / 2, getGraduationLength(), guideHeight, false);
        paintGraduation(dbg);
        dbg.setColor(color.darker());
        pointer.paint(dbg);
    ...
    protected void updateGraduationPosition() {
        ...
        int i = 0;
        edu.hkust.clap.monitor.Monitor.loopBegin(1155);
while (i < graduations.length) { 
edu.hkust.clap.monitor.Monitor.loopInc(1155);
{
            if (manageLong) {
                graduations[i] = new Graduation(gradx, y1, gradx, y2, (long) (min + i * step));
            } else {
...
edu.hkust.clap.monitor.Monitor.loopEnd(1155);

    }

    protected void paintGraduation(Graphics g) {
        g.setColor(Color.black);
        Font font = new Font(g.getFont().getName(), g.getFont().getStyle(), g.getFont().getSize() - 1);
        g.setFont(font);
        edu.hkust.clap.monitor.Monitor.loopBegin(1156);
for (int i = 0; i < graduations.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1156);
graduations[i].draw(g, (i == 0) || (i == graduations.length - 1));} 
edu.hkust.clap.monitor.Monitor.loopEnd(1156);
    ...
    public void initialize(double min, double max, double step, boolean border) {
    ...
    protected void update() {
        ...
        updateGraduationPosition();
        if (graduations != null) currentGraduation = graduations[0];
        setVisible(true);
     ...
     */
    public Slider() {
        pposition = new Point(marginx + rect_margin_x, marginy + rect_margin_y);
        pointer = new Pointer(pointerWidth, pointerHeight, pposition);
        PointerClickListener clickListener = new PointerClickListener(this);
        PointerMotionListener motionListener = new PointerMotionListener(this);
