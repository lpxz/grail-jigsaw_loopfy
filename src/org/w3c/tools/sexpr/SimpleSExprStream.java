...
public class SimpleSExprStream extends PushbackInputStream implements SExprStream {
   ...
   */
    public Dictionary getSymbols() {
        if (!noSymbols && symbols == null) symbols = new Hashtable();
        return symbols;
    }
   ...
   */
    public Dictionary setSymbols(Dictionary symbols) {
        if (symbols == null) noSymbols = true;
   ...
   */
    public Readtable getReadtable() {
        if (readtable == null) readtable = new SimpleReadtable();
        return readtable;
    }
   ...
   * @exception IOException if any other I/O-related problem occurred
   */
    public Object parse(char c, SExprStream stream) throws SExprParserException, IOException {
        SExprParser parser = getReadtable().getParser(c);
        if (parser != null) return parser.parse(c, this); else if (c == '(') {
            if (getListsAsVectors()) return parseVector(new Vector(), ')'); else return parseList();
        } else if (c == '"') return parseString(); else if (isAtomChar(c, true)) return parseAtom(c); else throw new SExprParserException(c);
    }
   ...
   * @exception IOException if any other I/O-related problem occurred
   */
    protected Vector parseVector(Vector vector, char delimiter) throws SExprParserException, IOException {
        char c = readSkipWhite();
        if (c == delimiter) return vector; else {
            unread(c);
...
while (true) { 
{
   ...
   */
    public static void printExpr(Object expr, PrintStream out) {
        if (expr == null) out.print("nil"); else if (expr instanceof Number) out.print(expr); else if (expr instanceof String) {
            out.print('"');
            out.print(expr);
            out.print('"');
        } else if (expr instanceof Vector) {
            out.print("(");
            edu.hkust.clap.monitor.Monitor.loopBegin(487);
for (int i = 0; i < ((Vector) expr).size(); i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(487);
{
                if (i != 0) out.print(" ");
                printExpr(((Vector) expr).elementAt(i), out);
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(487);

            out.print(")");
        } else if (expr instanceof SExpr) ((SExpr) expr).printExpr(out); else out.print("#<unknown " + expr + ">");
    }
