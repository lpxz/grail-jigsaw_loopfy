...
public class Base64Decoder {
    ...
    private void printHex(int x) {
        int h = (x & 0xf0) >> 4;
        int l = (x & 0x0f);
        System.out.print((new Character((char) ((h > 9) ? 'A' + h - 10 : '0' + h))).toString() + (new Character((char) ((l > 9) ? 'A' + l - 10 : '0' + l))).toString());
    }

    private void printHex(byte buf[], int off, int len) {
        edu.hkust.clap.monitor.Monitor.loopBegin(310);
while (off < len) { 
edu.hkust.clap.monitor.Monitor.loopInc(310);
{
            printHex(buf[off++]);
            System.out.print(" ");
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(310);
    ...
    public void process() throws IOException, Base64FormatException {
        ...
        int got = -1;
        int ready = 0;
        edu.hkust.clap.monitor.Monitor.loopBegin(311);
        fill: 
while ((got = in.read(buffer)) > 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(311);
{
            int skiped = 0;
            edu.hkust.clap.monitor.Monitor.loopBegin(312);
while (skiped < got) { 
edu.hkust.clap.monitor.Monitor.loopInc(312);
{
                edu.hkust.clap.monitor.Monitor.loopBegin(313);
while (ready < 4) { 
edu.hkust.clap.monitor.Monitor.loopInc(313);
{
                    if (skiped >= got) continue fill;
                    int ch = check(buffer[skiped++]);
                    if (ch >= 0) chunk[ready++] = (byte) ch;
                }} 
edu.hkust.clap.monitor.Monitor.loopEnd(313);
...
edu.hkust.clap.monitor.Monitor.loopEnd(312);

        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(311);

        if (ready != 0) throw new Base64FormatException("Invalid length.");
        out.flush();
    }
