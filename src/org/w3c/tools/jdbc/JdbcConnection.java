...
public class JdbcConnection implements LRUAble {
    ...
    public static final int QUERY_TIMEOUT = 5 * 60;
    ...
    public synchronized boolean markUsed() {
        ...
        if (conn == null) {
            try {
                ...
                conn = DriverManager.getConnection(server.uri, server.user, server.password);
    ...
    public ResultSet performQuery(String command) throws SQLException {
        if (conn != null) {
            try {
    ...
    public int performUpdate(String command) throws SQLException {
        if (conn != null) {
            try {
                querystamp = System.currentTimeMillis();
                Statement smt = conn.createStatement();
