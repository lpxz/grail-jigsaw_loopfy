...
public class JdbcServer {
     ...
     */
    public static JdbcServer getServer(String uri, String user, String password) {
    ...
    public static JdbcServer getServer(String uri, Properties props) {
        return getServer(uri, Jdbc.getUser(props), Jdbc.getPassword(props), Jdbc.getDriver(props), props);
    }
     ...
     * @return an instance of JdbcServer 
     */
    public static JdbcServer getServer(String uri, String user, String password, String driver) {
     ...
     * @return an instance of JdbcServer 
     */
    public static JdbcServer getServer(String uri, String user, String password, String driver, Properties props) {
        if (drivers == null) drivers = new Vector(4);
    ...
    public ResultSet runRequest(String command, boolean close) throws SQLException {
        return runQuery(command, close);
    }

    public ResultSet runQuery(String command, boolean close) throws SQLException {
        if (debug) {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(890);
for (int i = 0; (reply == null) && (i < maxretry); i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(890);
{
            if ((conn = manager.getConnection(this)) == null) continue;
            try {
...
edu.hkust.clap.monitor.Monitor.loopEnd(890);
    ...
    public int runUpdate(String command, boolean close) throws SQLException {
        if (debug) {
            System.err.println(">>> " + command);
        }
        int reply = -1;
        JdbcConnection conn = null;
        int maxretry = 3;
        edu.hkust.clap.monitor.Monitor.loopBegin(891);
for (int i = 0; (reply == -1) && (i < maxretry); i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(891);
{
            if ((conn = manager.getConnection(this)) == null) continue;
            try {
...
edu.hkust.clap.monitor.Monitor.loopEnd(891);
    ...
    public DatabaseMetaData getMetaData() throws SQLException {
        ...
        JdbcConnection conn = null;
        int maxretry = 3;
        edu.hkust.clap.monitor.Monitor.loopBegin(892);
for (int i = 0; (reply == null) && (i < maxretry); i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(892);
{
            if ((conn = manager.getConnection(this)) == null) continue;
            try {
...
edu.hkust.clap.monitor.Monitor.loopEnd(892);
     ...
     */
    protected JdbcServer(String uri, String user, String password, ConnectionManager manager) {
        this.uri = uri;
        this.user = user;
