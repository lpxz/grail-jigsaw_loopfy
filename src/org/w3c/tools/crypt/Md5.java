...
public class Md5 {
    ...
    private static String stringify(byte buf[]) {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(290);
for (int i = 0; i < buf.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(290);
{
            int h = (buf[i] & 0xf0) >> 4;
            int l = (buf[i] & 0x0f);
...
edu.hkust.clap.monitor.Monitor.loopEnd(290);
    ...
    private final void decode(int output[], byte input[], int off, int len) {
        int i = 0;
        int j = 0;
        edu.hkust.clap.monitor.Monitor.loopBegin(291);
for (; j < len; i++, j += 4) { 
edu.hkust.clap.monitor.Monitor.loopInc(291);
{
            output[i] = (((int) (input[off + j] & 0xff)) | (((int) (input[off + j + 1] & 0xff)) << 8) | (((int) (input[off + j + 2] & 0xff)) << 16) | (((int) (input[off + j + 3] & 0xff)) << 24));
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(291);
    ...
    private final void update(byte input[], int len) {
        ...
        if (len >= partLen) {
            ...
            transform(buffer, 0);
            edu.hkust.clap.monitor.Monitor.loopBegin(292);
for (i = partLen; i + 63 < len; i += 64) { 
edu.hkust.clap.monitor.Monitor.loopInc(292);
transform(input, i);} 
edu.hkust.clap.monitor.Monitor.loopEnd(292);
    ...
    private byte[] end() {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(293);
for (int i = 0; i < 8; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(293);
bits[i] = (byte) ((count >>> (i * 8)) & 0xff);} 
edu.hkust.clap.monitor.Monitor.loopEnd(293);
    ...
    private byte[] encode(int input[], int len) {
        ...
        int i = 0;
        int j = 0;
        edu.hkust.clap.monitor.Monitor.loopBegin(294);
for (; j < len; i++, j += 4) { 
edu.hkust.clap.monitor.Monitor.loopInc(294);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(294);
    ...
    public byte[] getDigest() throws IOException {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(295);
while ((got = in.read(buffer)) > 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(295);
update(buffer, got);} 
edu.hkust.clap.monitor.Monitor.loopEnd(295);
