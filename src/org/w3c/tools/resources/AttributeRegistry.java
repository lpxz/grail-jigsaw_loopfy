...
class ClassAttributes {
    ...
    int add(Attribute attr) {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(1007);
for (int i = 0; i < attrs.size(); i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1007);
{
            String name = ((Attribute) attrs.elementAt(i)).getName();
            if (attr.getName().equals(name)) throw new DuplicateNameException(name);
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1007);
...
edu.hkust.clap.monitor.Monitor.loopEnd(590);
        ...
        try {
            ...
            System.err.println("Invalid attribute name \"" + ex.getName() + "\" " + "in class " + cls.getName() + ".");
            System.err.println("There is already an attribute defined with " + "this name.");
