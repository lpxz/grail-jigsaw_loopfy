...
edu.hkust.clap.monitor.Monitor.loopEnd(626);
     ...
     * because an abnormal situation occured.
     */
    public ReplyInterface ingoingFilter(RequestInterface request, FilterInterface filters[], int i) throws ProtocolException {
     ...
     * because an abnormal situation occured.
     */
    public ReplyInterface outgoingFilter(RequestInterface request, ReplyInterface reply) throws ProtocolException {
        return null;
    }
     ...
     * because an abnormal situation occured.
     */
    public ReplyInterface outgoingFilter(RequestInterface request, ReplyInterface reply, FilterInterface filters[], int fidx) throws ProtocolException {
        ReplyInterface fr = outgoingFilter(request, reply);
        return (fr != reply) ? fr : null;
    }

    public OutputStream outputFilter(RequestInterface request, ReplyInterface reply, OutputStream output) {
        return output;
    }
    ...
    public void pickleValues(Hashtable defs) {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(627);
for (int i = 0; i < nvalues.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(627);
{
            String attrname = attributes[i].getName();
            Object def = defs.get(attrname);
            if ((def != null) && ((i == ATTR_HELP_URL) || (i == ATTR_IDENTIFIER)) && (def instanceof String)) {
                nvalues[i] = ((String) def).intern();
            } else {
                nvalues[i] = def;
            }
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(627);
    ...
    public void initialize(Hashtable defs) {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(628);
for (int i = 0; i < values.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(628);
{
            String attrname = attributes[i].getName();
            Object def = defs.get(attrname);
            if (values[i] == null) {
...
edu.hkust.clap.monitor.Monitor.loopEnd(628);
