...
public class ResourceStoreImpl implements ResourceStore {
    ...
    protected final void markUsed() {
        if (manager != null) manager.markUsed(token);
    }
    ...
    protected void warning(String msg) {
        System.out.println("[" + getClass().getName() + "@" + repository + "]: " + msg);
    }
     ...
     */
    public Resource loadResource(String identifier, Hashtable defs) throws InvalidResourceException {
        loadResources();
        markUsed();
        ResourceIndex index = (ResourceIndex) resources.get(identifier);
        if (index == null) return null;
        if (defs == null) defs = new Hashtable(3);
    ...
    public void saveResource(Resource resource) {
        ...
        if (index == null) throw new UnknownResourceException(resource);
        if (index.isModified()) save();
        markUsed();
    }
    ...
    public synchronized boolean acceptUnload() {
        ...
        boolean accept = true;
        if ((manager != null) && (manager.getStoreSizeLimit() > 0) && resources.size() > manager.getStoreSizeLimit()) {
            accept = false;
        } else {
            Enumeration e = resources.elements();
            edu.hkust.clap.monitor.Monitor.loopBegin(1041);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(1041);
{
                ResourceIndex entry = (ResourceIndex) e.nextElement();
                Resource resource = entry.getResource();
                synchronized (entry) {
                    if (!resource.acceptUnload()) accept = false;
                }
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1041);

        }
        if (!accept) {
            try {
                if (modified) internalSave(false);
    ...
    protected synchronized void internalSave(boolean unload) throws IOException {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(1042);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(1042);
{
            Resource res = ((ResourceIndex) e.nextElement()).getResource();
            vres.addElement(res);
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1042);

        Resource resourcearray[] = new Resource[vres.size()];
        vres.copyInto(resourcearray);
        File tmp = new File(repository.getParent(), repository.getName() + ".tmp");
        ...
        modified = false;
        if (unload) {
            edu.hkust.clap.monitor.Monitor.loopBegin(1043);
for (int i = 0; i < resourcearray.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1043);
resourcearray[i].notifyUnload();} 
edu.hkust.clap.monitor.Monitor.loopEnd(1043);
    ...
    public synchronized void shutdown() {
        if (modified) {
            ...
            if (resources != null) {
                ...
                edu.hkust.clap.monitor.Monitor.loopBegin(1044);
while (entries.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(1044);
{
                    ResourceIndex index = (ResourceIndex) entries.nextElement();
                    index.unloadResource();
                }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1044);
    ...
    protected synchronized void loadResources() {
        ...
        if (resources == null) {
            try {
                ...
                if (repository.exists()) {
                    Reader reader = new BufferedReader(new FileReader(repository));
                    Resource resourceArray[] = serializer.readResources(reader);
                    edu.hkust.clap.monitor.Monitor.loopBegin(1045);
for (i = 0; i < resourceArray.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1045);
{
                        ResourceIndex entry = new ResourceIndex(resourceArray[i], false);
                        if (entry != null && entry.getIdentifier() != null) {
                            resources.put(entry.getIdentifier(), entry);
                        }
                    }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1045);
                ...
                ex.printStackTrace();
                String err = "Error in " + repository.getName() + " in dir " + repository.getParent() + ": [" + i + "] " + ex.getMessage();
                warning(err);
            }
        }
    }
     ...
     *    repository.
     */
    public void initialize(ResourceStoreManager manager, Object token, File repository, Serializer serializer) {
