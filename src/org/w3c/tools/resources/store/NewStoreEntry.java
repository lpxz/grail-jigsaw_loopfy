...
class NewReference implements ResourceReference {
    ...
    public Resource unsafeLock() throws InvalidResourceException {
        ...
        if (entry == null) {
            throw new InvalidResourceException(identifier, "This reference has been invalidated");
        }
        ResourceStore store = entry.getStore();
        Resource resource = store.lookupResource(identifier);
        if (debug) {
...
public class NewStoreEntry extends AttributeHolder implements LRUAble {
    ...
    synchronized void deleteStore() {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(902);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(902);
{
            rr = (NewReference) e.nextElement();
            rr.invalidate();
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(902);
     ...
     */
    ResourceReference addResource(Resource resource, Hashtable defs) {
...
edu.hkust.clap.monitor.Monitor.loopEnd(595);
    ...
    NewStoreEntry(ResourceStoreManager manager, String repository, Integer key) {
        this.manager = manager;
        this.store = null;
        this.references = new Hashtable(3);
    ...
    NewStoreEntry(ResourceStoreManager manager, File repository, Integer key) {
        this.manager = manager;
        this.store = null;
        this.rep = repository;
        this.references = new Hashtable(3);
