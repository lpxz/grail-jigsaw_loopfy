...
public class FileResource extends FramedResource {
    ...
    static {
        ...
        a = new FilenameAttribute("filename", null, Attribute.EDITABLE);
        ATTR_FILENAME = AttributeRegistry.registerAttribute(cls, a);
        a = new DateAttribute("file-stamp", new Long(-1), Attribute.COMPUTED);
        ATTR_FILESTAMP = AttributeRegistry.registerAttribute(cls, a);
        a = new IntegerAttribute("file-length", null, Attribute.COMPUTED);
        ATTR_FILE_LENGTH = AttributeRegistry.registerAttribute(cls, a);
        a = new BooleanAttribute("backup", Boolean.FALSE, Attribute.EDITABLE);
    ...
    public synchronized boolean newContent(InputStream in) throws IOException {
        ...
        try {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(208);
for (int got = 0; (got = in.read(buf)) > 0; ) { 
edu.hkust.clap.monitor.Monitor.loopInc(208);
fout.write(buf, 0, got);} 
edu.hkust.clap.monitor.Monitor.loopEnd(208);
    ...
    public synchronized void setValue(int idx, Object value) {
        if (idx == ATTR_IDENTIFIER) {
            ...
            if (getFilename() == null) {
                ...
                if ((oldfile != null) && (oldfile.exists())) super.setValue(ATTR_FILENAME, oldid);
            }
        }
        super.setValue(idx, value);
        if ((idx == ATTR_FILENAME) || (idx == ATTR_IDENTIFIER)) file = null;
    }
...
while (true) { 
{
                try {
                    ...
                    if (p instanceof DirectoryResource) {
                        ...
                        if (!d.unsafeGetShrinkableFlag()) return false; else {
    ...
    public void updateAttributes() {
        ...
        if ((stamp < 0) || (stamp != fstamp)) updateFileAttributes();
    }
