...
public class Resource extends AttributeHolder {
    ...
    static {
        ...
        a = new ObjectAttribute("parent", "org.w3c.tools.resources.Resource", null, Attribute.COMPUTED | Attribute.DONTSAVE);
        ATTR_PARENT = AttributeRegistry.registerAttribute(cls, a);
        a = new ObjectAttribute("context", "org.w3c.tools.resources.ResourceContext", null, Attribute.COMPUTED | Attribute.DONTSAVE);
        ATTR_CONTEXT = AttributeRegistry.registerAttribute(cls, a);
        a = new ObjectAttribute("store-entry", "java.lang.Object", null, Attribute.DONTSAVE);
        ATTR_STORE_ENTRY = AttributeRegistry.registerAttribute(cls, a);
        a = new StringAttribute("identifier", null, Attribute.MANDATORY | Attribute.EDITABLE);
        ATTR_IDENTIFIER = AttributeRegistry.registerAttribute(cls, a);
        a = new FrameArrayAttribute("frames", null, Attribute.COMPUTED);
        ATTR_RESOURCE_FRAMES = AttributeRegistry.registerAttribute(cls, a);
        a = new StringAttribute("url", null, Attribute.COMPUTED | Attribute.DONTSAVE);
        ATTR_URL = AttributeRegistry.registerAttribute(cls, a);
        a = new DateAttribute("last-modified", null, Attribute.COMPUTED | Attribute.EDITABLE);
        ATTR_LAST_MODIFIED = AttributeRegistry.registerAttribute(cls, a);
        a = new StringAttribute("help-url", null, Attribute.COMPUTED);
    ...
    public Object getClone(Object values[]) {
        ...
        if (f != null) {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(851);
for (int i = 0; i < f.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(851);
{
                if (f[i] == null) c[i] = null; else c[i] = (ResourceFrame) f[i].getClone();
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(851);
    ...
    public synchronized Object getValue(int idx, Object def) {
        if ((idx == ATTR_HELP_URL) && (values[ATTR_HELP_URL] == null)) values[ATTR_HELP_URL] = computeHelpUrl();
    ...
    protected void setContext(ResourceContext context, boolean keepmodules) {
        ...
        if (keepmodules) {
            ...
            if (ctxt != null) context.modules = ctxt.modules;
        }
        setValue(ATTR_CONTEXT, context);
    }
    ...
    public ResourceReference getResourceReference() {
        ...
        if (context != null) return context.getResourceReference();
        return null;
    }
    ...
    public void registerFrame(ResourceFrame frame, Hashtable defs) {
        synchronized (this) {
            ...
            if (frames == null) {
                ...
                int slot = -1;
                edu.hkust.clap.monitor.Monitor.loopBegin(852);
for (int i = 0; i < frames.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(852);
{
                    if (frames[i] == null) {
                        slot = i;
                        break;
...
edu.hkust.clap.monitor.Monitor.loopEnd(852);
    ...
    public synchronized void unregisterFrame(ResourceFrame frame) {
        ...
        if (frames != null) {
            ...
            int j = 0;
            edu.hkust.clap.monitor.Monitor.loopBegin(853);
for (int i = 0; i < frames.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(853);
{
                if (frames[i] == frame) {
...
edu.hkust.clap.monitor.Monitor.loopEnd(853);
    ...
    public synchronized ResourceFrame[] collectFrames(Class c) {
        ...
        if (frames != null) {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(854);
for (int i = 0; i < frames.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(854);
{
                if (c.isInstance(frames[i])) v.addElement(frames[i]);
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(854);
...
edu.hkust.clap.monitor.Monitor.loopEnd(854);
    ...
    public synchronized ResourceFrame getFrame(Class c) {
        ...
        frames = (ResourceFrame[]) getValue(ATTR_RESOURCE_FRAMES, null);
        if (frames != null) {
            edu.hkust.clap.monitor.Monitor.loopBegin(855);
for (int i = 0; i < frames.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(855);
{
                if (c.isInstance(frames[i])) return frames[i];
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(855);
...
edu.hkust.clap.monitor.Monitor.loopEnd(855);
    ...
    public synchronized Object getValue(Class c, int idx, Object def) {
        ...
        if (frame != null) return frame.getValue(idx, def);
    ...
    public void setValue(int idx, Object value) {
        if (idx == ATTR_IDENTIFIER) {
            ...
            if (rr != null) {
                try {
                    Resource r = rr.lock();
                    setValue(ATTR_URL, r.getURLPath() + java.net.URLEncoder.encode((String) value));
                } catch (Exception ex) {
                    ex.printStackTrace();
                } finally {
                    rr.unlock();
        ...
        super.setValue(idx, value);
        if ((!attributes[idx].checkFlag(Attribute.DONTSAVE)) && (idx != ATTR_LAST_MODIFIED)) markModified();
    }
    ...
    protected void checkMultipleLock(ResourceReference rr) throws MultipleLockException {
        if (rr.nbLock() > 1) throw new MultipleLockException(rr.nbLock(), this, "can't delete");
    }
    ...
    public void pickleValues(Hashtable defs) {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(856);
for (int i = 0; i < nvalues.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(856);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(856);
