...
public class XMLDescrReader extends HandlerBase implements JigXML {
    ...
    public void startElement(String name, AttributeList attributes) throws SAXException {
        ...
        if (iname == iINHERIT_TAG) {
            ...
            resourceStack.push(new EmptyDescription(resourceclass, identifier));
            ...
            try {
                ...
                ex.printStackTrace();
                throw new SAXException("Unable to create an instance of " + attrclass);
            }
    ...
    public void endElement(String name) throws SAXException {
        ...
        if (iname == iRESOURCE_TAG) {
            ...
            ResourceDescription res = (ResourceDescription) resourceStack.peek();
            ...
            AttributeDescription ad = new AttributeDescription(currentA, currentA.unpickle(array));
            Vector attrs = (Vector) defsStack.peek();
            attrs.addElement(ad);
            currentA = null;
        } else if (iname == iRESARRAY_TAG) {
            Vector vframes = (Vector) resourceSetStack.pop();
            ResourceDescription frames[] = new ResourceDescription[vframes.size()];
            vframes.copyInto(frames);
            AttributeDescription ad = new AttributeDescription((Attribute) FAStack.pop(), frames);
            Vector attrs = (Vector) defsStack.peek();
            attrs.addElement(ad);
        } else if ((iname == iCHILD_TAG) || (iname == iVALUE_TAG)) {
            isavalue = false;
    ...
    private void endCharacters() {
        ...
        if (currentS != null) {
            ...
            if (value.equals(NULL)) ad = new AttributeDescription(currentS, null); else ad = new AttributeDescription(currentS, currentS.unpickle(value));
            Vector attrs = (Vector) defsStack.peek();
            attrs.addElement(ad);
        } else if (isavalue) {
