...
public class XMLSerializer implements Serializer, JigXML {
     ...
     */
    public void writeResourceDescriptions(ResourceDescription descr[], Writer writer) throws IOException, SerializationException {
        XMLDescrWriter xmlwriter = null;
        try {
            xmlwriter = new XMLDescrWriter(writer);
            xmlwriter.startDocument();
            edu.hkust.clap.monitor.Monitor.loopBegin(393);
for (int i = 0; i < descr.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(393);
{
                xmlwriter.writeResourceDescription(descr[i]);
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(393);
     ...
     * @param writer the writer
     */
    public void writeResourceDescriptions(Resource descr[], Writer writer) throws IOException, SerializationException {
...
edu.hkust.clap.monitor.Monitor.loopEnd(393);
     ...
     * @param writer the writer
     */
    public void writeResources(AttributeHolder holders[], Writer writer) throws IOException, SerializationException {
        XMLResourceWriter xmlwriter = null;
        try {
            xmlwriter = new XMLResourceWriter(writer);
            xmlwriter.startDocument();
            edu.hkust.clap.monitor.Monitor.loopBegin(394);
for (int i = 0; i < holders.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(394);
{
                xmlwriter.writeResource(holders[i]);
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(394);
    ...
    protected Parser getParser() throws SerializationException {
        try {
            ...
            throw new SerializationException("Unable to intantiate : " + parser_name);
        }
    }
     ...
     */
    public LookupTable[] readAttributes(Reader reader, String attributes[]) throws IOException, SerializationException {
        Parser parser = getParser();
        XMLSubsetReader xmlreader = new XMLSubsetReader(reader, parser, attributes);
