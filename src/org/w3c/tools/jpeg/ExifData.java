...
public class ExifData {
    ...
    public byte[] getBytes(int offset, int length) {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(1049);
for (int count = offset; length > 0; count++, length--) { 
edu.hkust.clap.monitor.Monitor.loopInc(1049);
{
            raw[count - offset] = data[count];
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1049);
    ...
    protected String getString(int offset, int length, boolean nullTerminated) {
        ...
        String result = "";
        edu.hkust.clap.monitor.Monitor.loopBegin(1050);
for (int count = offset; (length > 0) && (!nullTerminated || data[count] != 0); count++, length--) { 
edu.hkust.clap.monitor.Monitor.loopInc(1050);
{
            short ub = data[count];
            ub = (short) (ub & 0xFF);
            String ch = "" + (char) ub;
...
edu.hkust.clap.monitor.Monitor.loopEnd(1050);
