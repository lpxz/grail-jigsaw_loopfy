...
public class Sorter {
    ...
    private static int compare(File file1, File file2) {
        if (file1.isDirectory() && file2.isFile()) return -1; else if (file1.isFile() && file2.isDirectory()) return 1; else return file1.compareTo(file2);
    }
    ...
    public static void orderedFileInsert(File file, Vector into) {
        ...
        int cmp = 0;
        if (hi >= lo) {
            edu.hkust.clap.monitor.Monitor.loopBegin(212);
while ((hi - lo) > 1) { 
edu.hkust.clap.monitor.Monitor.loopInc(212);
{
                idx = (hi - lo) / 2 + lo;
...
edu.hkust.clap.monitor.Monitor.loopEnd(212);
    ...
    static void orderedStringInsert(String key, Vector into) {
        ...
        int cmp = 0;
        if (hi >= lo) {
            edu.hkust.clap.monitor.Monitor.loopBegin(213);
while ((hi - lo) > 1) { 
edu.hkust.clap.monitor.Monitor.loopInc(213);
{
                idx = (hi - lo) / 2 + lo;
...
edu.hkust.clap.monitor.Monitor.loopEnd(213);
    ...
    static void quickSortStringArray(String array[], int lo0, int hi0) {
        ...
        if (hi0 > lo0) {
            mid = array[(lo0 + hi0) / 2];
            edu.hkust.clap.monitor.Monitor.loopBegin(214);
while (lo <= hi) { 
edu.hkust.clap.monitor.Monitor.loopInc(214);
{
                edu.hkust.clap.monitor.Monitor.loopBegin(215);
while ((lo < hi0) && (array[lo].compareTo(mid) < 0)) { 
edu.hkust.clap.monitor.Monitor.loopInc(215);
++lo;} 
edu.hkust.clap.monitor.Monitor.loopEnd(215);

                edu.hkust.clap.monitor.Monitor.loopBegin(216);
while ((hi > lo0) && (array[hi].compareTo(mid) > 0)) { 
edu.hkust.clap.monitor.Monitor.loopInc(216);
--hi;} 
edu.hkust.clap.monitor.Monitor.loopEnd(216);
...
edu.hkust.clap.monitor.Monitor.loopEnd(214);

            if (lo0 < hi) quickSortStringArray(array, lo0, hi);
            if (lo < hi0) quickSortStringArray(array, lo, hi0);
        }
    }
    ...
    public static Vector sortStringEnumeration(Enumeration e) {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(217);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(217);
{
            orderedStringInsert((String) e.nextElement(), sorted);
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(217);
    ...
    static void orderedComparableInsert(Comparable key, Vector into) {
        ...
        int cmp = 0;
        if (hi >= lo) {
            edu.hkust.clap.monitor.Monitor.loopBegin(218);
while ((hi - lo) > 1) { 
edu.hkust.clap.monitor.Monitor.loopInc(218);
{
                idx = (hi - lo) / 2 + lo;
...
edu.hkust.clap.monitor.Monitor.loopEnd(218);
    ...
    public static Vector sortComparableEnumeration(Enumeration e) {
        Vector sorted = new Vector();
        edu.hkust.clap.monitor.Monitor.loopBegin(219);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(219);
{
            orderedComparableInsert((Comparable) e.nextElement(), sorted);
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(219);
    ...
    static void quickSortCompArray(Comparable array[], int lo0, int hi0) {
        ...
        if (hi0 > lo0) {
            mid = array[(lo0 + hi0) / 2];
            edu.hkust.clap.monitor.Monitor.loopBegin(220);
while (lo <= hi) { 
edu.hkust.clap.monitor.Monitor.loopInc(220);
{
                edu.hkust.clap.monitor.Monitor.loopBegin(221);
while ((lo < hi0) && (mid.greaterThan(array[lo]))) { 
edu.hkust.clap.monitor.Monitor.loopInc(221);
++lo;} 
edu.hkust.clap.monitor.Monitor.loopEnd(221);

                edu.hkust.clap.monitor.Monitor.loopBegin(222);
while ((hi > lo0) && (array[hi].greaterThan(mid))) { 
edu.hkust.clap.monitor.Monitor.loopInc(222);
--hi;} 
edu.hkust.clap.monitor.Monitor.loopEnd(222);
...
edu.hkust.clap.monitor.Monitor.loopEnd(220);

            if (lo0 < hi) quickSortCompArray(array, lo0, hi);
            if (lo < hi0) quickSortCompArray(array, lo, hi0);
        }
    }
