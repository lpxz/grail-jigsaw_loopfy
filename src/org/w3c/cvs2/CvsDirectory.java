...
edu.hkust.clap.monitor.Monitor.loopEnd(16);
    ...
    File computeRepositoryDirectory(File dir) {
        ...
        File ret = null;
        if (!rep.exists()) return null;
        try {
            DataInputStream in = new DataInputStream(new FileInputStream(rep));
            String nm = in.readLine();
            if (nm.startsWith("/")) ret = new File(nm); else ret = new File(cvsroot, nm);
            in.close();
        } catch (Exception ex) {
            return null;
        }
    ...
    protected synchronized void createFileEntry(long timestamp, String name, int status) {
        if (entries == null) entries = new Hashtable(13);
     ...
     */
    public static synchronized CvsDirectory getManager(File directory, Properties props, String cvspath, String cvsroot, String cvswrap[]) throws CvsException {
        if (defprops == null) defprops = System.getProperties();
        ...
        cvs = new CvsDirectory(directory, ((props == null) ? defprops : props), cvspath, cvsroot, cvswrap);
        if (cache.size() >= cachesize) cacheUnload();
     ...
     * @exception CvsException If some initialisation failed
     */
    public static CvsDirectory getManager(File directory, Properties props) throws CvsException {
     ...
     * @exception CvsException If some initialisation failed
     */
    protected static CvsDirectory getManager(CvsDirectory father, File dir) throws CvsException {
    ...
    public static String statusToString(int st) {
        return (((st > 0) && (st < status.length)) ? status[st] : "unknown");
    }
     ...
     * @exception CvsException If the underlying CVS command failed.
     */
    protected void refresh(String filename) throws CvsException {
        synchronized (this) {
            LoadUpdateHandler handler = new LoadUpdateHandler(this);
            CvsStatusHandler statush = new CvsStatusHandler(this);
     ...
     * @exception CvsException If the underlying CVS command failed.
     */
    protected void refreshStatus(String filename) throws CvsException {
        synchronized (this) {
            LoadUpdateHandler handler = new LoadUpdateHandler(this);
     ...
     * @exception CvsException If the underlying CVS command failed.
     */
    protected void refreshRevision(String filename) throws CvsException {
        synchronized (this) {
            CvsStatusHandler statush = new CvsStatusHandler(this);
    ...
    protected synchronized void checkUse(String filename) throws CvsException {
        ...
        if ((entry == null) || entry.needsUpdate()) refresh();
    }
    ...
    protected synchronized void checkDirectoryUse() throws CvsException {
        if (repdir == null) {
            if ((repdir = computeRepositoryDirectory(getDirectory())) == null) throw new CvsException("Repository not accessible.");
        }
        if (cvsrep_stamp < repdir.lastModified()) {
            long stamp = System.currentTimeMillis();
            String subdirs[] = repdir.list(new DirectoryFilter());
            if (subdirs != null) {
                edu.hkust.clap.monitor.Monitor.loopBegin(634);
for (int i = 0; i < subdirs.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(634);
{
                    File subdir = new File(getDirectory(), subdirs[i]);
                    if (subdir.exists()) createDirectoryEntry(stamp, subdirs[i], DIR_CO); else createDirectoryEntry(stamp, subdirs[i], DIR_NCO);
                }} 
edu.hkust.clap.monitor.Monitor.loopEnd(634);

            }
            subdirs = getDirectory().list(new DirectoryFilter());
            if (subdirs != null) {
                edu.hkust.clap.monitor.Monitor.loopBegin(635);
for (int i = 0; i < subdirs.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(635);
{
                    if (getDirectoryEntry(subdirs[i]) == null) createDirectoryEntry(stamp, subdirs[i], DIR_Q);
                }} 
edu.hkust.clap.monitor.Monitor.loopEnd(635);
     ...
     * @exception CvsException If a CVS error occurs.
     */
    protected synchronized void checkDirectoryUse(String subdir) throws CvsException {
        if (repdir == null) {
            if ((repdir = computeRepositoryDirectory(getDirectory())) == null) throw new CvsException("Repository not accessible.");
        }
        if (cvsrep_stamp < repdir.lastModified()) {
            ...
            if (dir.exists()) {
                ...
                if (subd.exists()) createDirectoryEntry(stamp, subdir, DIR_CO); else createDirectoryEntry(stamp, subdir, DIR_NCO);
            } else {
    ...
    protected boolean needsUpdate() {
        ...
        if (entries != null) {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(636);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(636);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(636);
     ...
     * the cvs status of entries.
     */
    public int status(String filename, boolean refresh) throws CvsException {
        if (refresh) refreshStatus(filename);
    ...
    public int status(String filename) throws CvsException {
        lru.toHead(this);
        checkUse(filename);
        CvsEntry entry = (entries != null) ? (CvsEntry) entries.get(filename) : null;
        if (entry == null) {
            refresh(filename);
            entry = (entries != null) ? (CvsEntry) entries.get(filename) : null;
            if (entry == null) throw new CvsException(filename + ": no such entry.");
        }
     ...
     */
    public String revision(String filename) throws CvsException {
        lru.toHead(this);
        checkUse(filename);
        CvsEntry entry = (entries != null) ? (CvsEntry) entries.get(filename) : null;
        if (entry == null) {
            ...
            entry = (entries != null) ? (CvsEntry) entries.get(filename) : null;
            if (entry == null) throw new CvsException(filename + ": no such entry.");
        } else if ((entry.getRevision() == null) && (entry.getStatus() != FILE_Q)) {
     ...
     * @exception CvsException if some CVS errors occurs
     */
    public String stickyOptions(String filename) throws CvsException {
        lru.toHead(this);
        checkUse(filename);
        CvsEntry entry = (entries != null) ? (CvsEntry) entries.get(filename) : null;
        if (entry == null) {
     ...
     * @exception CvsException If CVS process failed.
     */
    public void updateRegexp(String regexp) throws CvsException {
        update(regexp);
    }
     ...
     * @exception CvsException If CVS process failed.
     */
    public void get(String path) throws CvsException {
        lru.toHead(this);
     ...
     */
    public void commit(String names[], String msg, String env[]) throws CvsException {
        lru.toHead(this);
     ...
     * @exception CvsException If some error occured during the CVS process.
     */
    public void commit(String file, String msg, String env[]) throws CvsException {
        lru.toHead(this);
     ...
     * @exception CvsException If some error occured during the CVS process.
     */
    public void commit(String names[], String msg) throws CvsException {
        lru.toHead(this);
        CvsCommitHandler handler = new CvsCommitHandler(this);
     ...
     * @exception CvsException If some error occured during the CVS process.
     */
    public void commit(String file, String msg) throws CvsException {
        String names[] = new String[1];
        names[0] = file;
        commit(names, msg);
    }
     ...
     * @exception CvsException If some CVS error occurs during the CVS process.
     */
    public void commit(String msg, String env[]) throws CvsException {
        lru.toHead(this);
        CvsCommitHandler handler = new CvsCommitHandler(this);
     ...
     * @exception CvsException If some error occured during the CVS process.
     */
    public void commitRegexp(String regexp, String comment, String env[]) throws CvsException {
     ...
     * @exception CvsException If some CVS error occurs during the CVS process.
     */
    public void revert(String filename, String revision, String msg, String env[]) throws CvsException {
        File file = new File(getDirectory(), filename);
        if (!file.exists()) throw new CvsException("the file " + file + " can't be reverted : " + "it doesn't exists!");
        file.delete();
        String names[] = new String[1];
        names[0] = filename;
     ...
     * @exception CvsException If some CVS error occurs during the CVS process.
     */
    public void revert(String filename, OutputStream out, String revision, String env[]) throws CvsException {
    ...
    public String diff(String filename) throws CvsException {
        ...
        checkUse(filename);
        CvsEntry entry = getFileEntry(filename);
        if ((entry != null) && (!entry.needsUpdate()) && (entry.getStatus() == FILE_OK)) return null;
     ...
     * @exception CvsException If some CVS error occurs during the process.
     */
    public void add(String names[], String env[]) throws CvsException {
        lru.toHead(this);
        long stamp = System.currentTimeMillis();
        runner.cvsAdd(this, names, env);
        edu.hkust.clap.monitor.Monitor.loopBegin(637);
for (int i = 0; i < names.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(637);
{
            CvsEntry entry = getFileEntry(names[i]);
            if (entry != null) entry.setStatus(stamp, FILE_A); else createFileEntry(stamp, names[i], FILE_A);
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(637);
     ...
     * @exception CvsException If some CVS error occurs during the process.
     */
    public void addRegexp(String regexp, String env[]) throws CvsException {
        lru.toHead(this);
        String files[] = new String[1];
        files[0] = regexp;
     ...
     */
    public void remove(String names[], String msg, String env[]) throws CvsException {
        lru.toHead(this);
        long stamp = System.currentTimeMillis();
        runner.cvsRemove(this, names);
        commit(names, msg, env);
        edu.hkust.clap.monitor.Monitor.loopBegin(638);
for (int i = 0; i < names.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(638);
{
            removeFileEntry(names[i]);
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(638);
     ...
     * @exception CvsException If some CVS error occurs during that process.
     */
    public void admin(String command[]) throws CvsException {
        lru.toHead(this);
    ...
    public void display(PrintStream prt) {
        ...
        try {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(639);
while (files.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(639);
{
                String name = (String) files.nextElement();
                int stat = status(name);
                prt.println(name + ": " + status[stat]);
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(639);
     ...
     * @exception CvsException if the CVS process failed.
     */
    public int getDirectoryStatus(String subdir, boolean update) throws CvsException {
        lru.toHead(this);
        if (update) checkDirectoryUse(); else checkDirectoryUse(subdir);
    ...
    public void updateDirectory(String subdir) throws CvsException {
        ...
        checkDirectoryUse();
        CvsEntry entry = getDirectoryEntry(subdir);
        if (entry == null) throw new CvsException("Unknown subdirectory " + subdir);
     ...
     * @exception CvsException If some initialisation failed
     */
    protected CvsDirectory(File directory, Properties props, String cvspath, String cvsroot, String cvswrap[]) throws CvsException {
        if (!directory.exists()) throw new UncheckedOutException("Unchecked out directory: " + directory.getAbsolutePath());
        this.props = props;
        this.cvspath = ((cvspath == null) ? props.getProperty(CVSPATH_P, cvspath_def) : cvspath);
        this.cvsroot = ((cvsroot == null) ? props.getProperty(CVSROOT_P, cvsroot_def) : cvsroot);
        this.cvswrapper = ((cvswrap == null) ? parseArrayProperty(props.getProperty(CVSWRAP_P, cvswrap_def)) : cvswrap);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            if (command == null) {
                ...
                if (params != null) cvs.update(params); else cvs.update();
            } else if (command.equals("status")) {
                if (params.length != 1) usage();
                ...
                if (params.length == 0) usage();
                cvs.add(params);
            } else if (command.equals("listdir")) {
                Enumeration e = cvs.listDirectories();
                edu.hkust.clap.monitor.Monitor.loopBegin(640);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(640);
{
                    String subdir = (String) e.nextElement();
                    System.out.println(subdir + ": " + status[cvs.getDirectoryStatus(subdir)]);
                }} 
edu.hkust.clap.monitor.Monitor.loopEnd(640);
