...
class CvsEntry {
    ...
    protected synchronized boolean needsUpdate() {
        ...
        if (dirrep == null) return (timestamp < file.lastModified());
        File filrep = new File(dirrep, file.getName() + ",v");
        return (((filrep != null) && filrep.exists()) ? ((timestamp < filrep.lastModified()) || (timestamp < file.lastModified())) : (timestamp < file.lastModified()));
    }

    CvsEntry(CvsDirectory cvs, long timestamp, String name, boolean isdir, int status) {
        this.cvs = cvs;
