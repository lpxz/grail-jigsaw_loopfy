...
public class TimeFormatter {
    ...
    public static String format(Date date, String format) {
        ...
        char ch;
        edu.hkust.clap.monitor.Monitor.loopBegin(277);
for (int i = 0; i < format.length(); i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(277);
{
            ch = format.charAt(i);
            if (ch == '%') {
                ++i;
...
edu.hkust.clap.monitor.Monitor.loopEnd(277);

        return buf.toString();
    }

    private static void appendPadded(StringBuffer buf, int n, int digits, char pad) {
        String foo = String.valueOf(n).trim();
        edu.hkust.clap.monitor.Monitor.loopBegin(278);
for (int i = 0; i < digits - foo.length(); i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(278);
buf.append(pad);} 
edu.hkust.clap.monitor.Monitor.loopEnd(278);

        buf.append(foo);
    }

    private static final void appendPadded(StringBuffer buf, int n, int digits) {
    ...
    public static void main(String[] args) {
        try {
            System.out.println(TimeFormatter.format(new Date(), args[0]));
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
