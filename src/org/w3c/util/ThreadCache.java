...
class CachedThread extends Thread {
    ...
    synchronized Runnable waitForRunner() {
        boolean to = false;
        edu.hkust.clap.monitor.Monitor.loopBegin(539);
while (alive) { 
edu.hkust.clap.monitor.Monitor.loopInc(539);
{
            if (runner != null) {
...
edu.hkust.clap.monitor.Monitor.loopEnd(539);
    ...
    public void run() {
        try {
            edu.hkust.clap.monitor.Monitor.loopBegin(540);
while (true) { 
edu.hkust.clap.monitor.Monitor.loopInc(540);
{
                Runnable torun = waitForRunner();
                if (torun != null) torun.run();
                if (!alive) break;
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(540);
...
public class ThreadCache {
    ...
    final synchronized void isDead(CachedThread t) {
        if (debug) System.out.println("** " + t + ": is dead tc=" + threadcount);
        if (!t.isTerminated()) {
            threadcount--;
            notifyAll();
        }
    ...
    protected synchronized CachedThread allocateThread(boolean waitp) {
        CachedThread t = null;
        edu.hkust.clap.monitor.Monitor.loopBegin(541);
while (true) { 
edu.hkust.clap.monitor.Monitor.loopInc(541);
{
            if (freelist != null) {
                if (debug) System.out.println("*** allocateThread: free thread");
                t = freelist;
                ...
                if (debug) System.out.println("*** create new thread.");
                t = createThread();
                break;
            } else if (waitp) {
                if (debug) System.out.println("*** wait for a thread.");
                try {
                    wait();
                } catch (InterruptedException ex) {
                }
...
edu.hkust.clap.monitor.Monitor.loopEnd(541);
    ...
    public synchronized void setCachesize(int cachesize, boolean update) {
        ...
        if (update) this.idlethreads = (cachesize >> 1);
    }
    ...
    public boolean getThread(Runnable runnable, boolean waitp) {
        if (debug) System.out.println("*** getting a thread for " + runnable);
        if (!inited) throw new RuntimeException("Uninitialized thread cache");
while (true) { 
{
    ...
    public synchronized void waitForCompletion() {
        edu.hkust.clap.monitor.Monitor.loopBegin(543);
while (usedthreads > 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(543);
{
            if (debug) System.out.println("*** Waiting for " + usedthreads + " threads");
            try {
                wait();
            } catch (InterruptedException ex) {
            }
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(543);
    ...
    public synchronized void initialize() {
        ...
        t.prev = null;
        t.start();
        edu.hkust.clap.monitor.Monitor.loopBegin(544);
for (int i = 1; i < idlethreads; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(544);
{
            t = createThread();
            t.next = freelist;
            t.prev = null;
...
edu.hkust.clap.monitor.Monitor.loopEnd(544);
