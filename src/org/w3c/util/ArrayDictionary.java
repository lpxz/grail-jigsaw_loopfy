...
public class ArrayDictionary extends Dictionary implements Cloneable {
    ...
    public Object get(Object key) {
        int n, i;
        edu.hkust.clap.monitor.Monitor.loopBegin(297);
for (i = 0, n = 0; i < keys.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(297);
{
            if (n >= nelems) break;
...
edu.hkust.clap.monitor.Monitor.loopEnd(297);
    ...
    public Object[] getMany(Object[] rKeys) {
        ...
        int i, n;
        edu.hkust.clap.monitor.Monitor.loopBegin(298);
for (i = 0, n = 0; i < keys.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(298);
{
            if (n >= nelems) break;
            if (keys[i] == null) continue;
            edu.hkust.clap.monitor.Monitor.loopBegin(299);
            inloop: 
for (int j = 0; j < rKeys.length; j++) { 
edu.hkust.clap.monitor.Monitor.loopInc(299);
if (keys[i].equals(rKeys[j])) {
                rValues[j] = values[i];
                break inloop;
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(299);

            n++;
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(298);
    ...
    public Object put(Object key, Object value) {
        int empty = -1;
        int i, n;
        edu.hkust.clap.monitor.Monitor.loopBegin(300);
for (i = 0, n = 0; i < keys.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(300);
{
            if (n >= nelems) break;
            if (keys[i] == null) {
                empty = i;
...
edu.hkust.clap.monitor.Monitor.loopEnd(300);
    ...
    public Object remove(Object key) {
        int i, n;
        edu.hkust.clap.monitor.Monitor.loopBegin(301);
for (i = 0, n = 0; i < keys.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(301);
{
            if (n >= nelems) break;
            if (keys[i] == null) continue;
            if (keys[i].equals(key)) {
...
edu.hkust.clap.monitor.Monitor.loopEnd(301);
    ...
    public String toString() {
        ...
        buf.append('[');
        edu.hkust.clap.monitor.Monitor.loopBegin(302);
for (int i = 0; i < keys.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(302);
{
            if (keys[i] == null) continue;
            buf.append(keys[i]);
            buf.append('=');
...
edu.hkust.clap.monitor.Monitor.loopEnd(302);
