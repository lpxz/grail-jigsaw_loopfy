...
public class XMLProperties extends Properties {
    ...
    class XMLParser implements DocumentHandler {
        ...
        private void compute(char[] ch, int start, int length) {
            int st = start;
            int len = length - 1;
            edu.hkust.clap.monitor.Monitor.loopBegin(499);
while (st < length && ((ch[st] == '\n') || (ch[st] == '\t') || (ch[st] == ' ') || (ch[st] == '\r'))) { 
edu.hkust.clap.monitor.Monitor.loopInc(499);
{
                st++;
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(499);

            edu.hkust.clap.monitor.Monitor.loopBegin(500);
while (len > 0 && ((ch[len] == '\n') || (ch[len] == '\t') || (ch[len] == ' ') || (ch[len] == '\r'))) { 
edu.hkust.clap.monitor.Monitor.loopInc(500);
{
                len--;
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(500);

            edu.hkust.clap.monitor.Monitor.loopBegin(501);
while (st <= len) { 
edu.hkust.clap.monitor.Monitor.loopInc(501);
{
                value.append(ch[st]);
                st++;
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(501);
    ...
    public synchronized void store(OutputStream out, String header) throws IOException {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(502);
for (Enumeration e = keys(); e.hasMoreElements(); ) { 
edu.hkust.clap.monitor.Monitor.loopInc(502);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(502);
    ...
    protected StringBuffer encode(String string) {
        ...
        char c;
        edu.hkust.clap.monitor.Monitor.loopBegin(503);
for (int i = 0; i < len; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(503);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(503);

        return buffer;
    }

    private Class getParserClass() throws ClassNotFoundException {
        if (parser_class == null) parser_class = Class.forName(PARSER_P);
        return parser_class;
    }
