...
class UrlPopup extends Frame implements ActionListener {
    ...
    private class Openner extends Thread {
        ...
        private Openner(ServerBrowser browser, AdminContext ac) {
    ...
    private String getAdminUrl() {
        ...
        if (adminUrl.length() < 1) return null; else if (!adminUrl.startsWith("http")) {
     ...
     */
    UrlPopup(String title, ServerBrowser browser, Frame frame, boolean keepOnCancel) {
        super(title);
...
class ServerMenu extends MenuBar implements ActionListener {
    ...
    public void actionPerformed(ActionEvent evt) {
        ...
        if (command.equals("open")) {
            new UrlPopup("Open new Admin-server", browser, (Frame) getParent(), true);
            ...
            cont.dispose();
            WindowCloser.windows--;
            if (WindowCloser.windows < 0) System.exit(0);
...
public class ServerBrowser extends TreeBrowser implements NodeHandler {
    ...
    public void popupDialog(String name) {
        if (popup == null) {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(323);
while (!ap.waitForCompletion()) { 
edu.hkust.clap.monitor.Monitor.loopInc(323);
;} 
edu.hkust.clap.monitor.Monitor.loopEnd(323);
    ...
    public void insertNode(RemoteResourceWrapper father, RemoteResourceWrapper son, String name) {
        TreeNode fatherNode;
        boolean ic = false;
        if (father == null) System.out.println("Error null father");
        fatherNode = getNode(father);
        if (fatherNode.getChildren() == TreeNode.NOCHILD) return;
        ...
        if (ic) insert(fatherNode, son, this, name, diricon); else insert(fatherNode, son, this, name, fileicon);
        repaint();
    }

    protected RemoteResourceWrapper getResources(RemoteResourceWrapper rw, String name) {
    ...
    private final Frame getFrame(Component c) {
        edu.hkust.clap.monitor.Monitor.loopBegin(358);
while (!(c instanceof Frame)) { 
edu.hkust.clap.monitor.Monitor.loopInc(358);
{
            c = c.getParent();
            if (c == null) return null;
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(358);
    ...
    protected void open(AdminContext ac) {
        ...
        if (rootResource != null) removeNode(rootResource);
        tl.focusChanged(null);
        edu.hkust.clap.monitor.Monitor.loopBegin(955);
while (!authorized) { 
edu.hkust.clap.monitor.Monitor.loopInc(955);
{
            try {
                authorized = true;
                ac.initialize();
...
edu.hkust.clap.monitor.Monitor.loopEnd(955);
    ...
    */
    public void notifyExpander(TreeBrowser browser, TreeNode nd) {
        ...
        if (ic) {
            ...
            setCursor(Frame.WAIT_CURSOR);
            authorized = false;
            edu.hkust.clap.monitor.Monitor.loopBegin(956);
while (!authorized) { 
edu.hkust.clap.monitor.Monitor.loopInc(956);
{
                authorized = true;
                try {
...
edu.hkust.clap.monitor.Monitor.loopEnd(956);

            Sorter.sortStringArray(names, true);
            if (debug) System.out.println("Found " + names.length + " identifiers");
            edu.hkust.clap.monitor.Monitor.loopBegin(957);
for (int i = 0; i < names.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(957);
{
                boolean nic = false;
...
edu.hkust.clap.monitor.Monitor.loopEnd(957);
        ...
        if (isDirectory(this, nd)) nd.setIcon(diropenedicon);
    }
    ...
    */
    ...
    public static void main(String args[]) {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(958);
for (int i = 0; i < args.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(958);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(958);
