...
public class SlideIntegerAttributeEditor extends AttributeEditor {
    ...
    public void initialize(RemoteResourceWrapper w, Attribute a, Object o, Properties p) throws RemoteAccessException {
        if (p != null) {
            ...
            long step;
            try {
                min = Long.parseLong(p.getProperty(MIN_P, "0"));
            } catch (NumberFormatException ex) {
                min = 0;
            }
            try {
                max = Long.parseLong(p.getProperty(MAX_P, "100"));
            } catch (NumberFormatException ex) {
                max = 100;
            }
            try {
                step = Long.parseLong(p.getProperty(STEP_P, "5"));
            } catch (NumberFormatException ex) {
                step = 5;
            }
        ...
        if (o == null) {
            ...
            if (v == null) if (a.getDefault() != null) v = (Integer) a.getDefault();
            if (v != null) {
                origs = v.intValue();
                widget.setValue((long) origs);
            }
