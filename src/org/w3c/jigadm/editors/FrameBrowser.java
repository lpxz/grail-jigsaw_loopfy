...
edu.hkust.clap.monitor.Monitor.loopEnd(358);
    ...
    public void renameNode(RemoteResourceWrapper rw, String label) {
        ...
        if (tn != null) {
            try {
                if (rw != rootResource) label = getFrameName(rw.getResource(), label);
    ...
    public FrameBrowser(FrameTreeListener tl, RemoteResourceWrapper rrw) {
        ...
        rootResource = rrw;
        String name = null;
        edu.hkust.clap.monitor.Monitor.loopBegin(359);
while (!authorized) { 
edu.hkust.clap.monitor.Monitor.loopInc(359);
{
            try {
                authorized = true;
                rr = rrw.getResource();
...
edu.hkust.clap.monitor.Monitor.loopEnd(359);

        if (name != null) initialize(rootResource, name, this, frameicon);
    }
     ...
     */
    public String getFrameName(RemoteResource frame, String name) throws RemoteAccessException {
     ...
     * @exception RemoteAccessException if a remote access error occurs.
     */
    public String getFrameName(RemoteResource frame) throws RemoteAccessException {
        String className = frame.getClassHierarchy()[0];
        String shortName = className.substring(className.lastIndexOf('.') + 1);
    ...
    public void notifyExpander(TreeBrowser browser, TreeNode nd) {
        ...
        try {
            ...
            if (frames != null) {
                if (debug) System.out.println("Found " + frames.length + " identifiers");
                edu.hkust.clap.monitor.Monitor.loopBegin(360);
for (int i = 0; i < frames.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(360);
{
                    RemoteResourceWrapper frw = new RemoteResourceWrapper(rrw, frames[i], serverBrowser);
                    browser.insert(nd, frw, this, getFrameName(frames[i]), frameicon);
                }} 
edu.hkust.clap.monitor.Monitor.loopEnd(360);
