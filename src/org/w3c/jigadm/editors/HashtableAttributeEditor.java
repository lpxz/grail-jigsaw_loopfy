    ...
    class HashtableAttributePopup extends ClosableFrame implements ActionListener, ItemListener {
        ...
        public void actionPerformed(ActionEvent evt) {
            ...
            if (command.equals("add")) {
                ...
                if (selectedKey != null) table.remove(selectedKey);
                String key = tkey.getText();
                String value = tvalue.getText();
                if (key.length() > 0) {
                    table.put(key, value);
                ...
                if ((isels != null) && (isels.length > 0)) {
                    modified = true;
                    edu.hkust.clap.monitor.Monitor.loopBegin(784);
for (int i = 0; i < isels.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(784);
{
                        String key = keys.getItem(isels[i]);
                        table.remove(key);
                        keys.deselect(isels[i]);
                    }} 
edu.hkust.clap.monitor.Monitor.loopEnd(784);

                    edu.hkust.clap.monitor.Monitor.loopBegin(785);
for (int i = 0; i < isels.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(785);
keys.delItem(isels[i] - i);} 
edu.hkust.clap.monitor.Monitor.loopEnd(785);
        ...
        protected void updateKeys() {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(786);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(786);
keys.addItem((String) e.nextElement());} 
edu.hkust.clap.monitor.Monitor.loopEnd(786);
        ...
        HashtableAttributePopup(HashtableAttributeComponent parent, ArrayDictionary table, String title) {
            super(title);
            this.parent = parent;
            this.table = table;
    ...
    class HashtableAttributeComponent extends ListEditor {
        ...
        protected void edit() {
            if (popup == null) popup = new HashtableAttributePopup(this, (ArrayDictionary) hashtable.clone(), "Edit"); else popup.start((ArrayDictionary) hashtable.clone());
            popup.show();
            popup.toFront();
        }
        ...
        protected void setTable(ArrayDictionary table) {
            ...
            list.removeAll();
            edu.hkust.clap.monitor.Monitor.loopBegin(787);
while (keys.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(787);
list.addItem((String) keys.nextElement());} 
edu.hkust.clap.monitor.Monitor.loopEnd(787);
        ...
        HashtableAttributeComponent(HashtableAttributeEditor editor, ArrayDictionary table) {
            super(3, false);
            this.editor = editor;
            setTable(table);
        }
    ...
    protected void createComponent(ArrayDictionary table) {
        if (comp == null) comp = new HashtableAttributeComponent(this, table);
    }
