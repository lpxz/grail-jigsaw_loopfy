...
public class SlideDoubleAttributeEditor extends AttributeEditor {
    ...
    public void initialize(RemoteResourceWrapper w, Attribute a, Object o, Properties p) throws RemoteAccessException {
        if (p != null) {
            ...
            double step;
            try {
                min = Double.valueOf(p.getProperty(MIN_P, "0.0")).doubleValue();
            } catch (NumberFormatException ex) {
                min = 0.0;
            }
            try {
                max = Double.valueOf(p.getProperty(MAX_P, "100.0")).doubleValue();
            } catch (NumberFormatException ex) {
                max = 100.0;
            }
            try {
                step = Double.valueOf(p.getProperty(STEP_P, "5.0")).doubleValue();
            } catch (NumberFormatException ex) {
                step = 5.0;
            }
            widget.initialize(min, max, step, p.getProperty(BORDER_P, "false").equals("true"));
        } else {
        ...
        if (o == null) {
            ...
            if (v == null) if (a.getDefault() != null) v = (Double) a.getDefault();
            if (v != null) {
                origs = v.doubleValue();
                widget.setValue(origs);
            }
