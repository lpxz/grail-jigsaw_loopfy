...
public class MimeTypeAttributeEditor extends AttributeEditor {
    ...
    class MimeTypeAttributePopup extends ClosableFrame implements ActionListener {
        ...
        MimeTypeAttributePopup(MimeTypeAttributeComponent parent) {
            ...
            e = mimeTypes.keys();
            edu.hkust.clap.monitor.Monitor.loopBegin(477);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(477);
{
                String major = (String) e.nextElement();
                Menu imenu = new Menu(major);
                imenu.addActionListener(this);
                minor = (String[]) mimeTypes.get(major);
                edu.hkust.clap.monitor.Monitor.loopBegin(478);
for (int i = 0; i < minor.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(478);
{
                    item = new MenuItem(minor[i]);
                    addMenuListener(item, major + "/" + minor[i]);
                    imenu.add(item);
                }} 
edu.hkust.clap.monitor.Monitor.loopEnd(478);

                menu.add(imenu);
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(477);
    ...
    class MimeTypeAttributeComponent extends Panel implements ActionListener, TextListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (command != null) {
                if (command.equals("edit")) {
                    if (popup == null) popup = new MimeTypeAttributePopup(this);
                    popup.show();
                }
            }
        }
