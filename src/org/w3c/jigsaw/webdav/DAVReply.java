...
public class DAVReply extends Reply implements WEBDAV {
    ...
    static {
        registerHeader(DAV_HEADER, "org.w3c.www.http.HttpString");
        registerHeader(LOCK_TOKEN_HEADER, "org.w3c.www.http.HttpString");
        registerHeader(STATUS_URI_HEADER, "org.w3c.www.webdav.DAVStatusURIList");
    }
    ...
    public void setStatusURI(int status, String uri) {
        DAVStatusURIList list = (DAVStatusURIList) getHeaderValue(STATUS_URI_HEADER);
    ...
    public static String getDAVReason(int status) {
        ...
        switch(category) {
            case 1:
                if ((catcode >= 0) && (catcode < dav_msg_100.length)) return dav_msg_100[catcode];
                break;
            case 2:
                if ((catcode >= 0) && (catcode < dav_msg_200.length)) return dav_msg_200[catcode];
                break;
            case 3:
                if ((catcode >= 0) && (catcode < dav_msg_300.length)) return dav_msg_300[catcode];
                break;
            case 4:
                if ((catcode >= 0) && (catcode < dav_msg_400.length)) return dav_msg_400[catcode];
                break;
            case 5:
                if ((catcode >= 0) && (catcode < dav_msg_500.length)) return dav_msg_500[catcode];
                break;
        }
        return null;
    }
     ...
     */
    public DAVReply(Client client, Request request, short major, short minor, int status) {
