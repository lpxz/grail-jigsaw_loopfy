...
public class AuthRealm extends ExternalContainer {
    ...
    public void registerResource(String name, Resource resource, Hashtable defs) {
        if (resource instanceof AuthUser) {
            registerUser(AuthUser.makeUser(resource, name, new ResourceContext(getContext())));
        }
    }
     ...
     * @param repository The file to use to store the realm database.
     */
    public static AuthRealm makeRealm(Resource res, ResourceContext context, String name) {
    ...
    public File getRepository(ResourceContext context) {
        return new File(context.getServer().getAuthDirectory(), getIdentifier() + ".db");
    }
