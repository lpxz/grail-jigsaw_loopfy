...
public class ServletWrapper extends FramedResource implements ServletConfig {
    ...
    private final void tuneCache(int capacity) {
        ...
        if (update != 0) {
            ...
            synchronized (threadCache) {
                ...
                if (debug) {
                    System.out.println("cacheupdate local=" + local + " global=" + global);
                }
            }
        }
    }
    ...
    static {
        ...
        a = new StringAttribute("servlet-class", null, Attribute.EDITABLE | Attribute.MANDATORY);
        ATTR_SERVLET_CLASS = AttributeRegistry.registerAttribute(cls, a);
        a = new PropertiesAttribute("servlet-parameters", null, Attribute.EDITABLE);
        ATTR_PARAMETERS = AttributeRegistry.registerAttribute(cls, a);
        a = new ObjectAttribute("servlet-context", "org.w3c.jigsaw.servlet.JigsawServletContext", null, Attribute.DONTSAVE);
        ATTR_SERVLET_CONTEXT = AttributeRegistry.registerAttribute(cls, a);
        a = new ObjectAttribute("session-context", "org.w3c.jigsaw.servlet.JigsawHttpSessionContext", null, Attribute.DONTSAVE);
        ATTR_SESSION_CONTEXT = AttributeRegistry.registerAttribute(cls, a);
        a = new LongAttribute("servlet-timeout", null, Attribute.EDITABLE);
        ATTR_SERVLET_TIMEOUT = AttributeRegistry.registerAttribute(cls, a);
        a = new IntegerAttribute("servlet-instancemax", null, Attribute.EDITABLE);
    ...
    public File getServletDirectory() {
        ...
        if (rr != null) {
            try {
                ...
                if (parent.definesAttribute("directory")) return (File) parent.getValue("directory", null);
    ...
    public ServletContext getServletContext() {
        ServletContext ctxt = (ServletContext) getValue(ATTR_SERVLET_CONTEXT, null);
        return ctxt;
    }
    ...
    protected void invalidateAllSession() {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(247);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(247);
{
            ctxt.getSession((String) e.nextElement()).invalidate();
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(247);
    ...
    protected void checkServlet() throws ClassNotFoundException, ServletException {
        synchronized (servletPool) {
            boolean classmodified = getLocalServletLoader().classChanged(getServletClass());
            if ((!inited) || classmodified || (servletPool.getLoadedClass() != getLocalServletLoader().loadClass(getServletClass()))) {
	 ...
	 * (using external synchronization)
        ...
        protected boolean add(Servlet servlet) {
            if (requiresInitialization()) {
                ...
                if (singleThreaded) maximum = getInstanceMax(); else maximum = 1;
                if (maximum > 0) {
	 ...
	 * pool (using external synchronization)
        ...
        protected Servlet remove() {
            if ((capacity > 0) && ((usage < 1) || (singleThreaded && (capacity > usage)))) {
            ...
            if (increasing) {
                ...
                if (usage < capacity2) {
                    ...
                    if (nexttime < 0) {
                        ...
                        if (debug) System.out.println("considering usage=" + usage + " next=" + capacity2 + " delay=" + idletime);
                    } else {
                        long wait = nexttime - now;
                        if (wait <= 0L) {
                            if (singleThreaded) {
                                edu.hkust.clap.monitor.Monitor.loopBegin(248);
for (int i = capacity; i > capacity2; i--) { 
edu.hkust.clap.monitor.Monitor.loopInc(248);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(248);

                            } else capacity = capacity2;
                            nexttime = -1L;
                            tuneCache(capacity);
                            if (debug) System.out.println("reducing usage=" + usage + (singleThreaded ? " real" : " virtual") + " capacity=" + capacity + " maximum=" + maximum + " thread=" + Thread.currentThread().getName());
                        }
                    }
                    if (usage < 1) {
                        if (debug) System.out.println("finalizing delay=" + idletime);
                        clean((idletime << 1));
                    }
                }
            }
...
while (true) { 
{
                        if (usage < capacity) {
                            ...
                            tune(true);
                            if (debug) System.out.println("taking usage=" + usage + " real capacity=" + capacity + " maximum=" + maximum + " servlet=" + servlet.hashCode() + " thread=" + Thread.currentThread().getName());
                            return servlet;
                        } else {
                            if ((maximum < 1) || (capacity < maximum)) {
                                if (launchServlet(loadedClass)) {
                                    ...
                                    if (debug) System.out.println("waiting thread=" + Thread.currentThread().getName());
                                    wait();
                                }
                            } else {
                                if (debug) System.out.println("waiting thread=" + Thread.currentThread().getName());
                ...
                tune(true);
                if (debug) System.out.println("taking usage=" + usage + " virtual capacity=" + capacity + " maximum=" + maximum + " servlet=" + servlet.hashCode() + " thread=" + Thread.currentThread().getName());
                return servlet;
            }
        }
	 ...
	 * initialized
         */
        protected synchronized void releaseServlet(Servlet servlet, int duration) throws ServletException {
            if (requiresInitialization()) {
            ...
            if (usage > 0) {
                if (singleThreaded) {
                    ...
                    tune(false);
                    if (debug) System.out.println("releasing usage=" + usage + " real capacity=" + capacity + " maximum=" + maximum + " servlet=" + servlet.hashCode() + " thread=" + Thread.currentThread().getName());
                    notifyAll();
                } else {
                    usage--;
                    tune(false);
                    if (debug) System.out.println("releasing usage=" + usage + " virtual capacity=" + capacity + " maximum=" + maximum + " servlet=" + servlet.hashCode() + " thread=" + Thread.currentThread().getName());
                }
            } else {
	 ...
	 */
        ...
        protected void signalTimeout() {
            synchronized (this) {
                stimer = null;
                if (t != null) {
                    if (debug) System.out.println("Killing " + t);
                    t.interrupt();
                }
            }
        }
        ...
        public void run() {
            ...
            stimer = server.timer.registerTimer(server.getRequestTimeOut(), this, null);
            if (debug) System.out.println("running servlet=" + srServlet.hashCode() + " thread=" + Thread.currentThread().getName());
            Object o = null;
            try {
                ...
                if (uex.isPermanent()) {
                    message = "<h2>The servlet is permanently " + "unavailable :</h2>" + "Details: <b>" + uex.getMessage() + "</b>";
                    try {
                    ...
                    if (delay > 0) {
                        message = "<h2>The servlet is temporarily " + "unavailable :</h2>" + "Delay : " + delay + " seconds<br><br>Details: <b>" + uex.getMessage() + "</b>";
                        ...
                        message = "<h2>The servlet is temporarily " + "unavailable :</h2>" + "Details: <b>" + uex.getMessage() + "</b>";
                        try {
                            srResp.sendError(HTTP.SERVICE_UNAVAILABLE, message);
                        } catch (IOException ioex) {
                        }
                ...
                if (srResp.isStreamObtained()) {
                    ...
                    try {
                        srResp.sendError(HTTP.INTERNAL_SERVER_ERROR, "Servlet has thrown exception:" + ex.toString());
                    } catch (IOException ioex) {
                    }
                }
            } finally {
        ...
        ServletRunner(Servlet servlet, JigsawHttpServletRequest request, JigsawHttpServletResponse response) {
            srServlet = servlet;
            srReq = request;
            srResp = response;
        }
    ...
    protected void service(Request request, Reply reply) throws ServletException, IOException {
        ...
        JigsawServletContext jco = (JigsawServletContext) getServletContext();
        jReq = new JigsawHttpServletRequest(servlet, jco, request, jRes, getSessionContext());
    ...
    public void setValue(int idx, Object value) {
        super.setValue(idx, value);
        if (((idx == ATTR_SERVLET_CLASS) && (value != null)) || (idx == ATTR_PARAMETERS)) {
            try {
                synchronized (servletPool) {
                    inited = launchServlet();
                }
            } catch (Exception ex) {
                String msg = ("unable to set servlet class \"" + getServletClass() + "\" : " + ex.getMessage());
    ...
    protected void destroyServlet() {
        ...
        synchronized (servletPool) {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(250);
while (servlet != null) { 
edu.hkust.clap.monitor.Monitor.loopInc(250);
{
                try {
...
edu.hkust.clap.monitor.Monitor.loopEnd(250);
    ...
    protected boolean launchServlet(Class cls) throws ServletException {
        ...
        try {
            ...
            String msg = ("Illegal access during servlet instantiation, " + ex.getClass().getName() + ": " + ex.getMessage());
            if (debug) {
                ex.printStackTrace();
            }
            ...
            String msg = ("unable to instantiate servlet, " + iex.getClass().getName() + ": " + iex.getMessage());
            if (debug) {
                iex.printStackTrace();
            }
            getServer().errlog(this, msg);
    ...
    protected boolean launchServlet() throws ClassNotFoundException, ServletException {
        destroyServlet();
        if (inited) {
            String msg = "relaunching servlet failed due to incomplete \"" + getServletClass() + "\" cleanup.";
            getServer().errlog(this, msg);
            return false;
        } else {
            String clsname = getServletClass();
            if (clsname == null) {
                ...
                try {
                    ...
                    String msg = ("unable to find servlet class \"" + getServletClass() + "\"");
                    getServer().errlog(msg);
                    throw ex;
                }
