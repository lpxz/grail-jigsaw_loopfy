...
public class JigsawHttpServletRequest implements HttpServletRequest {
    ...
    private Hashtable convertParameters(Hashtable source) {
        if (source != null) {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(1165);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(1165);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(1165);
    ...
    private Hashtable mergeParameters(Hashtable source, Hashtable dest) {
        ...
        if (dest != null) {
            Enumeration e = source.keys();
            edu.hkust.clap.monitor.Monitor.loopBegin(1166);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(1166);
{
                String name = (String) e.nextElement();
                Object value = dest.get(name);
                if (value == null) dest.put(name, source.get(name)); else if (value instanceof String[]) {
...
edu.hkust.clap.monitor.Monitor.loopEnd(1166);
    ...
    private synchronized void prepareQueryParameters() {
        ...
        if (request.getMethod().equals("POST")) {
            if ((!request.hasContentType()) || (type.match(request.getContentType()) < 0)) return;
    ...
    public void setCharacterEncoding(String enc) throws java.io.UnsupportedEncodingException {
        InputStreamReader isr = new InputStreamReader(new PipedInputStream(), enc);
        encoding = enc;
    }
    ...
    */
    ...
    public String getRealPath(String name) {
        ...
        return JigsawServletContext.getRealPath(name, rr_root, request.getTargetResource());
    }
    ...
    public ServletInputStream getInputStream() throws IOException {
        if (stream_state == STREAM_READER_USED) throw new IllegalStateException("Reader used");
    ...
    public Enumeration getParameterNames() {
        prepareQueryParameters();
        return ((queryParameters == null) ? new EmptyEnumeration() : queryParameters.keys());
    }
    ...
    public Enumeration getLocales() {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(1167);
for (int i = 0; i < languages.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1167);
{
            ...
            if (size > 0) {
                ...
                edu.hkust.clap.monitor.Monitor.loopBegin(1168);
while ((qidx < size) && (ql.getLanguageQuality() >= quality)) { 
edu.hkust.clap.monitor.Monitor.loopInc(1168);
{
                    try {
...
edu.hkust.clap.monitor.Monitor.loopEnd(1168);

                locales.insertElementAt(new QLocale(lang, country, quality), qidx);
            } else {
                locales.addElement(new QLocale(lang, country, quality));
            }
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1167);

        int size = locales.size();
        Vector vlocale = new Vector(size);
        edu.hkust.clap.monitor.Monitor.loopBegin(1169);
for (int i = 0; i < size; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1169);
{
            vlocale.addElement(((QLocale) locales.elementAt(i)).getLocale());
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1169);
    ...
    public String getPathInfo() {
        if (request.hasState(JigsawRequestDispatcher.PATH_INFO_P)) {
            String pathinfo = (String) request.getState(JigsawRequestDispatcher.PATH_INFO_P);
    ...
    public String getPathTranslated() {
        ...
        if (pathinfo != null) return getRealPath(pathinfo);
        return null;
    }
    ...
    public String getQueryString() {
        if (request.hasQueryString()) {
            ...
            return (String) request.getState(JigsawRequestDispatcher.QUERY_STRING_P);
        }
        return null;
    }
    ...
    public long getDateHeader(String name) {
        ...
        if (v != null) {
            Object o = v.getValue();
            if ((o != null) && (o instanceof Long)) return ((Long) o).longValue();
        }
        return (long) -1;
    }
    ...
    public String getRequestURI() {
        ...
        if (request.hasState(JigsawRequestDispatcher.REQUEST_URI_P)) {
            uri = (String) request.getState(JigsawRequestDispatcher.REQUEST_URI_P);
            try {
    ...
    public RequestDispatcher getRequestDispatcher(String path) {
        ...
        return JigsawRequestDispatcher.getRequestDispatcher(urlpath, getServer(), rr);
    }
    ...
    public String getServletPath() {
        if (request.hasState(JigsawRequestDispatcher.SERVLET_PATH_P)) {
            return (String) request.getState(JigsawRequestDispatcher.SERVLET_PATH_P);
        } else {
            ResourceReference rr = request.getTargetResource();
            try {
    ...
    public Cookie[] getCookies() {
        ...
        if (cookielist != null) {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(1170);
for (int i = 0; i < cookies.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1170);
{
                Scookies[i] = convertCookie(cookies[i]);
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1170);
    ...
    protected Cookie convertCookie(HttpCookie httpCookie) {
        ...
        String val = null;
        if ((val = httpCookie.getDomain()) != null) cookie.setDomain(val);
        if ((val = httpCookie.getPath()) != null) cookie.setPath(val);
    ...
    protected String getRequestedSessionIdFromCookie() {
        HttpCookieList cookielist = request.getCookie();
        if (cookielist != null) {
            HttpCookie httpCookie = request.getCookie().getCookie(getCookieName());
            if (httpCookie != null) return httpCookie.getValue();
        }
        return null;
    }
    ...
    public String getRequestedSessionId() {
        if (requestedSessionID == null) {
            ...
            if (requestedSessionID == null) requestedSessionID = getRequestedSessionIdFromURL();
        }
    ...
    public HttpSession getSession(boolean create) {
        if (httpSession == null) {
            httpSession = (JigsawHttpSession) getSession(getRequestedSessionId());
            if (httpSession != null) httpSession.setNoMoreNew();
        }
        if (httpSession == null & create) {
            httpSession = new JigsawHttpSession(getSessionContext(), servletContext, createCookie());
    ...
    protected String getCookieName() {
        ObservableProperties props = request.getClient().getServer().getProperties();
        return props.getString(ServletProps.SERVLET_COOKIE_NAME, ServletProps.DEFAULT_COOKIE_NAME);
    }

    protected Cookie createCookie() {
        ObservableProperties props = request.getClient().getServer().getProperties();
        String name = props.getString(ServletProps.SERVLET_COOKIE_NAME, ServletProps.DEFAULT_COOKIE_NAME);
        String path = props.getString(ServletProps.SERVLET_COOKIE_PATH, "/");
        String domain = props.getString(ServletProps.SERVLET_COOKIE_DOMAIN, null);
        String comment = props.getString(ServletProps.SERVLET_COOKIE_COMMENT, null);
        int maxage = props.getInteger(ServletProps.SERVLET_COOKIE_MAXAGE, -1);
        boolean secure = props.getBoolean(ServletProps.SERVLET_COOKIE_SECURE, isSecure());
        Cookie cookie = new Cookie(name, null);
        cookie.setPath(path);
        cookie.setMaxAge(maxage);
        if ((comment != null) && (comment.length() > 0)) cookie.setComment(comment);
    ...
    protected HttpSession getSession(String sessionId) {
        if (sessionId != null) return getSessionContext().getSession(sessionId);
        return null;
    }
    ...
    public boolean isRequestedSessionIdValid() {
        JigsawHttpSession session = (JigsawHttpSession) getSession(getRequestedSessionId());
    ...
    public BufferedReader getReader() throws IOException {
        if (stream_state == INPUT_STREAM_USED) throw new IllegalStateException("Input Stream used");
        ...
        if (reader == null) {
            ...
            if (enc != null) {
                ...
                if (isr != null) reader = new BufferedReader(isr); else reader = new BufferedReader(new InputStreamReader(is));
            } else {
    ...
    JigsawHttpServletRequest(Servlet servlet, JigsawServletContext servletContext, Request request, JigsawHttpServletResponse response, JigsawHttpSessionContext sessionContext) {
