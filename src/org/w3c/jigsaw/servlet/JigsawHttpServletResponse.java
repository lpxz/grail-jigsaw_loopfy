...
public class JigsawHttpServletResponse implements HttpServletResponse {
    ...
    synchronized ServletOutputStream getJigsawOutputStream(boolean writerUsed) throws IOException {
        if (output != null) return output;
        if (state == STATE_ALL_DONE) {
            throw new IOException("Processing finished");
        }
        if (setContentTypeException != null) {
            throw new IOException("Illegal Content Type: " + setContentTypeException.toString());
        }
        if (request.hasState(INCLUDED)) {
            out = new ByteArrayOutputStream();
            output = new JigsawServletOutputStream(this, new DataOutputStream(out), buffer_size, writerUsed);
            ...
            if (reply.hasState(STREAM)) {
                try {
                    ...
                    output = new JigsawServletOutputStream(this, dos, buffer_size, writerUsed);
                    reply.setState(STREAM, null);
                } catch (ClassCastException ex) {
                    output = new JigsawServletOutputStream(this, reply, buffer_size, writerUsed);
                    reply.setState(STREAM, new Object());
                }
            } else {
                output = new JigsawServletOutputStream(this, reply, buffer_size, writerUsed);
     ...
     */
    public synchronized void sendError(int i, String msg) throws IOException {
     ...
     * @exception IOException If an I/O error has occurred. 
     */
    public synchronized void sendRedirect(String url) throws IOException {
        URL loc = null;
        if (isStreamObtained() && (output != null) && output.isCommitted()) {
            throw new IOException("Reply already started in servlet");
        }
        ...
        try {
            ...
            loc = new URL(requrl.getProtocol(), requrl.getHost(), requrl.getPort(), requri);
            loc = new URL(loc, url);
            setStatus(SC_MOVED_TEMPORARILY);
            reply.setLocation(loc);
            HtmlGenerator g = new HtmlGenerator("Moved");
            g.append("<P>This resource has moved, click on the link if your" + " browser doesn't support automatic redirection<BR>" + "<A HREF=\"" + loc.toExternalForm() + "\">" + loc.toExternalForm() + "</A>");
            reply.setStream(g);
        } catch (Exception ex) {
            ex.printStackTrace();
        } finally {
    ...
    public String encodeRedirectUrl(String url) {
        try {
            ...
            if (redirect.getHost().equals(requested.getHost()) && redirect.getPort() == requested.getPort()) return encodeUrl(url);
    ...
    public String encodeUrl(String url) {
        if (!jrequest.isRequestedSessionIdFromCookie()) {
            url = url + ((url.indexOf("?") != -1) ? "&" : "?") + jrequest.getCookieName() + "=" + jrequest.getSession(true).getId();
        }
        return url;
    }
    ...
    public synchronized PrintWriter getWriter() throws IOException, UnsupportedEncodingException {
        ...
        if (writer == null) {
            writer = new PrintWriter(new OutputStreamWriter(getJigsawOutputStream(true), getCharacterEncoding()));
        }
        return writer;
    }
     ...
     */
    protected synchronized void flushStream(boolean close) throws IOException {
        if (state == STATE_ALL_DONE) {
            return;
        }
        int writeLength;
        ...
        if (request.hasState(INCLUDED)) {
            ...
            if (fixedContentLength != CALC_CONTENT_LENGTH) {
                writeLength = (out.size() < fixedContentLength) ? (out.size()) : (fixedContentLength);
            } else {
            ...
            if (close) out.close(); else out.reset();
            rout.write(content);
            rout.flush();
        } else {
            if ((pout != null) && close) {
    ...
    public void addHeader(String name, String value) {
        ...
        if (hvalue == null) {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(108);
for (int i = 0; i < scookies.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(108);
{
                HttpCookie cookie = scookies[i];
                cl.addCookie(cookie.getName(), cookie.getValue());
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(108);
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(109);
for (int i = 0; i < scookies.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(109);
{
                scl.addSetCookie(scookies[i]);
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(109);
