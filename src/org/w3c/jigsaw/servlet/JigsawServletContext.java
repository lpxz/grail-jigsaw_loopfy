...
public class JigsawServletContext extends StructureChangedAdapter implements ServletContext, PropertyMonitoring {
    ...
    class Logger {
        ...
        String getDate() {
            Date now = new Date();
            return (now.getDate() + "/" + monthnames[now.getMonth()] + "/" + (now.getYear() + 1900) + ((now.getHours() < 10) ? (":0" + now.getHours()) : (":" + now.getHours())) + ((now.getMinutes() < 10) ? (":0" + now.getMinutes()) : (":" + now.getMinutes())) + ((now.getSeconds() < 10) ? (":0" + now.getSeconds()) : (":" + now.getSeconds())) + ((now.getTimezoneOffset() < 0) ? " " + (now.getTimezoneOffset() / 60) : " +" + (now.getTimezoneOffset() / 60)));
        }

        void log(String msg) {
            msg = "[" + getDate() + "] " + msg + "\n";
            try {
                if (log == null || closed) openLogFile();
                if (log != null) {
                    int len = msg.length();
                    if (len > msgbuf.length) msgbuf = new byte[len];
                    msg.getBytes(0, len, msgbuf, 0);
                    log.write(msgbuf, 0, len);
                }
            } catch (IOException ex) {
                System.out.println("Can't write (" + msg + ") to logfile [" + logfile + "] : " + ex.getMessage());
            }
        }
    ...
    public boolean propertyChanged(String name) {
        if (name.equals(ServletProps.SERVLET_LOG_FILE_P)) {
            if (logger != null) {
                logger.close();
                File newlogfile = new File((String) props.get(ServletProps.SERVLET_LOG_FILE_P));
                if (newlogfile.getPath().length() < 1) newlogfile = getServletLogFile();
    ...
    public File getServletLogFile() {
        ...
        File logfile = null;
        String file = (String) server.getProperties().getString(ServletProps.SERVLET_LOG_FILE_P, null);
        if (file != null) logfile = new File(file);
        if ((logfile != null) && (logfile.getPath().length() < 1)) logfile = null;
        if (logfile == null) {
            ...
            server.getProperties().putValue(ServletProps.SERVLET_LOG_FILE_P, logfile.getAbsolutePath());
        }
        return logfile;
    }
     ...
     */
    protected static String getRealPath(String path, ResourceReference rr_root, ResourceReference rr) {
    ...
    protected static String getFilePath(DirectoryResource dir) {
        HTTPFrame frame = (HTTPFrame) dir.getFrame("org.w3c.jigsaw.frames.HTTPFrame");
        String indexes[] = frame.getIndexes();
        if (indexes != null) {
            edu.hkust.clap.monitor.Monitor.loopBegin(275);
for (int i = 0; i < indexes.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(275);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(275);
    ...
    protected static ResourceReference getLocalRoot(ResourceReference rr_root, ResourceReference ref) {
        try {
            ...
            if (root instanceof VirtualHostResource) {
                ...
                edu.hkust.clap.monitor.Monitor.loopBegin(276);
while (true) { 
edu.hkust.clap.monitor.Monitor.loopInc(276);
{
                    try {
                        res = (FramedResource) rr.lock();
                        rrp = res.getParent();
                        if ((rrp == rr_root) || (rrp == null)) {
                        	edu.hkust.clap.monitor.Monitor.loopEnd(276);
                ...
                try {
                    ...
                    ForwardFrame ffr = (ForwardFrame) res.getFrame("org.w3c.jigsaw.proxy.ForwardFrame");
                    if (ffr == null) {
                        return rr_root;
                    } else {
    ...
    protected synchronized AutoReloadServletLoader createNewLocalServletLoader(boolean keepold) {
        if ((loader != null) && keepold) loader = new AutoReloadServletLoader(loader); else loader = new AutoReloadServletLoader(this);
        return loader;
    }
    ...
    public RequestDispatcher getRequestDispatcher(String urlpath) {
        return JigsawRequestDispatcher.getRequestDispatcher(urlpath, (httpd) getServer(), reference);
    }
    ...
    public RequestDispatcher getNamedDispatcher(String name) {
        ...
        return JigsawRequestDispatcher.getRequestDispatcher(name, reference, (httpd) getServer());
    }
        ...
        if (r_target != null) {
            try {
                ...
                if (target instanceof FramedResource) {
                    ServletDirectoryFrame frame = (ServletDirectoryFrame) ((FramedResource) target).getFrame("org.w3c.jigsaw.servlet.ServletDirectoryFrame");
                    if (frame != null) return frame.getServletContext();
                }
            } catch (InvalidResourceException ex) {
            } finally {
                r_target.unlock();
    ...
    protected JigsawServletContext(ResourceReference ref, ObservableProperties props) {
        ...
        try {
            ...
            if (!(res instanceof ServletDirectoryFrame)) {
                ...
                if (resource.definesAttribute("directory")) this.directory = (File) resource.getValue("directory", null);
            }
        } catch (InvalidResourceException ex) {
