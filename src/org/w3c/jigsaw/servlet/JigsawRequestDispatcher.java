...
public class JigsawRequestDispatcher implements RequestDispatcher {
    ...
    public void forward(ServletRequest request, ServletResponse response) throws ServletException, IOException {
        ...
        if (jres.isStreamObtained()) throw new IllegalStateException("Can't Forward! OutputStream or " + "Writer has allready been obtained.");
        ...
        try {
            if (host == null) {
                ...
                req.setURL(new URL(server.getURL().getProtocol(), host, urlpath));
            }
        ...
        try {
            ...
            if (pex.hasReply()) reply = (Reply) pex.getReply(); else {
                reply = req.makeReply(HTTP.INTERNAL_SERVER_ERROR);
        ...
        if (reply.hasStream()) {
            ...
            try {
                ...
                int len = -1;
                edu.hkust.clap.monitor.Monitor.loopBegin(1140);
while ((len = is.read(buffer, 0, 512)) != -1) { 
edu.hkust.clap.monitor.Monitor.loopInc(1140);
out.write(buffer, 0, len);} 
edu.hkust.clap.monitor.Monitor.loopEnd(1140);
                ...
                int len = -1;
                edu.hkust.clap.monitor.Monitor.loopBegin(1141);
while ((len = reader.read(buffer, 0, 512)) != -1) { 
edu.hkust.clap.monitor.Monitor.loopInc(1141);
writer.write(buffer, 0, len);} 
edu.hkust.clap.monitor.Monitor.loopEnd(1141);
    ...
    public void include(ServletRequest request, ServletResponse response) throws ServletException, IOException {
        ...
        String host = req.getHost();
        try {
            if (host == null) req.setURL(new URL(server.getURL(), urlpath)); else req.setURL(new URL(server.getURL().getProtocol(), host, urlpath));
        } catch (MalformedURLException ex) {
        }
...
edu.hkust.clap.monitor.Monitor.loopEnd(1141);
     ...
     */
    public static RequestDispatcher getRequestDispatcher(String name, ResourceReference rr, httpd server) {
        try {
            ...
            if (sdf.isServletLoaded(name)) {
                ...
                urlpath = urlpath.endsWith("/") ? urlpath + name : urlpath + "/" + name;
     ...
     * @return the RequestDispatcher
     */
    public static RequestDispatcher getRequestDispatcher(String urlpath, httpd server, ResourceReference rr) {
        ResourceReference rr_root = null;
        FramedResource root = null;
        rr_root = JigsawServletContext.getLocalRoot(server.getRootReference(), rr);
        try {
