...
public class AutoReloadServletLoader extends ClassLoader {
    ...
    private void trace(String msg, boolean display) {
        if (display) System.out.println("[" + context.getServletDirectory() + "]: " + msg);
    }
...
edu.hkust.clap.monitor.Monitor.loopEnd(271);
    ...
    private String noext(String name) {
        ...
        if ((idx != -1) && ((idx + 6 == name.length()) || (Character.isWhitespace(name.charAt(idx + 6))))) {
    ...
    protected final Class getCachedClass(String name, boolean resolve) throws ClassNotFoundException {
        ...
        if (entry != null) {
            if (!entry.isModified()) {
                ...
                if (resolve) resolveClass(entry.servletClass);
                entry.update();
    ...
    protected Class loadClassFile(File file) throws ClassNotFoundException {
        byte data[] = null;
        if (file == null) throw new ClassNotFoundException("invalid servlet base");
