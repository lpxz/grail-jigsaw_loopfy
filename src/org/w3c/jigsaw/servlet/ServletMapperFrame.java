...
public class ServletMapperFrame extends HTTPFrame {
    ...
    static {
        ...
        a = new StringAttribute("servlet-url", null, Attribute.EDITABLE);
    ...
    public ReplyInterface perform(RequestInterface req) throws ProtocolException, ResourceException {
        ...
        try {
            String target = null;
            if (request.hasQueryString()) target = getTarget() + "?" + request.getQueryString(); else target = getTarget();
            if (host == null) {
                request.setURL(new URL(server.getURL(), target));
            } else {
                int ic = host.indexOf(':');
                if (ic < 0) {
                    request.setURL(new URL(server.getURL().getProtocol(), host, target));
                } else {
                    request.setURL(new URL(server.getURL().getProtocol(), host.substring(0, ic), Integer.parseInt(host.substring(ic + 1)), target));
                }
            }
