...
public class ServletWrapperFrame extends HTTPFrame {
    ...
    public void registerResource(FramedResource resource) {
        ...
        if (resource instanceof ServletWrapper) wrapper = (ServletWrapper) resource;
    }
    ...
    public ReplyInterface perform(RequestInterface req) throws ProtocolException, ResourceException {
        ...
        try {
            ...
            Reply reply = request.makeReply(HTTP.INTERNAL_SERVER_ERROR);
            reply.setContent("The server was unable to find the " + "servlet class : " + ex.getMessage());
            if (wrapper.debug) ex.printStackTrace();
            throw new HTTPException(reply);
        } catch (ServletException ex) {
            Reply reply = request.makeReply(HTTP.INTERNAL_SERVER_ERROR);
            reply.setContent("The server was unable to initialize the " + "servlet : " + ex.getMessage());
            if (wrapper.debug) ex.printStackTrace();
            throw new HTTPException(reply);
        }
        if (!wrapper.isInited()) {
        ...
        try {
                ...
                if (server instanceof httpd) {
                    ...
                    if (strm != null) {
                        ...
                        if (strm instanceof PipedOutputStream) {
                            ...
                            r = (ServletWrapper.ServletRunner) reply.getState(ServletWrapper.RUNNER);
                            if (r != null) {
                                r.signalTimeout();
                            }
            ...
            if (uex.isPermanent()) {
                reply.setContent("<h2>The servlet is permanently " + "unavailable :</h2>" + "Details: <b>" + uex.getMessage() + "</b>");
            } else {
                int delay = uex.getUnavailableSeconds();
                if (delay > 0) {
                    reply.setRetryAfter(delay);
                    reply.setContent("<h2>The servlet is temporarily " + "unavailable :</h2>" + "Delay : " + delay + " seconds<br><br>Details: <b>" + uex.getMessage() + "</b>");
                } else {
                    reply.setContent("<h2>The servlet is temporarily " + "unavailable :</h2>" + "Details: <b>" + uex.getMessage() + "</b>");
                }
            }
            if (pis != null) {
                try {
    ...
    protected boolean lookupOther(LookupState ls, LookupResult lr) throws ProtocolException {
        ...
        if (request != null) {
            if (request.getState(JigsawRequestDispatcher.PATH_INFO_P) == null) request.setState(JigsawRequestDispatcher.PATH_INFO_P, extraPath);
        }
