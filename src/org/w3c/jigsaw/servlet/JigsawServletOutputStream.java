...
class JigsawServletOutputStream extends ServletOutputStream {
    ...
    private void flushBuffer() throws IOException {
        ...
        committed = true;
        if (out == null) {
            if (reply != null) out = new DataOutputStream(reply.getOutputStream());
        }
        if (count > 0) {
     ...
     */
    JigsawServletOutputStream(JigsawHttpServletResponse resp, DataOutputStream out, int bufsize, boolean writerUsed) {
        this.out = out;
        this.resp = resp;
     ...
     * @exception IllegalArgumentException if bufsize <= 0.
     */
    JigsawServletOutputStream(JigsawHttpServletResponse resp, Reply reply, int bufsize, boolean writerUsed) {
        this.resp = resp;
        this.reply = reply;
        this.writerUsed = writerUsed;
        if (bufsize <= 0) {
