...
public class CommonLogger extends Logger implements PropertyMonitoring {
     ...
     */
    protected synchronized void output(RandomAccessFile f, String msg) throws IOException {
        int len = msg.length();
    ...
    protected void openLogFile(int year, int month, int day) {
        ...
        try {
            ...
            throw new HTTPRuntimeException(this.getClass().getName(), "openLogFile", "unable to open " + logname);
        }
    }
    ...
    protected void openLogFile(int year, int month) {
        ...
        this.month = month;
        String ext = null;
        if (month < 9) ext = "_" + year + "_0" + (month + 1); else ext = "_" + year + "_" + (month + 1);
        String logname = getFilename(LOGNAME_P, "log") + ext;
        try {
            RandomAccessFile old = log;
            log = new RandomAccessFile(logname, "rw");
...
edu.hkust.clap.monitor.Monitor.loopEnd(349);
    ...
    protected String getFilename(String propname, String def) {
        ...
        if (filename == null) {
            ...
            if (logdirname == null) {
                ...
                if (root_dir == null) {
                    String msg = "unable to build a default value for the \"" + propname + "\" value.";
                    throw new HTTPRuntimeException(this.getClass().getName(), "getFilename", msg);
                }
            ...
            String logdirname = props.getString(LOGDIRNAME_P, null);
            if (logdirname == null) return filename;
            try {
                flogdir = new File(logdirname);
            } catch (RuntimeException ex) {
                String msg = "unable to access log directory " + logdirname;
    ...
    protected void openErrorLogFile() {
        ...
        try {
            ...
            throw new HTTPRuntimeException(this.getClass().getName(), "openErrorLogFile", "unable to open " + errlogname);
        }
    }
    ...
    protected void openTraceFile() {
        ...
        try {
            ...
            throw new HTTPRuntimeException(this.getClass().getName(), "openTraceFile", "unable to open " + tracename);
        }
    }
    ...
    public synchronized void sync() {
        try {
            ...
            server.errlog(getClass().getName() + ": IO exception in method sync \"" + ex.getMessage() + "\".");
        }
    }
    ...
    public synchronized void shutdown() {
        ...
        try {
            ...
            server.errlog(getClass().getName() + ": IO exception in method shutdown \"" + ex.getMessage() + "\".");
        }
    }
    ...
    public void initialize(httpd server) {
        ...
        openErrorLogFile();
        openTraceFile();
        if ((bufsize = props.getInteger(BUFSIZE_P, bufsize)) > 0) buffer = new byte[bufsize];
        return;
    }
