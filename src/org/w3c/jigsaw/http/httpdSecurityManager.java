...
public class httpdSecurityManager extends SecurityManager {
    ...
    protected void trace(String msg) {
        if (inAgent()) System.out.println("[agent-security] " + msg); else System.out.println("[httpd-security] " + msg);
    }

    public void checkAccept(String host, int port) {
        if (debug) trace("checkAccept: " + host + "@" + port);
        if (inAgent() && (!agent_accept)) throw new SecurityException();
        return;
    }

    public void checkAccess(Thread thr) {
        if (debug) trace("checkAccess: " + thr.getName());
        if (inAgent()) throw new SecurityException("Access denied to agents.");
        return;
    }

    public void checkCreateClassLoader() {
        if (debug) trace("checkCreateClassLoader.");
        if (inAgent()) throw new SecurityException("createClassLoader denied to agents.");
        return;
    }

    public void checkListen(int port) {
        if (debug) trace("checkListen: " + port);
        if (inAgent()) throw new SecurityException("Listen denied to agents.");
        return;
    }

    public void checkPropertiesAccess() {
        if (debug) trace("checkPropertiesAccess.");
        if (inAgent()) throw new SecurityException("Properties denied to agents");
        return;
    }

    public void checkRead(String file) {
        if (debug) trace("checkRead: " + file);
        if (inAgent()) throw new SecurityException("Read(file) denied to agents.");
        return;
    }

    public void checkRead(int fd) {
        if (debug) trace("checkRead: " + fd);
        if (inAgent()) throw new SecurityException("Read(fd) denied to agents.");
        return;
    }

    public void checkWrite(int fd) {
        if (debug) trace("checkWrite: " + fd);
        if (inAgent()) throw new SecurityException("Write(fd) denied to agents.");
        return;
    }

    public void checkWrite(String file) {
        if (debug) trace("checkWrite: " + file);
        if (inAgent() && (!agent_write)) throw new SecurityException("write(file) denied to agents.");
        return;
    }
}
