...
public class SocketClient extends Client implements Runnable {
    ...
    public String toString() {
        if (thread != null) return "client-" + state.id + "(" + thread.getName() + ")"; else return "client-" + state.id;
    }
    ...
    public void join() {
        if (thread != null) {
            edu.hkust.clap.monitor.Monitor.loopBegin(1004);
while (true) { 
edu.hkust.clap.monitor.Monitor.loopInc(1004);
{
                try {
                    thread.join();
    ...
    public void run() {
        ...
        if (trace) System.out.println(this + ": powered by " + thread);
        try {
            if (bufout == null) {
                int bufsize = getServer().getClientBufferSize() - 1;
                bufout = new SocketOutputBuffer(socket.getOutputStream(), bufsize);
            } else {
                bufout.reuse(socket.getOutputStream());
            }
            startConnection(new BufferedInputStream(socket.getInputStream()), new DataOutputStream(bufout));
        } catch (IOException ex) {
            if (debug) ex.printStackTrace();
        } catch (ClientException ex) {
            if (debug) ex.printStackTrace();
            if (debug) {
                if (ex.ex != null) ex.ex.printStackTrace(); else ex.printStackTrace();
            }
            if (alive && !idle) {
                error("caught ClientException: [" + ex.getClass().getName() + "] " + ex.getMessage());
            }
        } catch (Exception ex) {
            if (debug) {
    ...
    protected void stopConnection() {
        if (trace) System.out.println(this + ": stopConnection.");
        if (socket != null) {
            try {
                socket.close();
            } catch (Exception ex) {
    ...
    protected boolean idleConnection() {
        synchronized (state) {
            if (trace) System.out.println(this + ": idleConnection.");
            idle = true;
    ...
    protected void usedConnection() {
        synchronized (state) {
            if (trace) System.out.println(this + ": usedConnection.");
            idle = false;
            pool.notifyUse(this);
        }
    }
    ...
    protected synchronized void bind(Socket socket) {
        ...
        if (trace) System.out.println(this + ": bind.");
        this.socket = socket;
        try {
    ...
    protected synchronized void unbind() {
        if (trace) System.out.println(this + ": unbind.");
    ...
    protected synchronized void kill(boolean now) {
        if (trace) System.out.println(this + ": kill.");
        alive = false;
