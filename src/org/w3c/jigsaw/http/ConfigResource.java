...
class ConfigResourceEnumeration implements Enumeration {
    ...
    private static final String ids[] = { ConfigResource.SPACE_NAME, "properties", "indexers", "realms", "control" };
    ...
    public Object nextElement() {
        if (idx >= ids.length) throw new NoSuchElementException("config resource enumeration");
        return ids[idx++];
    }
...
public class ConfigResource extends AbstractContainer {
    ...
    public ConfigResource(httpd server) {
        this.server = server;
        this.propConfig = new DummyResourceReference(new PropertiesConfig(server));
        this.realmConfig = new DummyResourceReference(server.getRealmsCatalog());
        this.controlConfig = new DummyResourceReference(new ControlResource(server));
        this.indexers = new DummyResourceReference(server.getIndexersCatalog());
    }
}
