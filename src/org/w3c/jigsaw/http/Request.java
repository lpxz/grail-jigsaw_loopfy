...
public class Request extends HttpRequestMessage implements RequestInterface {
    ...
    public void notifyEndParsing(MimeParser parser) throws HttpParserException, IOException {
        ...
        if ((proto != null) && (proto.equals("http") || proto.equals("ftp"))) {
            try {
                if ((at = target.indexOf('@', 6)) != -1) {
                    String auth = target.substring(colon + 3, at);
                    int sep = -1;
                    if ((auth.indexOf('/') == -1) && ((sep = auth.indexOf(':')) != -1)) {
                        if (!hasAuthorization()) {
                        ...
                        setURL(new URL(proto + "://" + target.substring(at + 1)));
                    } else {
            ...
            try {
                String host = getHost();
                if (host == null) {
                    if ((major == 1) && (minor >= 1)) throw new HttpParserException("No Host Header");
                    ...
                    if (ic < 0) {
                        ...
                        setURL(new URL("http", host.substring(0, ic), Integer.parseInt(host.substring(ic + 1)), target));
                    }
                }
            } catch (Exception ex) {
                throw new HttpParserException("Bogus URL [" + url + "]", this);
    ...
    public boolean canKeepConnection() {
        if ((!keepcon) || (major < 1)) return false;
        if (minor >= 1) return hasConnection("close") ? false : true;
        if (is_proxy) return hasProxyConnection("keep-alive"); else return hasConnection("keep-alive");
    }
    ...
    public HttpMessage getClone() {
        ...
        cl.internal = true;
        if (cl.original == null) cl.original = this;
        return cl;
    }
    ...
    public InputStream getInputStream() throws IOException {
        ...
        if (te != null) {
            edu.hkust.clap.monitor.Monitor.loopBegin(1046);
for (int i = 0; i < te.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1046);
{
                if (te[i].equals("chunked")) in = new ChunkedInputStream(parser.getInputStream());
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1046);
    ...
    public static String unescape(String s) {
        ...
        int l = s.length();
        int ch = -1;
        edu.hkust.clap.monitor.Monitor.loopBegin(1047);
for (int i = 0; i < l; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1047);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(1047);
    ...
    public Reply makeReply(int status) {
        Reply reply = new Reply(client, this, getMajorVersion(), getMinorVersion(), status);
        if ((filters != null) && (infilters > 0)) reply.setFilters(filters, infilters);
        return reply;
    }
    ...
    public void skipBody() {
        if (getExpect() != null) return;
        try {
            InputStream is = getInputStream();
            int avail = is.available();
            edu.hkust.clap.monitor.Monitor.loopBegin(1048);
while (avail > 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(1048);
{
                is.skip(avail);
                avail = is.available();
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1048);
