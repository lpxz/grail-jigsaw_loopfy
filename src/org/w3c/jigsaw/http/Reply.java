...
public class Reply extends HttpReplyMessage implements ReplyInterface {
    ...
    public boolean tryKeepConnection() {
        if (!keep) {
            ...
            if (hasContentLength() || (is == null)) {
                if (is_proxy) addProxyConnection(ka); else addConnection(ka);
                return true;
            }
        }
        return false;
    ...
    public void setStream(org.w3c.jigsaw.html.HtmlGenerator g) {
        ...
        if (sendBody) setStream(g.getInputStream(), true);
    }
    ...
    public synchronized OutputStream getOutputStream(boolean doEmit) throws IOException {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(545);
while (--infilters >= 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(545);
output = filters[infilters].outputFilter(request, this, output);} 
edu.hkust.clap.monitor.Monitor.loopEnd(545);
    ...
    public void setContent(String msg, String encoding) {
        if (!hasContentType()) setHeaderValue(H_CONTENT_TYPE, DEFAULT_TYPE);
        byte byteBuffer[];
        try {
            byteBuffer = msg.getBytes(encoding);
        } catch (UnsupportedEncodingException ex) {
            throw new RuntimeException(this.getClass().getName() + "[setContent] Unable to convert" + "properly char to bytes");
        }
    ...
    public void emit(OutputStream out, int what) throws IOException {
        ...
        if (!hasContentType() && ((status != HTTP.CONTINUE) || (status != HTTP.SWITCHING))) {
     ...
     */
    public Reply(Client client, Request request, short major, short minor, int status) {
        this(client);
        ...
        if (request != null) if (request.getMethod().endsWith(HTTP.HEAD)) sendBody = false;
    }
}
