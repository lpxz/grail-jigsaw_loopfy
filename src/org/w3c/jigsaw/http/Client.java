...
public abstract class Client implements EventHandler {
    ...
    private static final byte hexaTable[] = { (byte) '0', (byte) '1', (byte) '2', (byte) '3', (byte) '4', (byte) '5', (byte) '6', (byte) '7', (byte) '8', (byte) '9', (byte) 'A', (byte) 'B', (byte) 'C', (byte) 'D', (byte) 'E', (byte) 'F' };
    ...
    private synchronized void removeTimeout() {
        if (timer != null) server.timer.recallTimer(timer);
    }
    ...
    protected boolean tryKeepConnection(Request request, Reply reply) {
        if (!server.getClientKeepConnection()) return false;
    ...
    protected Request getNextRequest() throws ClientException, IOException {
        ...
        if (debug) request.dump(System.out);
        return request;
    }
     ...
     */
    protected int chunkTransfer(InputStream is, Reply reply) throws IOException {
        byte zeroChunk[] = { ((byte) 48), ((byte) 13), ((byte) 10) };
        ...
        String header = null;
        try {
            edu.hkust.clap.monitor.Monitor.loopBegin(514);
while (got >= 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(514);
{
                if (got == 0) {
                    try {
                        got = is.read(buffer);
                    } catch (IOException ioex) {
                        if (reply.hasState(ServletWrapper.RUNNER) && (is instanceof PipedInputStream)) {
                ...
                bheader[30] = ((byte) 13);
                bheader[31] = ((byte) 10);
                edu.hkust.clap.monitor.Monitor.loopBegin(515);
while (sgot > 15) { 
edu.hkust.clap.monitor.Monitor.loopInc(515);
{
                    bheader[32 - blen] = hexaTable[sgot % 16];
                    sgot >>= 4;
                    blen++;
                }} 
edu.hkust.clap.monitor.Monitor.loopEnd(515);
...
edu.hkust.clap.monitor.Monitor.loopEnd(514);
    ...
    protected int emitReply(Reply reply) throws IOException {
        ...
        if (reply.getStatus() == HTTP.DONE) return -1;
        InputStream is = reply.openStream();
        if (is == null) {
            if (debug) reply.dump(System.out);
            reply.emit(output);
            return -1;
        } else {
            chunkable = reply.canChunkTransfer();
            if (debug) reply.dump(System.out);
            if (reply.getStatus() != HTTP.NOHEADER) reply.emit(output);
        }
        if (buffer == null) buffer = new byte[getServer().getClientBufferSize()];
        int written = 0;
        try {
            if (chunkable) {
                ...
                int got = 0;
                edu.hkust.clap.monitor.Monitor.loopBegin(516);
while (got >= 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(516);
{
                    output.write(buffer, 0, got);
                    written += got;
                    try {
...
edu.hkust.clap.monitor.Monitor.loopEnd(516);
    ...
    protected boolean startConnection(InputStream in, DataOutputStream out) throws ClientException {
        ...
        try {
            running = true;
            edu.hkust.clap.monitor.Monitor.loopBegin(517);
            alive_loop: 
while ((!interrupted) && keep) { 
edu.hkust.clap.monitor.Monitor.loopInc(517);
{
                try {
                    ...
                    if (processed == 0) {
                        if ((request = getNextRequest()) == null) break alive_loop;
                        major = request.getMajorVersion();
                        minor = request.getMinorVersion();
                    } else {
                        if (interrupted = idleConnection()) break alive_loop;
                        edu.hkust.clap.monitor.Monitor.loopBegin(518);
while ((request = getNextRequest()) == null) { 
edu.hkust.clap.monitor.Monitor.loopInc(518);
{
                            if (timedout) {
                                continue;
                            } else {
...
edu.hkust.clap.monitor.Monitor.loopEnd(518);
                    ...
                    if (cex.ex != null && (cex.ex instanceof HttpParserException)) {
                        ...
                        reqcount++;
                        break alive_loop;
                    } else if (cex.ex != null && (cex.ex instanceof MimeParserException)) {
                    ...
                    if (debug) genex.printStackTrace();
                    if (genex.getMessage().startsWith("Bad request") && (prev_body_count > 0)) {
                        usedConnection();
                        if (debug) System.out.println("Error after a body " + "request! (Skipping: " + prev_body_count + " bytes)");
                        input.reset();
                ...
                if (debuglog) System.out.println(this + ": request " + request.getURL());
                currentURI = request.getURL();
                tstart = System.currentTimeMillis();
                reply = processRequest(request);
                tend = System.currentTimeMillis();
                currentURI = null;
                if (keep) keep = tryKeepConnection(request, reply);
                sent = emitReply(reply);
                if (reply.hasContentLength() && (sent >= 0) && (reply.getContentLength() != sent)) keep = false;
                if (debuglog) System.out.println(this + ", cl=" + reply.getContentLength() + ", size=" + sent);
                prev_body_count = request.getContentLength();
                log(request, reply, sent, tend - tstart);
                processed++;
                if (keep) output.flush();
                ...
                if (request.hasState(JigsawHttpServletResponse.STREAM)) {
                    try {
                        ...
                        pis = (PipedInputStream) request.getState(JigsawHttpServletResponse.STREAM);
                        pis.close();
...
edu.hkust.clap.monitor.Monitor.loopEnd(517);
            ...
            try {
                InputStream i = null;
                if (reply != null) {
                    if ((i = reply.openStream()) != null) i.close();
                }
            } catch (IOException ioex) {
            }
            ...
            currentURI = null;
            if ((err == null) && (request != null) && (request.getContentLength() > 0) && (reply != null) && (reply.getStatus() != HTTP.SWITCHING) && (reply.getStatus() / 100 != 2)) {
                try {
                    InputStream i = request.getInputStream();
                    if (i != null) {
                        edu.hkust.clap.monitor.Monitor.loopBegin(519);
while (i.available() > 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(519);
i.read(buffer, 0, buffer.length);} 
edu.hkust.clap.monitor.Monitor.loopEnd(519);
    ...
    public int sendContinue() throws IOException {
        ...
        if ((major > 1) || ((major == 1) && (minor >= 1))) {
            if (contreply == null) contreply = new Reply(this, null, major, minor, HTTP.CONTINUE);
     ...
     * @exception IOException If some IO error occured.
     */
    public int sendContinue(Reply contReply) throws IOException {
        if (contReply == null) {
            return sendContinue();
        }
        if ((major > 1) || ((major == 1) && (minor >= 1))) {
