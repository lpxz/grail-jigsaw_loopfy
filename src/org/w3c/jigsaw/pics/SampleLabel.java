...
class LabelParser {
     ...
     */
    void expect(int c) throws InvalidLabelException {
        if (ch == c) return;
        String msg = ("expected " + (new Character((char) c)).toString() + "[" + c + "]" + " got " + (new Character((char) ch)).toString() + "[" + ch + "]");
        if (file == null) throw new InvalidLabelException(in.getLineNumber(), msg); else throw new InvalidLabelFileException(file, in.getLineNumber(), msg);
    }

    String parseVariableName() throws IOException {
        edu.hkust.clap.monitor.Monitor.loopBegin(1008);
while ((ch != '=') && (ch != '\n') && (ch != -1)) { 
edu.hkust.clap.monitor.Monitor.loopInc(1008);
{
            append(Character.toLowerCase((char) ch));
            ch = in.read();
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1008);

        return getToken(true);
    }

    String parseVariableValue() throws IOException {
        edu.hkust.clap.monitor.Monitor.loopBegin(1009);
while ((ch != -1) && (ch != '\n')) { 
edu.hkust.clap.monitor.Monitor.loopInc(1009);
{
            append(ch);
            ch = in.read();
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1009);

        return getToken(true);
    }

    SampleLabel parse(SampleLabel into) throws InvalidLabelException, InvalidLabelFileException {
        try {
            edu.hkust.clap.monitor.Monitor.loopBegin(1010);
while (true) { 
edu.hkust.clap.monitor.Monitor.loopInc(1010);
{
                switch(ch) {
                    case -1:
                    	edu.hkust.clap.monitor.Monitor.loopEnd(1010);
                        return into;
                    case ' ':
                    case '\t':
                    case '\n':
                        ...
                        continue;
                    case '#':
                        edu.hkust.clap.monitor.Monitor.loopBegin(1011);
while (((ch = in.read()) != '\n') && (ch != -1)) { 
edu.hkust.clap.monitor.Monitor.loopInc(1011);
;} 
edu.hkust.clap.monitor.Monitor.loopEnd(1011);
            ...
            if (file == null) throw new InvalidLabelException("IO exception."); else throw new InvalidLabelFileException(file.getAbsolutePath() + ": IO exception.");
        }
    }
    ...
    LabelParser(File file) throws InvalidLabelFileException {
        ...
        try {
            ...
            throw new InvalidLabelFileException(file.getAbsolutePath() + ": IO exception.");
        }
    }
...
public class SampleLabel implements LabelInterface {
    ...
    public void print(PrintStream out) {
        edu.hkust.clap.monitor.Monitor.loopBegin(1012);
for (int i = 0; i < varptr; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1012);
{
            System.out.println("\t" + vars[i] + " = " + vals[i]);
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1012);
    ...
    public boolean hasOption(String option) {
        edu.hkust.clap.monitor.Monitor.loopBegin(1013);
for (int i = 0; i < varptr; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1013);
{
            if (vars[i].equals(option)) return true;
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1013);
    ...
    public int getOptionIndex(String option) {
        edu.hkust.clap.monitor.Monitor.loopBegin(1014);
for (int i = 0; i < varptr; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1014);
{
            if (vars[i].equals(option)) return i;
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1014);
    ...
    public String getOptionValue(int idx) {
        if ((idx < 0) || (idx >= varptr)) throw new RuntimeException(this.getClass().getName() + "[getOptionValue]: " + " invalid index.");
        return vals[idx];
    }
    ...
    public String getOptionValue(String option) {
        edu.hkust.clap.monitor.Monitor.loopBegin(1015);
for (int i = 0; i < varptr; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1015);
{
            if (vars[i].equals(option)) return vals[i];
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1015);
    ...
    public boolean isGeneric() {
        ...
        if (idx >= 0) return (new Boolean(getOptionValue(idx))).booleanValue();
        return false;
    }
    ...
    public void dump(StringBuffer into, int format) {
        ...
        switch(format) {
            ...
            case LabelBureauInterface.FMT_FULL:
                edu.hkust.clap.monitor.Monitor.loopBegin(1016);
for (int i = 0; i < varptr; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1016);
{
                    if (vars[i].equals("ratings")) continue;
                    emit(space, into, i, LabelBureauInterface.FMT_FULL);
                    space = true;
                }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1016);
            ...
            default:
                throw new RuntimeException(this.getClass().getName() + "[dump]: " + " invalid format " + "\"" + format + "\"");
        }
    }
    ...
    public SampleLabel(String optnames[], String optvals[]) {
        if (optnames.length != optvals.length) throw new RuntimeException(this.getClass().getName() + " invalid constructor params:" + " bad length.");
        this.vars = optnames;
        this.vals = optvals;
