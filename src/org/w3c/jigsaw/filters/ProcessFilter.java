...
class ProcessFeeder extends Thread {
    ...
    public void run() {
        try {
            ...
            int emitted = 0;
            if (count >= 0) {
                edu.hkust.clap.monitor.Monitor.loopBegin(643);
while ((count > 0) && ((got = in.read(buffer)) > 0)) { 
edu.hkust.clap.monitor.Monitor.loopInc(643);
{
                    out.write(buffer, 0, got);
                    count -= got;
                    emitted += got;
                }} 
edu.hkust.clap.monitor.Monitor.loopEnd(643);

            } else {
                edu.hkust.clap.monitor.Monitor.loopBegin(644);
while ((got = in.read(buffer)) > 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(644);
{
                    out.write(buffer, 0, got);
                    emitted += got;
                }} 
edu.hkust.clap.monitor.Monitor.loopEnd(644);
...
public class ProcessFilter extends ResourceFilter {
    ...
    static {
        ...
        a = new StringArrayAttribute("command", null, Attribute.EDITABLE | Attribute.MANDATORY);
    ...
    public ReplyInterface outgoingFilter(RequestInterface req, ReplyInterface rep) throws ProtocolException {
        ...
        if (reply.getStatus() != HTTP.OK) return null;
        InputStream in = reply.openStream();
        if (in == null) return null;
        if (command == null) {
            Reply error = request.makeReply(HTTP.INTERNAL_SERVER_ERROR);
            error.setContent("The process filter of this resource is not " + " configured properly (it has no command).");
        ...
        try {
            ...
            error.setContent("The filter's process command " + command[0] + " wasn't found: " + e.getMessage());
            throw new HTTPException(error);
        }
        if (reply.hasContentLength()) {
            feeder = new ProcessFeeder(process, in, reply.getContentLength());
        } else {
