...
public class AccessLimitFilter extends ResourceFilter {
    ...
    static {
        ...
        a = new IntegerAttribute("limit", new Integer(1), Attribute.EDITABLE);
        ATTR_LIMIT = AttributeRegistry.registerAttribute(cls, a);
        a = new IntegerAttribute("timeout", new Integer(60000), Attribute.EDITABLE);
     ...
     */
    public synchronized ReplyInterface ingoingFilter(RequestInterface req) throws HTTPException {
        Request request = (Request) req;
        int limit = getLimit();
        int timeout = getTimeout();
        if (limit < 0) return null;
        edu.hkust.clap.monitor.Monitor.loopBegin(1114);
while (count >= limit) { 
edu.hkust.clap.monitor.Monitor.loopInc(1114);
{
            if (timeout > 0) {
                try {
                    wait((long) timeout, 0);
                ...
                if (count >= limit) {
                    String msg = "Simultaneous number of access to this page " + "is limited to " + limit + " you was not able to " + "get in.";
...
edu.hkust.clap.monitor.Monitor.loopEnd(1114);
     ...
     */
    public synchronized ReplyInterface outgoingFilter(RequestInterface req, ReplyInterface rep) {
        count--;
        notifyAll();
        return null;
    }
