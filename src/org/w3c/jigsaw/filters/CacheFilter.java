...
class CacheEntry extends LRUNode {
    ...
    private void readContent(Reply reply) throws IOException {
        ...
        if (reply.hasContentLength()) out = new ByteArrayOutputStream(reply.getContentLength()); else out = new ByteArrayOutputStream(8192);
...
edu.hkust.clap.monitor.Monitor.loopEnd(31);
     ...
     */
    CacheEntry(Request request, Reply reply, int defMaxAge) throws CacheException {
    ...
    Reply getReply(Request request) {
        ...
        if (etags != null && tag != null) {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(119);
for (int i = 0; i < etags.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(119);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(119);

            notMod = !noneMatch;
        } else {
            long ims = request.getIfModifiedSince();
            long lmd = this.reply.getLastModified();
            if (ims != -1 && lmd != -1) notMod = lmd > ims;
        }
        if (notMod) {
            System.out.println("**** replying NOT_MODIFIED");
            newReply.setStatus(HTTP.NOT_MODIFIED);
        } else if (!request.getMethod().equals("HEAD")) newReply.setStream(new ByteArrayInputStream(content)); else newReply.setStream((InputStream) null);
        return newReply;
    }
...
edu.hkust.clap.monitor.Monitor.loopEnd(120);
    ...
    public synchronized void remove(Request request) {
        ...
        CacheEntry ce = (CacheEntry) entries.remove(getNormalizedURL(request));
        if (ce == null) return;
        lruList.remove(ce);
    }
...
public class CacheFilter extends ResourceFilter {
    ...
    public int getMaxSize() {
        return ((Integer) getValue(ATTR_MAX_SIZE, new Integer(-1))).intValue();
    }

    public int getMaxEntries() {
        return ((Integer) getValue(ATTR_MAX_ENTRIES, new Integer(-1))).intValue();
    }

    public int getDefaultMaxAge() {
        return ((Integer) getValue(ATTR_DEFAULT_MAX_AGE, new Integer(300))).intValue();
    }
...
edu.hkust.clap.monitor.Monitor.loopEnd(122);
    ...
    private final Reply applyOut(Request request, Reply reply, FilterInterface[] filters) throws ProtocolException {
    ...
    public ReplyInterface ingoingFilter(RequestInterface req, FilterInterface[] filters, int fidx) throws ProtocolException {
        ...
        if (cache == null) cache = new Cache(getMaxSize(), getMaxEntries(), getDefaultMaxAge());
        String method = request.getMethod();
        if (!(method.equals("HEAD") || method.equals("GET"))) return null;
        tag(request);
        ...
        if (isCachable(reply)) {
            switch(reply.getStatus()) {
                ...
                case HTTP.NOT_MODIFIED:
                    CacheEntry cachEnt = cache.retrieve(request);
                    if (cachEnt != null) reply = cachEnt.getReply(request, reply);
                    break;
                default:
    ...
    private boolean isCachable(Reply reply) {
        if (reply.checkNoStore() || reply.getPrivate() != null) return false;
        return true;
    }
}
