...
public class RegexRedirectFilter extends ResourceFilter {
    ...
    static {
        ...
        a = new StringArrayAttribute("patterns", null, Attribute.EDITABLE);
    ...
    private Pattern[] getPatterns() {
        ...
        if (raw_pat == null) return null;
        patterns = new Pattern[raw_pat.length];
        substitutions = new String[raw_pat.length];
        int realsize = 0;
        PatternCompiler compiler = new Perl5Compiler();
        PatternMatcher matcher = new Perl5Matcher();
        edu.hkust.clap.monitor.Monitor.loopBegin(123);
for (int i = 0; i < raw_pat.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(123);
{
            try {
                if (matcher.matches(raw_pat[i], dec_pat)) {
                    patterns[realsize] = compiler.compile(matcher.getMatch().group(1));
...
edu.hkust.clap.monitor.Monitor.loopEnd(123);
    ...
    private String[] getSubstitutions() {
        if (substitutions != null) return substitutions;
        getPatterns();
    ...
    public ReplyInterface ingoingFilter(RequestInterface request) {
        ...
        String result = null;
        edu.hkust.clap.monitor.Monitor.loopBegin(124);
for (int i = 0; i < pat.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(124);
{
            if (matcher.matches(requrl, pat[i])) {
                Substitution s = new Perl5Substitution(sub[i]);
                result = Util.substitute(matcher, pat[i], s, requrl, Util.SUBSTITUTE_ALL);
                break;
            }
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(124);
        ...
        if (result != null) {
            ...
            g.append("<P>This resource has moved, click  if your browser" + " doesn't support automatic redirection<BR>" + "<A HREF=\"" + loc.toExternalForm() + "\">" + loc.toExternalForm() + "</A>");
            reply.setStream(g);
            return reply;
        }
        return null;
