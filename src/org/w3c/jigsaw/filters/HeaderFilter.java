...
public class HeaderFilter extends ResourceFilter {
    ...
    static {
        ...
        a = new StringAttribute("header-name", null, Attribute.EDITABLE);
        ATTR_HEADER_NAME = AttributeRegistry.registerAttribute(c, a);
        a = new StringAttribute("header-value", null, Attribute.EDITABLE);
        ATTR_HEADER_VALUE = AttributeRegistry.registerAttribute(c, a);
        a = new BooleanAttribute("no-cache", Boolean.FALSE, Attribute.EDITABLE);
        ATTR_NOCACHE = AttributeRegistry.registerAttribute(c, a);
        a = new BooleanAttribute("connection", Boolean.FALSE, Attribute.EDITABLE);
    ...
    private ReplyInterface modifyHeaders(ReplyInterface rep) {
        ...
        if (hname != null) {
            ...
            if (checkNoCache()) reply.addNoCache(hname);
            if (checkConnection()) reply.addConnection(hname);
        }
        return null;
    }
