...
public class URISizeLimiterFilter extends ResourceFilter {
    ...
    static {
        ...
        a = new IntegerAttribute("limit", new Integer(8192), Attribute.EDITABLE);
     ...
     */
    public int getLimit() {
        return getInt(ATTR_SIZE_LIMIT, 8192);
    }
    ...
    public synchronized ReplyInterface ingoingFilter(RequestInterface req) {
        ...
        if (request.getURL().toExternalForm().length() > limit) {
            ...
            g.append("Your request should have an URI of less than " + limit + " bytes");
            error.setStream(g);
            return error;
        }
        return null;
