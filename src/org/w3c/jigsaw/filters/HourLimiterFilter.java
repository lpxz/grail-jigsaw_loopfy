...
public class HourLimiterFilter extends ResourceFilter {
    ...
    public void setValue(int idx, Object value) {
        ...
        if (idx == ATTR_DATE_START) {
            ...
            a_time = cal.get(Calendar.HOUR_OF_DAY) * 3600 + cal.get(Calendar.MINUTE) * 60 + cal.get(Calendar.SECOND);
            ...
            b_time = cal.get(Calendar.HOUR_OF_DAY) * 3600 + cal.get(Calendar.MINUTE) * 60 + cal.get(Calendar.SECOND);
        }
    }
    ...
    public synchronized ReplyInterface ingoingFilter(RequestInterface req) {
        ...
        n_time = cal.get(Calendar.HOUR_OF_DAY) * 3600 + cal.get(Calendar.MINUTE) * 60 + cal.get(Calendar.SECOND);
        if (getDayRepeat()) {
            if ((n_time < a_time) || (n_time > b_time)) {
                reply = request.makeReply(HTTP.SERVICE_UNAVAILABLE);
                if (n_time < a_time) reply.setRetryAfter(a_time - n_time); else reply.setRetryAfter(84600 + n_time - a_time);
            }
            ...
            if ((n < a) || (n > b)) {
                reply = request.makeReply(HTTP.SERVICE_UNAVAILABLE);
                if (n < a) reply.setRetryAfter(a - n); else reply.setRetryAfter(84600 * 7 + n - a);
            }
            ...
            if ((n < a) || (n > b)) {
                reply = request.makeReply(HTTP.SERVICE_UNAVAILABLE);
                if (n < a) reply.setRetryAfter(a - n); else {
