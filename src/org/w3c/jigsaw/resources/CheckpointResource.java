...
public class CheckpointResource extends FramedResource implements Runnable {
    ...
    static {
        ...
        a = new IntegerAttribute("interval", new Integer(60), Attribute.EDITABLE);
        ATTR_INTERVAL = AttributeRegistry.registerAttribute(c, a);
        a = new IntegerAttribute("thread-priority", new Integer(2), Attribute.EDITABLE);
        ATTR_PRIORITY = AttributeRegistry.registerAttribute(c, a);
        a = new BooleanAttribute("flush-logs", Boolean.FALSE, Attribute.EDITABLE);
        ATTR_FLUSHLOG = AttributeRegistry.registerAttribute(c, a);
        a = new BooleanAttribute("flush-properties", Boolean.FALSE, Attribute.EDITABLE);
        ATTR_FLUSHPROPS = AttributeRegistry.registerAttribute(c, a);
        a = new BooleanAttribute("flush-configuration", Boolean.TRUE, Attribute.EDITABLE);
        ATTR_FLUSHCONFIG = AttributeRegistry.registerAttribute(c, a);
        a = new BooleanAttribute("trace-checkpoint", Boolean.FALSE, Attribute.EDITABLE);
    ...
    public void run() {
        ...
        int interval = -1;
        try {
            edu.hkust.clap.monitor.Monitor.loopBegin(1138);
while (alive) { 
edu.hkust.clap.monitor.Monitor.loopInc(1138);
{
                interval = getInterval();
                alive = ((getFlushLog() || getFlushProperties() || getFlushConfiguration()) && (interval > 0));
                if (!alive) break;
                synchronized (this) {
                    try {
                        wait(interval * 1000);
                ...
                checkpoint = new Date();
                if (getTraceFlag()) System.out.println("*** Checkpoint (" + server.getIdentifier() + ") at " + checkpoint);
                if (alive && getFlushConfiguration()) server.checkpoint();
                if (alive && getFlushLog()) {
                    Logger logger = ((httpd) getServer()).getLogger();
                    if (logger != null) logger.sync();
                }
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1138);

        } catch (Exception ex) {
            String msg = ("exception while running \"" + ex.getMessage() + "\". Stopped.");
