...
public abstract class GeneratedFrame extends PostableFrame {
     ...
     */
    protected void post(Request request, Reply reply, URLDecoder data, PageCompileOutputStream out) throws ProtocolException, IOException {
        Reply error = request.makeReply(HTTP.NOT_ALLOWED);
        if (allowed != null) error.setHeaderValue(Reply.H_ALLOW, allowed);
     ...
     * @exception IOException if an IO error occurs.
     */
    protected abstract void get(Request request, Reply reply, PageCompileOutputStream out) throws IOException;
}
