...
public class JpegComFrame extends HTTPFrame {
    ...
    static {
        ...
        a = new MimeTypeAttribute("comment-type", null, Attribute.EDITABLE);
    ...
    protected String getMetadata() {
        ...
        if (file.exists()) {
            ...
            comment = "";
            edu.hkust.clap.monitor.Monitor.loopBegin(931);
for (int i = 0; i < comments.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(931);
comment += comments[i];} 
edu.hkust.clap.monitor.Monitor.loopEnd(931);
        ...
        if (cometag == null) {
            ...
            if (fresource != null) {
                ...
                if (lstamp >= 0L) {
                    ...
                    etag_s = Integer.toString(getOid(), 32) + ":" + Long.toString(lstamp, 32);
                }
            }
    ...
    public void attributeChanged(AttributeChangedEvent evt) {
        ...
        if ((name.equals("file-stamp")) || (name.equals("file-stamp"))) comment = null;
    }
    ...
    public Reply createCommentReply(Request request, int status) {
        ...
        if (contentencoding != null) reply.setHeaderValue(Reply.H_CONTENT_ENCODING, contentencoding);
        ...
        reply.setContentLocation(getURL(request).toExternalForm() + ";" + URLEncoder.encode(commenttype));
        return reply;
    }
...
edu.hkust.clap.monitor.Monitor.loopEnd(263);
    ...
    protected boolean negotiate(Request request) throws ProtocolException {
        if (!request.hasAccept()) {
            ...
            HttpAccept imgAccept = getMatchingAccept(accepts, getContentType());
            HttpAccept comAccept = getMatchingAccept(accepts, getCommentType());
            ...
            } else if (comAccept != null) return true; else return false;
        }
    }
    ...
    protected HttpAccept getMatchingAccept(HttpAccept accepts[], MimeType mime) {
        int jmatch = -1;
        int jidx = -1;
        edu.hkust.clap.monitor.Monitor.loopBegin(264);
for (int i = 0; i < accepts.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(264);
{
            try {
...
edu.hkust.clap.monitor.Monitor.loopEnd(264);
    ...
    protected Reply headFileResource(Request request) throws ProtocolException, ResourceException {
        ...
        if (commentOnly) etag = getComETag(); else etag = getETag();
    ...
    protected Reply putFileResource(Request request) throws ProtocolException, ResourceException {
        ...
        int cim = checkIfMatch(request, etag);
        if ((request.getIfMatch() == null) || (cim == COND_FAILED) || (cim == COND_WEAK)) {
            return super.putFileResource(request);
        }
        if ((checkIfNoneMatch(request, etag) == COND_FAILED) || (checkIfModifiedSince(request) == COND_FAILED) || (checkIfUnmodifiedSince(request) == COND_FAILED)) {
        ...
        if (request.hasContentType()) {
            ...
            if (type == null) {
                ...
                if (debug) {
                    System.out.println("No match between: [" + rtype.toString() + "] and [" + type.toString() + "]");
                }
        ...
        try {
            ...
            if (ifresource.newMetadataContent(request.getInputStream())) status = HTTP.CREATED; else status = HTTP.NO_CONTENT;
        } catch (IOException ex) {
            throw new ClientException(request.getClient(), ex);
        }
