...
public class JpegXMPFrame extends HTTPFrame {
    ...
    public Reply createXMPReply(Request request, int status) {
        ...
        reply.setContentLocation(getURL(request).toExternalForm() + ";" + URLEncoder.encode(xmptype.toString()));
        return reply;
    }
...
edu.hkust.clap.monitor.Monitor.loopEnd(263);
    ...
    protected boolean negotiate(Request request) throws ProtocolException {
        if (!request.hasAccept()) {
            ...
            HttpAccept xmpAccept = getMatchingAccept(accepts, xmptype);
            ...
            } else if (xmpAccept != null) return true; else return false;
        }
    }
...
edu.hkust.clap.monitor.Monitor.loopEnd(264);
    ...
    protected Reply headFileResource(Request request) throws ProtocolException, ResourceException {
        ...
        if (xmpOnly) etag = getXMPETag(); else etag = getETag();
