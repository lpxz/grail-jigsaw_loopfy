...
public class VirtualHostFrame extends HTTPFrame {
    ...
    static {
        ...
        a = new StringAttribute("followup", null, Attribute.EDITABLE);
    ...
    public synchronized ResourceReference lookupFollowup() {
        if (followup == null) {
            ...
            if (followup == null) {
                getServer().errlog(getIdentifier() + "[" + getClass().getName() + "]: " + "unable to restore \"" + name + "\" " + " from root store.");
            }
        }
        return followup;
    }
    ...
    protected boolean lookupOther(LookupState ls, LookupResult lr) throws ProtocolException {
        ...
        try {
            ...
            boolean done = (resource != null) ? resource.lookup(ls, lr) : false;
            if (!done) {
                lr.setTarget(null);
            }
            return true;
