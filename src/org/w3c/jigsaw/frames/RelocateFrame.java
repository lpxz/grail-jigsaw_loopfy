...
edu.hkust.clap.monitor.Monitor.loopEnd(513);
    ...
    private Reply getRedirectReply(Request request) throws ProtocolException {
        ...
        if (location == null) {
            ...
            error.setContent("The target RelocateResource doesn't define the" + " relocation location. The server is " + " misconfigured.");
            ...
            try {
                ...
                if (host == null) loc = new URL(server.getURL(), location); else {
                    int ic = host.indexOf(':');
                    if (ic < 0) {
                        loc = new URL(new URL(server.getURL().getProtocol(), host, server.getURL().getFile()), location);
                    } else {
                        loc = new URL(new URL(server.getURL().getProtocol(), host.substring(0, ic), Integer.parseInt(host.substring(ic + 1)), server.getURL().getFile()), location);
                    }
                }
                if (checkHandlePathInfo()) {
                    ...
                    try {
                        ...
                        resource.getServer().errlog(resource, "This resource handle Pathinfo " + "but the request has an invalid " + "PATH_INFO state.");
                    }
            ...
            g.append("<P>This resources has moved, click on the link if your" + " browser doesn't support automatic redirection<BR>" + "<A HREF=\"" + loc.toExternalForm() + "\">" + loc.toExternalForm() + "</A>");
            reply.setStream(g);
            return reply;
        }
    }
