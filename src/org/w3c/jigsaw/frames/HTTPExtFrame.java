...
public class HTTPExtFrame extends HTTPFrame {
     ...
     */
    protected static final void registerExtension(String name, String methods[], String classname) {
        edu.hkust.clap.monitor.Monitor.loopBegin(451);
for (int i = 0; i < methods.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(451);
{
            registerExtension(name, methods[i], classname);
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(451);
     ...
     * @param classname the associated frame classname.
     */
    protected static final void registerExtension(String name, String method, String classname) {
        if (extensions == null) extensions = new Hashtable(2);
     ...
     * @return a boolean
     */
    protected static final boolean supportedExtension(String name, String method, Object obj) {
        if (extensions != null) {
            String idx = obj.getClass().getName() + "." + method;
            Hashtable methods = (Hashtable) extensions.get(idx);
            if (methods != null) return (methods.get(name) != null);
        }
        return false;
    }
    ...
    public Reply extended(Request request) throws ProtocolException, ResourceException {
        ...
        Reply reply = null;
        if ((method != null) && method.equals("BROWSE") && getBrowsableFlag()) return browse(request);
     ...
     * method.
     */
    public void checkMandatoryExtension(Request request, String mmethod) throws ProtocolException {
        ...
        if (extl != null) {
            ...
            int len = exts.length;
            nb_extensions += len;
            edu.hkust.clap.monitor.Monitor.loopBegin(452);
for (int i = 0; i < len; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(452);
{
                if (!supportedExtension(exts[i].getName(), method, this)) notsupported.addElement(exts[i].getName());
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(452);
        ...
        if (cextl != null) {
            ...
            nb_extensions += len;
            edu.hkust.clap.monitor.Monitor.loopBegin(453);
for (int i = 0; i < cexts.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(453);
{
                if (!supportedExtension(cexts[i].getName(), method, this)) notsupported.addElement(cexts[i].getName());
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(453);
        ...
        if ((len = notsupported.size()) > 0) {
            ...
            content.append("<h1>Mandatory extension(s) not supported:", "</h1><p>\n");
            content.append("<ul>\n");
            edu.hkust.clap.monitor.Monitor.loopBegin(454);
for (int i = 0; i < len; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(454);
content.append("<li> " + (String) notsupported.elementAt(i) + "\n");} 
edu.hkust.clap.monitor.Monitor.loopEnd(454);
