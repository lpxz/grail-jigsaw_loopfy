...
public class SeeOtherFrame extends PostableFrame {
    ...
    static {
        ...
        a = new StringAttribute("target-url", null, Attribute.EDITABLE);
    ...
    public Reply handle(Request request, URLDecoder data) throws ProtocolException {
        ...
        if (target == null) {
            ...
            g.append("The target RelocateResource doesn't define the" + " relocation location. The server is " + " misconfigured.");
            error.setStream(g);
            return error;
        }
        try {
        ...
        g.append("<P>You should see the following resource, with a GET" + ", click on the link if your" + " browser doesn't support automatic redirection<BR>" + "<A HREF=\"" + loc.toExternalForm() + "\">" + loc.toExternalForm() + "</A>");
        reply.setStream(g);
        return reply;
    }
}
