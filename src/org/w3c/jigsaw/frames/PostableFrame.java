...
public class PostableFrame extends HTTPFrame {
    ...
    static {
        ...
        a = new BooleanAttribute("override", Boolean.FALSE, Attribute.EDITABLE);
        ATTR_OVERIDE = AttributeRegistry.registerAttribute(cls, a);
        a = new BooleanAttribute("convert-get", Boolean.TRUE, Attribute.EDITABLE);
    ...
    public Reply get(Request request) throws ProtocolException, ResourceException {
        if ((!getConvertGetFlag()) || (!request.hasState("query"))) return super.get(request);
    ...
    public Reply post(Request request) throws ProtocolException, ResourceException {
        ...
        if (request.hasContentType()) {
            mustURLDecode = (type.match(request.getContentType()) == MimeType.MATCH_SPECIFIC_SUBTYPE);
        }
        URLDecoder dec = null;
        if (mustURLDecode) {
            try {
    ...
    public Reply handle(Request request, URLDecoder data) throws ProtocolException {
        ...
        if (data != null) {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(977);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(977);
{
                String name = (String) e.nextElement();
                g.append("<li><em>" + name + "</em> = <b>" + data.getValue(name) + "</b></li>");
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(977);

            g.append("</ul>");
        } else {
            g.append("<p>Not with the application/x-www-form-urlencoded" + " mime type");
        }
