...
public class CCPPFrame extends HTTPExtFrame {
    ...
    public Reply test_get(Request request) throws ProtocolException, ResourceException {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(822);
for (int i = 0; i < refs.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(822);
{
            try {
                if (refs[i].isURI()) {
                    ...
                    System.out.println("Ref[" + i + "] (dif) : (" + dn + ", " + refs[i].getDiffName() + ")");
                    System.out.println("  Diff[" + dn + "] : " + ccpp.getProfileDiff(dn));
                }
            } catch (InvalidProfileException ex) {
                System.out.println("Ref[" + i + "] (invalid) : " + refs[i].getUnparsedRef());
            }
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(822);
