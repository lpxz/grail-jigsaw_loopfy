...
public class CCPPRequest implements CCPP {
    ...
    public static String getStandardWarning(int warning) {
        ...
        switch(category) {
            case 1:
                if ((catcode >= 0) && (catcode < msg_100.length)) return msg_100[catcode];
                break;
            case 2:
                if ((catcode >= 0) && (catcode < msg_200.length)) return msg_200[catcode];
                break;
        }
    ...
    public ProfileRef[] getProfileReferences() {
        if (references == null) {
            ...
            if (profile != null) {
                String profiles[] = (String[]) HttpFactory.parseTokenList(profile).getValue();
                references = new ProfileRef[profiles.length];
                edu.hkust.clap.monitor.Monitor.loopBegin(520);
for (int i = 0; i < references.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(520);
{
                    references[i] = new ProfileRef(profiles[i]);
                }} 
edu.hkust.clap.monitor.Monitor.loopEnd(520);
    ...
    protected Reply acknowledge(Reply reply) {
        ...
        if ((man != null) && (man.getLength() == 1) && (man.getHttpExt(HTTP_EXT_ID) != null)) {
            reply.setEnd2EndExtensionAcknowledgmentHeader();
        }
        HttpExtList cman = request.getHttpCManExtDecl();
        if ((cman != null) && (cman.getLength() == 1) && (cman.getHttpExt(HTTP_EXT_ID) != null)) {
    ...
    public void addWarning(Reply reply, int warning, String reference) {
        ...
        reply.setExtensionHeader(ext, PROFILE_WARNING_HEADER, ccppwarning.toString());
    }
