...
public class PropertySet extends Resource {
    ...
    public synchronized void setValue(int idx, Object value) {
        ...
        if (idx > ATTR_LAST_MODIFIED) {
            ...
            if (value == null) {
                ...
                if (!server.getProperties().putValue(a.getName(), a.stringify(value))) throw new IllegalAttributeAccess(this, getAttributes()[idx], value);
            }
        }
    }
    ...
    protected Object convertingGet(httpd s, Attribute a, Object def) {
        ...
        if (a instanceof FileAttribute) {
            ...
            double d = ((def == null) ? Double.NaN : ((Double) def).doubleValue());
    ...
    public Object getValue(int idx, Object def) {
        if (idx <= ATTR_LAST_MODIFIED) return super.getValue(idx, def);
        return convertingGet(server, attributes[idx], def);
    }

    public Object unsafeGetValue(int idx, Object def) {
        if (idx <= ATTR_LAST_MODIFIED) return super.unsafeGetValue(idx, def);
        return convertingGet(server, attributes[idx], def);
    }
    ...
    public void initialize(Object values[]) {
        ...
        attributes[ATTR_IDENTIFIER] = new StringAttribute("identifier", null, 0);
    }
}
