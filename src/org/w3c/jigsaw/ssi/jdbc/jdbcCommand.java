...
public class jdbcCommand implements Command {
    ...
    public String getValue(Dictionary variables, String var, Request request) {
        ...
        if (!hasMoreValue(variables, var, request)) return "empty"; else return "not-empty";
    }
    ...
    public synchronized Reply execute(SSIFrame ssiframe, Request request, ArrayDictionary parameters, Dictionary variables) {
        ...
        if (select != null) {
            ...
            try {
                if (set != null) text = set.getObject(Integer.parseInt(column)).toString();
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        } else if (next != null) {
        ...
        if (text != null) reply.setContent(text);
        return reply;
    }
}
