...
public class DefaultCommandRegistry extends BasicCommandRegistry {
    ...
    private static Command[] cmds = { new ConfigCommand(), new IncludeCommand(), new EchoCommand(), new FSizeCommand(), new FLastModCommand(), new ExecCommand(), new SampleCommand(), new CounterCommand(), new CountCommand(), new CookieCommand(), new org.w3c.jigsaw.ssi.jdbc.jdbcCommand(), new LoopCommand(), new EndloopCommand(), new ExitloopCommand(), new IfCommand(), new ElseCommand(), new EndifCommand(), new org.w3c.jigsaw.ssi.servlets.ServletCommand() };

    static {
        edu.hkust.clap.monitor.Monitor.loopBegin(823);
for (int i = 0; i < 256; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(823);
needsEscape[i] = false;} 
edu.hkust.clap.monitor.Monitor.loopEnd(823);

        needsEscape[(int) '&'] = needsEscape[(int) ';'] = needsEscape[(int) '\''] = needsEscape[(int) '`'] = needsEscape[(int) '"'] = needsEscape[(int) '|'] = needsEscape[(int) '*'] = needsEscape[(int) '?'] = needsEscape[(int) '~'] = needsEscape[(int) '<'] = needsEscape[(int) '>'] = needsEscape[(int) '^'] = needsEscape[(int) '('] = needsEscape[(int) ')'] = needsEscape[(int) '['] = needsEscape[(int) ']'] = needsEscape[(int) '{'] = needsEscape[(int) '}'] = needsEscape[(int) '$'] = needsEscape[(int) '\\'] = true;
    }

    private String unescape(String str) {
        if (str == null) return null;
        StringBuffer buf = new StringBuffer(str.length() + 10);
        edu.hkust.clap.monitor.Monitor.loopBegin(824);
for (int i = 0; i < str.length(); i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(824);
{
            if (needsEscape[(int) ((byte) str.charAt(i))]) buf.append('\\');
            buf.append(str.charAt(i));
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(824);

        return buf.toString();
    }

    public DefaultCommandRegistry() {
        edu.hkust.clap.monitor.Monitor.loopBegin(825);
for (int i = 0; i < cmds.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(825);
{
            registerCommand(cmds[i]);
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(825);

    }

    public Dictionary initVariables(SSIFrame ssiframe, Request request, Dictionary variables) {
        variables = super.initVariables(ssiframe, request, variables);
        if (variables == null) variables = new Hashtable(5);
        safePut(variables, "sizefmt", "abbrev");
        safePut(variables, "datefmt", "%c");
        ArrayDictionary ssiVars = new ArrayDictionary(22);
        safePut(ssiVars, "DOCUMENT_NAME", ssiframe.getFileResource().getFilename());
        safePut(ssiVars, "DOCUMENT_URI", request.getURL().toString());
        safePut(ssiVars, "QUERY_STRING_UNESCAPED", unescape(request.getQueryString()));
        safePut(ssiVars, "SERVER_SOFTWARE", ssiframe.getFileResource().getServer().getSoftware());
        safePut(ssiVars, "SERVER_NAME", "jigsaw");
        safePut(ssiVars, "GATEWAY_INTERFACE", "org.w3c.jigsaw.ssi.SSIFrame");
        safePut(ssiVars, "SERVER_PROTOCOL", request.getVersion());
        safePut(ssiVars, "SERVER_PORT", String.valueOf(ssiframe.getFileResource().getServer().getLocalPort()));
        safePut(ssiVars, "REQUEST_METHOD", request.getMethod());
        safePut(ssiVars, "PATH_INFO", "");
        safePut(ssiVars, "PATH_TRANSLATED", "");
        safePut(ssiVars, "SCRIPT_NAME", "org.w3c.jigsaw.ssi.SSIFrame");
        String queryString = request.getQueryString();
        safePut(ssiVars, "QUERY_STRING", queryString);
        java.net.InetAddress addr = request.getClient().getInetAddress();
        safePut(ssiVars, "REMOTE_HOST", addr.getHostName());
        String s = addr.toString();
        byte[] ip = addr.getAddress();
        int idx = s.indexOf('/');
        safePut(ssiVars, "REMOTE_ADDR", (idx == -1) ? s : s.substring(idx + 1));
        safePut(ssiVars, "REMOTE_USER", request.getState(org.w3c.jigsaw.auth.AuthFilter.STATE_AUTHUSER));
        safePut(ssiVars, "AUTH_TYPE", request.getState(org.w3c.jigsaw.auth.AuthFilter.STATE_AUTHTYPE));
        safePut(ssiVars, "REMOTE_IDENT", "");
        safePut(ssiVars, "CONTENT_TYPE", request.getContentType());
        int cl = request.getContentLength();
        if (cl != -1) ssiVars.put("CONTENT_LENGTH", String.valueOf(cl));
        HeaderValue hval = request.getHeaderValue(Request.H_ACCEPT);
        if (hval != null) ssiVars.put("HTTP_ACCEPT", hval.toExternalForm());
        safePut(ssiVars, "HTTP_USER_AGENT", request.getUserAgent());
        safePut(ssiVars, "X_LAST_MODIFIED", new Date(ssiframe.getFileResource().getFile().lastModified()));
        safePut(ssiVars, "X_FILE_SIZE", new Long(ssiframe.getFileResource().getFile().length()));
        variables.put("ssiVars", ssiVars);
        if (variables.get("topSsiVars") == null) variables.put("topSsiVars", ssiVars);
        try {
    ...
    private static final void safePut(Dictionary dict, Object key, Object value) {
