...
public class FSizeCommand extends BasicCommand {
    ...
    public Reply execute(SSIFrame ssiframe, Request request, ArrayDictionary parameters, Dictionary variables) {
        ...
        Dictionary ssiVars = (Dictionary) variables.get(parameters.get("here") == null ? "topSsiVars" : "ssiVars");
        String sizefmt = (String) variables.get("sizefmt");
        Long Fsize = (Long) ssiVars.get("X_FILE_SIZE");
        long fsize = Fsize.longValue();
        if (sizefmt == null || sizefmt.equalsIgnoreCase("bytes")) reply.setContent(withCommas(fsize)); else if (sizefmt.equalsIgnoreCase("abbrev")) {
            String unit = null;
            long cut = 1;
            if (fsize >= MBsize) {
                unit = " MB";
    ...
    private String withCommas(long n) {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(325);
for (int i = 0; i < length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(325);
{
            buf.append(nstr.charAt(i));
            if ((length - i) % 3 == 1 && i + 1 != length) buf.append(',');
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(325);
