...
public class IfCommand implements ControlCommand {
    ...
    public void setPosition(SSIFrame ssiframe, Request request, CommandRegistry registry, ArrayDictionary parameters, Dictionary variables, int position) {
        ...
        if (name != null) ifstore.put(ssiframe.getResource().getURLPath() + ":" + name, new Integer(position));
    }
    ...
    protected boolean check(CommandRegistry registry, ArrayDictionary parameters, Dictionary variables, Request request) {
        Object values[] = parameters.getMany(keys);
        String name = (String) values[0];
        String command = (String) values[1];
        String var = (String) values[2];
        String equals = (String) values[3];
        if ((command == null) || (var == null) || (equals == null)) return false;
    ...
    public int jumpTo(SSIFrame ssiframe, Request request, CommandRegistry registry, ArrayDictionary parameters, Dictionary variables) throws ControlCommandException {
        ...
        String name = (String) values[0];
        if (name != null) {
            if (check(registry, parameters, variables, request)) return getPosition(ssiframe.getResource().getURLPath() + ":" + name) + 1;
            try {
                return (ElseCommand.getPosition(ssiframe.getResource().getURLPath() + ":" + name) + 1);
            } catch (ControlCommandException ex) {
                return (EndifCommand.getPosition(ssiframe.getResource().getURLPath() + ":" + name) + 1);
            }
        }
