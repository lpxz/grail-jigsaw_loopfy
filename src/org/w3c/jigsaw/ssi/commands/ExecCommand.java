...
public class ExecCommand extends BasicCommand {
    ...
    public Reply execute(SSIFrame ssiframe, Request request, ArrayDictionary parameters, Dictionary variables) {
        ...
        if (cmd != null) return executeCmd(ssiframe, request, parameters, variables, cmd);
    ...
    private Reply executeCmd(SSIFrame ssiframe, Request request, ArrayDictionary parameters, Dictionary variables, String cmd) {
        ...
        ArrayDictionary ssiVars = (ArrayDictionary) variables.get("ssiVars");
        if (ssiVars == null) {
            Reply reply = ssiframe.createCommandReply(request, HTTP.OK);
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(178);
for (int i = 0, j = 0; i < ssiVars.capacity() && ssiVars.keyAt(i) != null; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(178);
env[j++] = (ssiVars.keyAt(i).toString()) + "=" + (ssiVars.elementAt(i).toString());} 
edu.hkust.clap.monitor.Monitor.loopEnd(178);
    ...
    private final void addEnv(Vector env, String var, String val) {
        if (var != null && val != null) env.addElement(var + '=' + val);
    }
...
class DelayedProcessStream extends DelayedInputStream {
    ...
    protected final void init() {
        ...
        if (proc == null) in = new StringBufferInputStream("[exec: cannot start process: \"" + cmd + "\"]"); else in = proc.getInputStream();
    }
}
