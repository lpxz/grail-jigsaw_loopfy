...
public class MirrorFrame extends ForwardFrame {
    ...
    static {
        ...
        a = new StringAttribute("mirrors", null, Attribute.EDITABLE);
        ATTR_MIRRORS = AttributeRegistry.registerAttribute(c, a);
        a = new BooleanAttribute("partial", Boolean.FALSE, Attribute.EDITABLE);
                ...
                if ((mirrors != null) && (location != null)) {
                    try {
                        ...
                        if (isPartialMirroring()) {
                            fake = new URL(request.getURL().getProtocol(), loc.getHost(), loc.getPort(), getURLPath() + uloc.getFile());
                        } else if (location.startsWith(mirrors.toString())) {
                            fake = new URL(request.getURL().getProtocol(), loc.getHost(), loc.getPort(), uloc.getFile());
                        } else {
                            fake = uloc;
                        }
                        if (fake != null) {
    ...
    public boolean lookupOther(LookupState ls, LookupResult lr) throws org.w3c.tools.resources.ProtocolException {
        ...
        if (ls.isInternal()) return super.lookupOther(ls, lr);
        if (mirrors != null) {
        ...
        if (request.getMethod().equals("GET")) error.setContent("Target resource not found.");
        lr.setTarget(null);
        lr.setReply(error);
        return true;
    }
