...
public class ProxyDispatcherProp extends PropertySet {
    ...
    static {
        ...
        Attribute a = null;
        try {
            c = Class.forName("org.w3c.jigsaw.proxy.ProxyDispatcherProp");
        } catch (Exception ex) {
            ex.printStackTrace();
            System.exit(1);
        }
        a = new StringAttribute(ProxyDispatcher.RULE_P, null, Attribute.EDITABLE);
        ATTR_RULES = AttributeRegistry.registerAttribute(c, a);
        a = new BooleanAttribute(ProxyDispatcher.DEBUG_P, Boolean.FALSE, Attribute.EDITABLE);
        ATTR_DEBUG = AttributeRegistry.registerAttribute(c, a);
        a = new BooleanAttribute(ProxyDispatcher.CHECK_RULES_LAST_MODIFIED_P, Boolean.FALSE, Attribute.EDITABLE);
    ...
    public ProxyDispatcherProp(String name, httpd server) {
        super(name, server);
        if (getRulesLocation() == null) setValue(ATTR_RULES, getDefaultRulesLocation());
    }
}
