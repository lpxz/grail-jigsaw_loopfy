...
public class AclFilter extends ResourceFilter {
    ...
    static {
        ...
        a = new IntegerAttribute("security-level", new Integer(0), Attribute.EDITABLE);
        ATTR_SECURITY_LEVEL = AttributeRegistry.registerAttribute(c, a);
        a = new BooleanAttribute("strict-acl-merge-policy", Boolean.TRUE, Attribute.EDITABLE);
        ATTR_STRICT_ACL_MERGE_POLICY = AttributeRegistry.registerAttribute(c, a);
        ...
        a = new BooleanAttribute("lenient", Boolean.FALSE, Attribute.EDITABLE);
    ...
    protected JAcl[] getAcls() {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(524);
for (int i = 0; i < frames.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(524);
acls[i] = (JAcl) frames[i];} 
edu.hkust.clap.monitor.Monitor.loopEnd(524);
    ...
    protected void authenticate(Request request, JAcl acls[]) throws ProtocolException {
        ...
        if (princ != null) {
            if (strict) {
                authorized = true;
                edu.hkust.clap.monitor.Monitor.loopBegin(525);
for (int i = 0; i < acls.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(525);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(525);

            } else {
                edu.hkust.clap.monitor.Monitor.loopBegin(526);
for (int i = 0; i < acls.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(526);
{
                    boolean check = acls[i].checkPermission(princ, perm);
                    if (check) {
                        authorized = true;
...
edu.hkust.clap.monitor.Monitor.loopEnd(526);

            }
        }
        if (!authorized) {
            HttpChallenge challenge = security.getChallenge(acl.getName(), princ);
            Reply e = null;
            if (request.isProxy()) {
    ...
    public void setValue(int idx, Object value) {
        ...
        if ((idx == ATTR_SECURITY_LEVEL) || (idx == ATTR_ALGORITHM) || (idx == ATTR_LENIENT)) {
