...
public class URLDecoder {
    ...
    public String[] getMultipleValues(String name) {
        if (overide) throw new RuntimeException(this.getClass().getName() + "[getMultipleValues]: " + " overide not set !");
    ...
    public String getValue(String name) {
        Object value = values.get(name);
        if ((value != null) && !(value instanceof String)) throw new RuntimeException(this.getClass().getName() + "[getValue]:" + " use getMultipleValues in:\n\t" + name + " " + value);
        return (String) value;
    }

    private String stringify(byte[] bytes, int offset, int length) throws URLDecoderException {
        String __s = null;
        if (enc != null) {
            try {
    ...
    public Hashtable parse() throws IOException, URLDecoderException {
        String key = null;
        read_loop: ch = in.read();
        edu.hkust.clap.monitor.Monitor.loopBegin(1006);
while (true) { 
edu.hkust.clap.monitor.Monitor.loopInc(1006);
{
            switch(ch) {
                case '+':
                    append(' ');
                ...
                case '%':
                    int hi, lo;
                    if ((hi = ch = in.read()) == -1) throw new URLDecoderException("Invalid escape seq.");
                    if ((lo = ch = in.read()) == -1) throw new URLDecoderException("Invalid escape seq.");
                    hi = (Character.isDigit((char) hi) ? (hi - '0') : 10 + (Character.toUpperCase((char) hi) - 'A'));
                    lo = (Character.isDigit((char) lo) ? (lo - '0') : 10 + (Character.toUpperCase((char) lo) - 'A'));
                ...
                case -1:
                    ...
                    edu.hkust.clap.monitor.Monitor.loopEnd(1006);
                    return values;
                default:
                    append(ch);
                    break;
