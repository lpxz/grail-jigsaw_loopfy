...
public class ConfigResourceIndexer extends SampleResourceIndexer {
    ...
    private File getConfigFile(File directory, String name) {
        ...
        if (configdir.exists() && configdir.isDirectory()) {
            ...
            if (config.exists() && !config.isDirectory()) return config;
        }
        return null;
    }
    ...
    public Resource createResource(ContainerResource container, RequestInterface request, File directory, String name, Hashtable defs) {
        Resource r = super.createResource(container, request, directory, name, defs);
        Class proto = null;
        try {
        ...
        if (config != null) {
            try {
                ...
                MimeParser p = new MimeParser(buf, new MimeParserMessageFactory());
                HttpEntityMessage msg = (HttpEntityMessage) p.parse();
                Enumeration e = msg.enumerateHeaderDescriptions();
                edu.hkust.clap.monitor.Monitor.loopBegin(1017);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(1017);
{
                    ...
                    if (d.isHeader(HttpEntityMessage.H_CONTENT_TYPE)) {
                        ...
                        if (mtype.hasParameter("charset")) {
                            ...
                            MimeType m = new MimeType(mtype.getType(), mtype.getSubtype());
...
edu.hkust.clap.monitor.Monitor.loopEnd(1017);
