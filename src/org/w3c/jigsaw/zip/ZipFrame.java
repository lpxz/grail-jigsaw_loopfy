...
public class ZipFrame extends HTTPFrame {
    ...
    public void registerResource(FramedResource resource) {
        ...
        if (resource instanceof ZipFileResource) zipfresource = (ZipFileResource) resource;
    }
    ...
    protected Reply createFileReply(Request request) throws ProtocolException, ResourceException {
        ...
        if (zipfresource == null) {
            throw new ResourceException("this frame is not attached to a " + "ZipFileResource. (" + resource.getIdentifier() + ")");
        }
        reply = createDefaultReply(request, HTTP.OK);
        InputStream in = zipfresource.getInputStream();
        if (in != null) reply.setStream(in);
        return reply;
    }
    ...
    public synchronized boolean computeContainerListing(boolean refresh) {
        ...
        if ((refresh) || (listing == null) || (cresource.getLastModified() > listing_stamp) || (getLastModified() > listing_stamp)) {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(783);
for (int i = 0; i < resources.size(); i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(783);
{
                ...
                if (rr != null) {
                    try {
                        ...
                        if (http_class != null) {
                            itsframe = (HTTPFrame) resource.getFrame(http_class);
                        }
                        if (itsframe != null) {
                            String icon = itsframe.getIcon();
                            if (icon != null) g.append("<img src=\"" + getIconDirectory() + "/" + icon + "\" alt=\"" + icon + "\">");
                            if (resource instanceof ContainerInterface) g.append("<a href=\"", URLEncoder.encode(name), "/\">" + name + "</a>"); else g.append("<a href=\"", URLEncoder.encode(name), "\">" + name + "</a>");
                            String title = itsframe.getTitle();
                            if (title != null) g.append(" " + title);
                            if (size != -1) {
                                String s = null;
                                if (size > 1023) {
                                    s = " [" + (size / 1024) + " Kb]";
...
edu.hkust.clap.monitor.Monitor.loopEnd(783);
