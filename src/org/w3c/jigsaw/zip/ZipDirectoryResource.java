...
public class ZipDirectoryResource extends DirectoryResource {
    ...
    static {
        ...
        a = new FileAttribute("zipfile", null, Attribute.COMPUTED);
        ATTR_ZIPFILE = AttributeRegistry.registerAttribute(cls, a);
        a = new StringAttribute("entrypath", null, Attribute.COMPUTED);
    ...
    public synchronized Enumeration enumerateResourceIdentifiers(boolean all) {
        ...
        if (entry_path == null) {
            edu.hkust.clap.monitor.Monitor.loopBegin(105);
while (entries.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(105);
{
                ...
                if (lookuped.get(entry_name) != null) continue;
                lookuped.put(entry_name, Boolean.TRUE);
                if (lookup(entry_name) == null) {
                    createDefaultResource(entry_name);
                }
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(105);

        } else {
            int startidx = entry_path.length();
            edu.hkust.clap.monitor.Monitor.loopBegin(106);
while (entries.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(106);
{
                entry = (ZipEntry) entries.nextElement();
                entry_name = entry.getName();
                if (!entry_name.startsWith(entry_path)) continue;
                if (entry_name.length() > startidx + 1) entry_name = entry_name.substring(startidx + 1); else continue;
                idx = entry_name.indexOf('/');
                if (idx != -1) {
                    entry_name = entry_name.substring(0, idx);
                    if (entry_name.length() == 0) continue;
                    directories.put(entry_name, Boolean.TRUE);
                }
                if (lookuped.get(entry_name) != null) continue;
                lookuped.put(entry_name, Boolean.TRUE);
...
edu.hkust.clap.monitor.Monitor.loopEnd(106);
    ...
    protected boolean entryExists(String name) {
        ...
        if (entry_path != null) {
            if (entry_path.endsWith("/")) full_path = entry_path + name; else full_path = entry_path + "/" + name;
        } else {
            full_path = name;
        }
        try {
            return ((zip.getEntry(full_path) != null) || (zip.getEntry(full_path + "/") != null));
        } finally {
            try {
                zip.close();
            } catch (Exception ex) {
    ...
    protected Resource index(String name, Hashtable defs, RequestInterface req) {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(107);
while (context != null) { 
edu.hkust.clap.monitor.Monitor.loopInc(107);
{
            do {
            ...
            if ((rr_lastidx = rr_indexer) != null) {
                try {
                    ...
                    if (!(indexer instanceof ZipIndexer)) return null;
                    ZipIndexer zindexer = (ZipIndexer) indexer;
                    if (directories.get(name) != null) {
                        resource = zindexer.createDirectoryResource(getDirectory(), name, defs);
                    } else {
                        resource = zindexer.createFileResource(getDirectory(), name, defs);
                    }
...
edu.hkust.clap.monitor.Monitor.loopEnd(107);
    ...
    public void initialize(Object values[]) {
        ...
        disableEvent();
        if (getZipFile() != null) setValue(ATTR_DIRECTORY, getZipFile());
        enableEvent();
    }
