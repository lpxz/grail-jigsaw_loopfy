...
public class HttpRange extends BasicValue {
    ...
    protected void parse() throws HttpParserException {
        ...
        ps.separator = (byte) '=';
        if (HttpParser.nextItem(raw, ps) < 0) error("No byte range unit.");
        ...
        if (raw[off] == '-') {
            ...
            ps.separator = (byte) '-';
            if (HttpParser.nextItem(raw, ps) < 0) error("Invalid range spec: no first pos.");
            it.prepare(ps);
    ...
    protected void updateByteValue() {
        ...
        buf.append(unit);
        buf.append((byte) '=');
        if (firstpos >= 0) buf.appendInt(firstpos); else if (lastpos < 0) buf.append((byte) '0');
        buf.append('-');
        if (lastpos >= 0) buf.appendInt(lastpos);
        raw = buf.getByteCopy();
        roff = 0;
        rlen = raw.length;
    }
    ...
    public void setFirstPosition(int firstpos) {
        if (this.firstpos != firstpos) invalidateByteValue();
    ...
    public void setLastPosition(int lastpos) {
        if (this.lastpos != lastpos) invalidateByteValue();
