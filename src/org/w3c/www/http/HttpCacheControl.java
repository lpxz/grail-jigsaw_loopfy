...
public class HttpCacheControl extends BasicValue {
    ...
    protected void updateByteValue() {
        ...
        if (extensions != null) {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(893);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(893);
{
                if (cnt) {
                    buf.append(',');
                    cnt = true;
...
edu.hkust.clap.monitor.Monitor.loopEnd(893);
    ...
    private static byte bnocache[] = { (byte) 'n', (byte) 'o', (byte) '-', (byte) 'c', (byte) 'a', (byte) 'c', (byte) 'h', (byte) 'e' };

    private static byte bnostore[] = { (byte) 'n', (byte) 'o', (byte) '-', (byte) 's', (byte) 't', (byte) 'o', (byte) 'r', (byte) 'e' };

    private static final byte bmaxage[] = { (byte) 'm', (byte) 'a', (byte) 'x', (byte) '-', (byte) 'a', (byte) 'g', (byte) 'e' };

    private static final byte bs_maxage[] = { (byte) 's', (byte) '-', (byte) 'm', (byte) 'a', (byte) 'x', (byte) 'a', (byte) 'g', (byte) 'e' };

    private static final byte bmaxstale[] = { (byte) 'm', (byte) 'a', (byte) 'x', (byte) '-', (byte) 's', (byte) 't', (byte) 'a', (byte) 'l', (byte) 'e' };

    private static final byte bminfresh[] = { (byte) 'm', (byte) 'i', (byte) 'n', (byte) '-', (byte) 'f', (byte) 'r', (byte) 'e', (byte) 's', (byte) 'h' };

    private static final byte bonlyifcached[] = { (byte) 'o', (byte) 'n', (byte) 'l', (byte) 'y', (byte) '-', (byte) 'i', (byte) 'f', (byte) '-', (byte) 'c', (byte) 'a', (byte) 'c', (byte) 'h', (byte) 'e', (byte) 'd' };

    private static final byte bpublic[] = { (byte) 'p', (byte) 'u', (byte) 'b', (byte) 'l', (byte) 'i', (byte) 'c' };

    private static final byte bprivate[] = { (byte) 'p', (byte) 'r', (byte) 'i', (byte) 'v', (byte) 'a', (byte) 't', (byte) 'e' };

    private static final byte bnotransform[] = { (byte) 'n', (byte) 'o', (byte) '-', (byte) 't', (byte) 'r', (byte) 'a', (byte) 'n', (byte) 's', (byte) 'f', (byte) 'o', (byte) 'r', (byte) 'm' };

    private static final byte bmustrevalidate[] = { (byte) 'm', (byte) 'u', (byte) 's', (byte) 't', (byte) '-', (byte) 'r', (byte) 'e', (byte) 'v', (byte) 'a', (byte) 'l', (byte) 'i', (byte) 'd', (byte) 'a', (byte) 't', (byte) 'e' };

    private static final byte bproxyrevalidate[] = { (byte) 'p', (byte) 'r', (byte) 'o', (byte) 'x', (byte) 'y', (byte) '-', (byte) 'r', (byte) 'e', (byte) 'v', (byte) 'a', (byte) 'l', (byte) 'i', (byte) 'd', (byte) 'a', (byte) 't', (byte) 'e' };

    private final void parseDirective(int ds, int de, ParseState pval) throws HttpParserException {
        if (HttpParser.compare(raw, ds, de, bmaxage) == 0) {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(894);
while (HttpParser.nextItem(raw, sp) >= 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(894);
{
                fields.addElement(new String(raw, 0, sp.start, sp.end - sp.start));
                sp.prepare();
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(894);
            ...
            if (nocache == null) {
                ...
                edu.hkust.clap.monitor.Monitor.loopBegin(895);
for (int i = 0; i < num; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(895);
{
                    t_nocache[nocache.length + i] = (String) fields.elementAt(i);
                }} 
edu.hkust.clap.monitor.Monitor.loopEnd(895);
...
edu.hkust.clap.monitor.Monitor.loopEnd(894);
            ...
            if (priv == null) {
                ...
                edu.hkust.clap.monitor.Monitor.loopBegin(896);
for (int i = 0; i < num; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(896);
{
                    t_priv[priv.length + i] = (String) fields.elementAt(i);
                }} 
edu.hkust.clap.monitor.Monitor.loopEnd(896);
    ...
    private final void parseDirective(int ds, int de) throws HttpParserException {
    ...
    protected void parse() throws HttpParserException {
        ...
        ld.separator = (byte) '=';
        ld.spaceIsSep = false;
        edu.hkust.clap.monitor.Monitor.loopBegin(897);
while (HttpParser.nextItem(raw, ls) >= 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(897);
{
            ld.bufend = ls.end;
            ld.ioff = ls.start;
            if (HttpParser.nextItem(raw, ld) >= 0) {
                ...
                int dend = ld.end;
                ld.prepare();
                if (HttpParser.nextItem(raw, ld) >= 0) parseDirective(dstart, dend, ld); else parseDirective(dstart, dend);
            } else {
                parseDirective(ls.start, ls.end);
            }
            ls.prepare();
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(897);
    ...
    public void setNoCache(String fields[]) {
        validate();
        if (fields == null) {
            if (checkDirective(NOCACHE)) invalidateByteValue();
    ...
    public void addNoCache(String name) {
        validate();
        if (checkDirective(NOCACHE) && (nocache.length == 0)) return;
        if (nocache != null) {
            edu.hkust.clap.monitor.Monitor.loopBegin(898);
for (int i = 0; i < nocache.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(898);
if (nocache[i].equalsIgnoreCase(name)) return;} 
edu.hkust.clap.monitor.Monitor.loopEnd(898);
    ...
    public void setMaxAge(int age) {
        validate();
        if (age == -1) {
            if (checkDirective(MAXAGE)) invalidateByteValue();
            maxage = -1;
            unsetDirective(MAXAGE);
        } else {
            if ((age != maxage) || !checkDirective(MAXAGE)) invalidateByteValue();
            setDirective(MAXAGE);
            maxage = age;
        }
    }
    ...
    public void setSMaxAge(int age) {
        validate();
        if (age == -1) {
            if (checkDirective(S_MAXAGE)) invalidateByteValue();
            s_maxage = -1;
            unsetDirective(S_MAXAGE);
        } else {
            if ((age != s_maxage) || !checkDirective(S_MAXAGE)) invalidateByteValue();
            setDirective(S_MAXAGE);
            s_maxage = age;
        }
    }
    ...
    public void setMaxStale(int stale) {
        validate();
        if (stale == -1) {
            if (checkDirective(MAXSTALE)) invalidateByteValue();
            maxstale = -1;
            unsetDirective(MAXSTALE);
        } else {
            if ((stale != maxstale) || !checkDirective(MAXSTALE)) invalidateByteValue();
            setDirective(MAXSTALE);
            maxstale = stale;
        }
    }
    ...
    public void setMinFresh(int fresh) {
        validate();
        if (fresh == -1) {
            if (checkDirective(MINFRESH)) invalidateByteValue();
            minfresh = -1;
            unsetDirective(MINFRESH);
        } else {
            if ((fresh != minfresh) || !checkDirective(MINFRESH)) invalidateByteValue();
            setDirective(MINFRESH);
            minfresh = fresh;
        }
    }
    ...
    public void unsetPrivate() {
        validate();
        if (checkDirective(PRIV)) invalidateByteValue();
        unsetDirective(PRIV);
        priv = null;
    }
