...
public class HttpWarning extends BasicValue {
    ...
    protected void parse() throws HttpParserException {
        ...
        if (HttpParser.nextItem(raw, ps) < 0) error("Invalid warning, no status code.");
        it.ioff = ps.start;
        it.bufend = ps.end;
        this.status = HttpParser.parseInt(raw, it);
        ps.prepare();
        if (HttpParser.nextItem(raw, ps) < 0) error("Invalid warning, no agent field.");
        this.agent = new String(raw, 0, ps.start, ps.end - ps.start);
        ps.prepare();
        if (HttpParser.nextItem(raw, ps) < 0) error("Invalid warning, no text message.");
        it.ioff = ps.start;
        it.bufend = ps.end;
    ...
    public void setStatus(int status) {
        if (this.status != status) invalidateByteValue();
        this.status = status;
    }
    ...
    public void setAgent(String agent) {
        if ((agent != null) && !agent.equals(this.agent)) invalidateByteValue();
        this.agent = agent;
    }
    ...
    public void setText(String text) {
        if ((text != null) && !text.equals(this.text)) invalidateByteValue();
        this.text = text;
    }
