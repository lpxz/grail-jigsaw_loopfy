...
public class ContentLengthInputStream extends InputStream {
    ...
    public int read() throws IOException {
        if (length > 0) {
            length--;
            if (length <= 0) {
                if (observer != null) observer.notifyEOF(this);
                observer = null;
            }
            int r = in.read();
            if (r == -1) {
    ...
    public int read(byte b[], int off, int len) throws IOException {
        ...
        if (len == -1) {
            length = -1;
            if (observer != null) observer.notifyFailure(this);
            observer = null;
    ...
    public boolean markSupported() {
        if (in == null) return false; else return in.markSupported();
    }
    ...
    public ContentLengthInputStream(HttpStreamObserver observer, InputStream in, int length) {
