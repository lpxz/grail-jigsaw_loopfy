...
public class HttpEntityTag extends BasicValue {
    ...
    protected void parse() throws HttpParserException {
        ...
        ps.bufend = rlen;
        ps.separator = (byte) '/';
        if (HttpParser.nextItem(raw, ps) < 0) error("Invalid entity tag.");
        if ((raw[ps.start] == 'W') || (raw[ps.start] == 'w')) {
            weak = true;
            ps.prepare();
            if (HttpParser.nextItem(raw, ps) < 0) error("Invalid weak entity tag.");
            ps.ioff = ps.start;
            ps.bufend = ps.end;
    ...
    public void setWeak(boolean onoff) {
        if (onoff != weak) invalidateByteValue();
        weak = onoff;
    }
    ...
    public void setTag(String tag) {
        if (!tag.equals(this.tag)) invalidateByteValue();
        this.tag = tag;
    }
