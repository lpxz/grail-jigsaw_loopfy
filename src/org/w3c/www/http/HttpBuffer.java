...
class HttpBuffer {
    ...
    final void appendLong(long i, int padlen, byte pad) {
        ...
        int hackpos = len;
        if (!neg) i = -i;
        edu.hkust.clap.monitor.Monitor.loopBegin(1115);
while (i <= -10) { 
edu.hkust.clap.monitor.Monitor.loopInc(1115);
{
            append((byte) ('0' - (i % 10)));
            padlen--;
            i = i / 10;
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1115);
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(1116);
while (--padlen >= 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(1116);
append(pad);} 
edu.hkust.clap.monitor.Monitor.loopEnd(1116);

        int cnt = (len - hackpos) / 2;
        int j = len - 1;
        edu.hkust.clap.monitor.Monitor.loopBegin(1117);
while (--cnt >= 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(1117);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(1117);
    ...
    void appendQuoted(String name, byte sep, String values[]) {
        ...
        if (values.length > 0) {
            append(sep);
            append((byte) '"');
            edu.hkust.clap.monitor.Monitor.loopBegin(1118);
for (int i = 0; i < values.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1118);
{
                if (i > 0) append(',');
                append(values[i]);
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1118);
...
edu.hkust.clap.monitor.Monitor.loopEnd(1118);
     ...
     */
    public final void emit(OutputStream out) throws IOException {
        if (out == null) {
