...
public class HttpTokenList extends BasicValue {
    ...
    protected void parse() {
        ...
        ps.bufend = rlen;
        ps.spaceIsSep = false;
        edu.hkust.clap.monitor.Monitor.loopBegin(915);
while (HttpParser.nextItem(raw, ps) >= 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(915);
{
            it.ioff = ps.start;
            it.bufend = ps.end;
...
edu.hkust.clap.monitor.Monitor.loopEnd(915);
    ...
    protected void updateByteValue() {
        ...
        if (tokens != null) {
            edu.hkust.clap.monitor.Monitor.loopBegin(916);
for (int i = 0; i < tokens.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(916);
{
                if (i > 0) buf.append(',');
                buf.append(tokens[i]);
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(916);
    ...
    public void addToken(String token, boolean always) {
        ...
        if (!always) {
            if (tokens != null) {
                edu.hkust.clap.monitor.Monitor.loopBegin(917);
for (int i = 0; i < tokens.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(917);
if (tokens[i].equals(token)) return;} 
edu.hkust.clap.monitor.Monitor.loopEnd(917);
    ...
    public boolean hasToken(String token, boolean caseSensitive) {
        ...
        if (caseSensitive) {
            edu.hkust.clap.monitor.Monitor.loopBegin(918);
for (int i = 0; i < tokens.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(918);
if (tokens[i].equals(token)) return true;} 
edu.hkust.clap.monitor.Monitor.loopEnd(918);

        } else {
            edu.hkust.clap.monitor.Monitor.loopBegin(919);
for (int i = 0; i < tokens.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(919);
if (tokens[i].equalsIgnoreCase(token)) return true;} 
edu.hkust.clap.monitor.Monitor.loopEnd(919);
