...
public class ChunkedOutputStream extends DataOutputStream {
     ...
     * @exception IOException If writing fails.
     */
    protected void sendChunk(byte b[], int off, int len) throws IOException {
        if (len == 0) return;
        int size = len;
        int blen = 3;
        bheader[30] = ((byte) 13);
        bheader[31] = ((byte) 10);
        edu.hkust.clap.monitor.Monitor.loopBegin(795);
while (size > 15) { 
edu.hkust.clap.monitor.Monitor.loopInc(795);
{
            bheader[32 - blen] = hexaTable[size % 16];
            size >>= 4;
            blen++;
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(795);
    ...
    protected final void append(int v) throws IOException {
        if (bufptr + 1 >= chunksize) sendChunk();
     ...
     * @exception IOException If writing fails.
     */
    protected final void append(byte b[], int off, int len) throws IOException {
        if (bufptr + len >= chunksize) sendChunk();
     ...
     * @exception IOException If writing fails.
     */
    public void close(boolean really) throws IOException {
        if (really) {
            sendChunk();
            sendClose();
            super.close();
