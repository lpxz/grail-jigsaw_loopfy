...
public class HttpMessage implements MimeHeaderHolder, Cloneable, HTTP {
    ...
    public static final int EMIT_HEADERS = (1 << 0);

    public static final int EMIT_BODY = (1 << 2);

    public static final int EMIT_FOOTERS = (1 << 3);

    public static final int EMIT_ALL = ((1 << 0) | (1 << 1) | (1 << 2));
    ...
    protected HeaderValue values[] = null;

    protected static HeaderDescription descriptors[] = new HeaderDescription[MAX_HEADERS];

    protected static final void registerHeader(String name, String cls) {
        HeaderDescription d = new HeaderDescription(name, cls);
        factory.put(d.getName(), d);
    }

    protected static final void registerHeader(String name, String c, int i) {
    ...
    public void setHeaderValue(String name, HeaderValue value) {
        ...
        if (d != null && d.offset >= 0) {
            ...
            if (headers == null) headers = new ArrayDictionary(5, 5);
    ...
    public HeaderValue getHeaderValue(HeaderDescription d) {
        if (d.offset >= 0) {
            ...
            return ((headers != null) ? (HeaderValue) headers.get(d.getName()) : null);
        }
    }
    ...
    public final void removeHeader(int idx) {
        if ((idx >= 0) && (idx < MAX_HEADERS)) values[idx] = null;
    }
    ...
    public void setState(String name, Object value) {
        if (value != null) {
            if (state == null) state = new ArrayDictionary(4, 4);
    ...
    public HttpMessage getClone() {
        try {
            ...
            if (cl.headers != null) cl.headers = ((ArrayDictionary) ((ArrayDictionary) headers).clone());
            if (cl.state != null) cl.state = (ArrayDictionary) state.clone();
            return cl;
        } catch (Exception ex) {
    ...
    public HttpMessage getDeeperClone() {
        try {
            HttpMessage cl = (HttpMessage) clone();
            cl.values = new HeaderValue[MAX_HEADERS];
            edu.hkust.clap.monitor.Monitor.loopBegin(110);
for (int i = 0; i < MAX_HEADERS; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(110);
{
                if (values[i] != null) {
                    cl.values[i] = (HeaderValue) ((BasicValue) values[i]).clone();
                }
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(110);
    ...
    public void setValue(String name, String strval) {
        ...
        if (value == null) {
            if (d == null) d = (HeaderDescription) factory.get(lname);
            if (d == null) {
     ...
     * @exception IOException if an IO error occurs.
     */
    protected void startEmit(OutputStream out, int what) throws IOException {
        return;
    }
     ...
     * @exception IOException if an IO error occurs.
     */
    protected void endEmit(OutputStream out, int what) throws IOException {
        return;
    }
     ...
     * @exception IOException if an IO error occurs.
     */
    protected void emitHeaders(OutputStream out, int what) throws IOException {
        ...
        if (extl != null) {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(111);
for (int i = 0; i < exts.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(111);
{
                HttpExt ext = exts[i];
                Dictionary headers = getExtensionHeaders(ext);
                Enumeration henum = headers.keys();
                edu.hkust.clap.monitor.Monitor.loopBegin(112);
while (henum.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(112);
{
                    String header = ext.getRealHeader((String) henum.nextElement());
                    connection.addToken(header, false);
                }} 
edu.hkust.clap.monitor.Monitor.loopEnd(112);

            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(111);
...
edu.hkust.clap.monitor.Monitor.loopEnd(111);
        ...
        if (hasHeader("C-Ext")) connection.addToken("C-Ext", false);
        String tokens[] = (String[]) connection.getValue();
        if ((tokens != null) && (tokens.length > 0)) setHeaderValue(H_CONNECTION, connection);
        if (hasHeader("Ext")) addNoCache("Ext");
        edu.hkust.clap.monitor.Monitor.loopBegin(113);
for (int i = 0; i < MAX_HEADERS; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(113);
{
            ...
            if (v instanceof HttpSetCookieList) {
                ...
                if (hscl.isOriginal()) {
                    ...
                    if (nbc == 0) {
                        ...
                        edu.hkust.clap.monitor.Monitor.loopBegin(114);
for (int j = 0; j < nbc; j++) { 
edu.hkust.clap.monitor.Monitor.loopInc(114);
{
                            out.write(d.getTitle());
                            out.write(':');
                            out.write(' ');
...
edu.hkust.clap.monitor.Monitor.loopEnd(114);

                    }
                } else {
                    int rcs = hscl.copyLength();
                    edu.hkust.clap.monitor.Monitor.loopBegin(115);
for (int j = 0; j < rcs; j++) { 
edu.hkust.clap.monitor.Monitor.loopInc(115);
{
                        out.write(d.getTitle());
                        out.write(':');
                        out.write(' ');
...
edu.hkust.clap.monitor.Monitor.loopEnd(115);
...
edu.hkust.clap.monitor.Monitor.loopEnd(113);

        if (headers != null) {
            Enumeration e = headers.keys();
            edu.hkust.clap.monitor.Monitor.loopBegin(116);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(116);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(116);
    ...
    public void emit(OutputStream out) throws IOException {
        ...
        if (major > 0) emitHeaders(out, EMIT_ALL);
     ...
     * given stream, due to IO errors. 
     */
    public void emit(OutputStream out, int what) throws IOException {
        startEmit(out, what);
    ...
    public void setNoCache() {
        HttpCacheControl cc = getCacheControl();
        if (cc == null) setCacheControl(cc = new HttpCacheControl(true));
        cc.setNoCache();
    }
    ...
    public void setConnection(String tokens[]) {
        setHeaderValue(H_CONNECTION, ((tokens == null) ? null : new HttpTokenList(tokens)));
    }
    ...
    public void setProxyConnection(String tokens[]) {
        setHeaderValue(H_PROXY_CONNECTION, ((tokens == null) ? null : new HttpTokenList(tokens)));
    }
    ...
    public void setDate(long date) {
        setHeaderValue(H_DATE, ((date == -1) ? null : new HttpDate(true, date)));
    }
    ...
    public void setPragma(String tokens[]) {
        setHeaderValue(H_PRAGMA, ((tokens == null) ? null : new HttpTokenList(tokens)));
    }
    ...
    public void setTransferEncoding(String tokens[]) {
        setHeaderValue(H_TRANSFER_ENCODING, ((tokens == null) ? null : new HttpTokenList(tokens)));
    }
    ...
    public void setUpgrade(String products[]) {
        setHeaderValue(H_UPGRADE, ((products == null) ? null : new HttpTokenList(products)));
    }
    ...
    public void setVia(String vias[]) {
        setHeaderValue(H_VIA, ((vias == null) ? null : new HttpTokenList(vias)));
    }
    ...
    public void setTrailer(String trailers[]) {
        setHeaderValue(H_TRAILER, ((trailers == null) ? null : new HttpTokenList(trailers)));
    }
    ...
    protected void setNamespaces(HttpExtList extl) {
        HttpExt exts[] = extl.getHttpExts();
        edu.hkust.clap.monitor.Monitor.loopBegin(117);
for (int i = 0; i < exts.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(117);
{
            if (exts[i].isGenerated()) throw new HttpExtException("This extension Object is already " + "associated to an HttpMessage: " + exts[i].getName());
            if (exts[i].needsHeaders()) exts[i].setNamespace(generateExtNamespace());
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(117);
    ...
    public Dictionary getExtensionHeaders(HttpExt ext) {
        ...
        if (headers != null) {
            ...
            namespace += "-";
            edu.hkust.clap.monitor.Monitor.loopBegin(118);
while (henum.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(118);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(118);
     ...
     */
    public synchronized void setExtensionHeader(HttpExt ext, String name, String value) {
        if (ext.isGenerated()) throw new HttpExtException("This extension Object is already " + "associated to an HttpMessage: " + ext.getName());
        String namespace = ext.getNamespace();
        if (namespace == null) {
    ...
    public static final void registerExtHeader(String name, String cls) {
