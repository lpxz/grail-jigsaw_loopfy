...
public class HttpContentRange extends BasicValue {
    ...
    protected void parse() throws HttpParserException {
        ...
        ps.separator = (byte) ' ';
        if (HttpParser.nextItem(raw, ps) < 0) error("Invalid byte range (no byte unit).");
        this.unit = new String(raw, 0, ps.start, ps.end - ps.start);
        ps.separator = (byte) '-';
        ps.prepare();
        if (HttpParser.nextItem(raw, ps) < 0) error("Invalid byte range (no first position).");
        ...
        ps.prepare();
        ps.separator = (byte) '/';
        if (HttpParser.nextItem(raw, ps) < 0) error("Invalid byte range (no last position).");
        pos.ioff = ps.start;
        pos.bufend = ps.end;
        lastpos = HttpParser.parseInt(raw, pos);
        ps.prepare();
        if (HttpParser.nextItem(raw, ps) < 0) error("Invalid byte range (no full length).");
        pos.ioff = ps.start;
        pos.bufend = ps.end;
    ...
    public void setFirstPosition(int firstpos) {
        if (firstpos != this.firstpos) invalidateByteValue();
    ...
    public void setLastPosition(int lastpos) {
        if (lastpos != this.lastpos) invalidateByteValue();
    ...
    public void setFullLength(int length) {
        if (length != this.length) invalidateByteValue();
        this.length = length;
    }
    ...
    public HttpContentRange(boolean isValid, String unit, int firstpos, int lastpos, int length) {
        this.isValid = true;
        setUnit(unit);
