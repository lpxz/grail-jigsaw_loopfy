...
public class MuxInputStream extends InputStream implements MUX {
    ...
    private void addCredit(int consumed) throws IOException {
     ...
     */
    protected synchronized void push(byte data[], int off, int len, boolean noflush) throws IOException {
        if (debugPush) System.out.println("MuxReader[push]: " + len + " bytes" + ", noflush=" + noflush);
        if (closed) {
            return;
        }
        int bufpos = -1;
        int avail = -1;
        edu.hkust.clap.monitor.Monitor.loopBegin(630);
while (len > 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(630);
{
            bufpos = bufptr + buflen;
            avail = buffer.length - bufpos;
            if ((avail < len) && (bufptr > 0)) {
                System.arraycopy(buffer, bufptr, buffer, 0, buflen);
                if (markptr >= 0) markptr = (markptr >= bufptr) ? markptr - bufptr : -1;
                bufptr = 0;
                bufpos = buflen;
                avail = buffer.length - bufpos;
            }
            if (debugPush) System.out.println("push: " + len + " bytes, avail=" + avail);
            edu.hkust.clap.monitor.Monitor.loopBegin(631);
while (avail <= 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(631);
{
                if (buflen > 0) notifyAll();
                try {
                    wait();
...
edu.hkust.clap.monitor.Monitor.loopEnd(631);
            ...
            if (!(pushpending = noflush)) notifyAll();
            if (debugPush) System.out.println("push: " + buflen + " total bytes pushed.");
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(630);
