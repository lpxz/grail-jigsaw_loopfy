...
while (true) { 
{
    ...
    public DiscardServer(int port) throws IOException {
...
class MuxTestClient extends Thread {
    ...
    public static void makeClient(String host, int port, int bufsize, int count) throws IOException {
        if (socket == null) {
    ...
    public void run() {
        ...
        try {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(694);
while (--count >= 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(694);
out.write(buffer, 0, buffer.length);} 
edu.hkust.clap.monitor.Monitor.loopEnd(694);

            out.close();
            out.flush();
            long end = System.currentTimeMillis();
            System.out.println(this + ": emited " + total + " bytes " + " in " + (end - start) + " ms.");
        } catch (Exception ex) {
    ...
    MuxTestClient(MuxSession session, int bufsize, int count) throws IOException {
...
public class MuxTest {
    ...
    public static void main(String args[]) {
        ...
        String host = null;
        int port = -1;
        edu.hkust.clap.monitor.Monitor.loopBegin(695);
for (int i = 0; i < args.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(695);
{
            if (args[i].equals("-s")) {
                server = true;
...
edu.hkust.clap.monitor.Monitor.loopEnd(695);
