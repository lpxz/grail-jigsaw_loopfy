...
class MuxReader extends Thread {
    ...
    private final int computeWord(byte w0, byte w1, byte w2, byte w3) {
        return (((((int) w3) & 0xff) << 24) | ((((int) w2) & 0xff) << 16) | ((((int) w1) & 0xff) << 8) | ((((int) w0) & 0xff)));
    }
     ...
     */
    private final boolean parseMuxWord0(byte w0, byte w1, byte w2, byte w3, MuxMessage into) {
     ...
     * @see #readMessage
     */
    private final void parseMuxWord1(byte w0, byte w1, byte w2, byte w3, MuxMessage into) {
    ...
    private final void setupLongMessage(MuxMessage m) {
        if (setupControlMessage(m)) return;
        int a = -1;
        m.bytes = m.llen;
        m.pad = ((a = (m.llen & 0x7)) != 0) ? 8 - a : 0;
        m.hsize = 8;
     ...
     */
    private final void parseMuxHeader(MuxMessage into) throws IOException {
        edu.hkust.clap.monitor.Monitor.loopBegin(70);
while (buflen < 4) { 
edu.hkust.clap.monitor.Monitor.loopInc(70);
fillBuffer();} 
edu.hkust.clap.monitor.Monitor.loopEnd(70);

        boolean isLong = parseMuxWord0(buffer[bufptr], buffer[bufptr + 1], buffer[bufptr + 2], buffer[bufptr + 3], into);
        if (isLong) {
            edu.hkust.clap.monitor.Monitor.loopBegin(70);
while (buflen < 4) { 
edu.hkust.clap.monitor.Monitor.loopInc(70);
...
edu.hkust.clap.monitor.Monitor.loopEnd(70);
            ...
            parseMuxWord1(buffer[bufptr + 4], buffer[bufptr + 5], buffer[bufptr + 6], buffer[bufptr + 7], into);
    ...
    private final boolean parseMuxHeaderAhead(int ptr, int avail, MuxMessage into) {
        int a = -1;
        if (avail < 4) return false;
        boolean isLong = parseMuxWord0(buffer[ptr], buffer[ptr + 1], buffer[ptr + 2], buffer[ptr + 3], into);
        if (isLong) {
            if (avail < 8) return false;
            parseMuxWord1(buffer[ptr + 4], buffer[ptr + 5], buffer[ptr + 6], buffer[ptr + 7], into);
            setupLongMessage(into);
        } else {
            setupMessage(into);
        }
    ...
    private final void fillBuffer() throws IOException {
        ...
        if (got > 0) {
            buflen += got;
            if (debug) System.out.println("MuxReader.fillBuffer: " + buflen + " bytes.");
        } else if (got < 0) {
            stream.error(this, "Gracefull close.");
            throw new EOFException("Gracefull close.");
        }
        if (debug) System.out.println("MuxReader: got " + got + " bytes.");
    }
    ...
    protected void readMessage() throws IOException {
        ...
        if (nextAvailable = (nbufptr < bufptr + buflen)) {
            nextAvailable = parseMuxHeaderAhead(nbufptr, bufptr + buflen - nbufptr, nmsg);
        }
        if (debug) System.out.println("[readMessage] bufptr=" + bufptr + ", f=" + Integer.toString(msgflags, 16) + ", i=" + msgsessid + ", l=" + msglen + ", s=" + msgbytes);
    }
    ...
    private final int readMessageBody() throws IOException {
        if (debug) System.out.println("readMessageBody: " + msgbytes + " bytes avail.");
        if (msgbytes > 0) {
            if (buflen <= 0) fillBuffer();
            if (msgbytes > buflen) {
                msgbytes -= buflen;
                return buflen;
            } else {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(71);
while (buflen <= msgpad) { 
edu.hkust.clap.monitor.Monitor.loopInc(71);
fillBuffer();} 
edu.hkust.clap.monitor.Monitor.loopEnd(71);
    ...
    protected String msgToString() throws IOException {
        if (buffer.length < msglen) throw new RuntimeException("String doesn't hold in buffer !");
        edu.hkust.clap.monitor.Monitor.loopBegin(72);
while (buflen < msglen) { 
edu.hkust.clap.monitor.Monitor.loopInc(72);
fillBuffer();} 
edu.hkust.clap.monitor.Monitor.loopEnd(72);
    ...
    protected int[] msgShortArrayToIntArray() throws IOException {
        if (buffer.length < msglen) throw new RuntimeException("ShortArray doesn't hold in buffer !");
        edu.hkust.clap.monitor.Monitor.loopBegin(72);
while (buflen < msglen) { 
edu.hkust.clap.monitor.Monitor.loopInc(72);
fillBuffer();} 
edu.hkust.clap.monitor.Monitor.loopEnd(72);
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(73);
for (int i = 0; i < a.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(73);
{
            a[i] = (buffer[bufptr] | (buffer[bufptr + 1] << 8)) & 0xffff;
            bufptr += 2;
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(73);
    ...
    protected void dispatchMessage() throws IOException {
        msgsess = stream.lookupSession(msgflags, msgsessid, msglen, msgllen);
        if (msgsess != null) {
            if (msgisctrl) {
                ...
                int got = 0;
                edu.hkust.clap.monitor.Monitor.loopBegin(74);
while ((got = readMessageBody()) > 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(74);
{
                    msgsess.pushInput(buffer, bufptr, got, noflush);
                    bufptr += got;
                    buflen -= got;
                }} 
edu.hkust.clap.monitor.Monitor.loopEnd(74);

                if ((msgflags & MUX.FIN) == MUX.FIN) msgsess.notifyFIN();
                if ((msgflags & MUX.RST) == MUX.RST) msgsess.notifyRST();
                if ((msgflags & MUX.PUSH) == MUX.PUSH) msgsess.notifyPUSH();
            }
        } else {
            int got = -1;
            edu.hkust.clap.monitor.Monitor.loopBegin(75);
while ((got = readMessageBody()) > 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(75);
{
                bufptr += got;
                buflen -= got;
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(75);
    ...
    public void run() {
        try {
            edu.hkust.clap.monitor.Monitor.loopBegin(76);
while (alive) { 
edu.hkust.clap.monitor.Monitor.loopInc(76);
{
                readMessage();
                dispatchMessage();
                msgflags = 0;
...
edu.hkust.clap.monitor.Monitor.loopEnd(76);
    ...
    MuxReader(MuxStream stream, InputStream in) throws IOException {
        this.stream = stream;
        this.in = in;
