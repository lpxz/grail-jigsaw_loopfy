...
public class SampleMuxHandler implements MuxStreamHandler {
    ...
    public static synchronized MuxStreamHandler getStreamHandler() {
        if (sample == null) sample = new SampleMuxHandler();
        return sample;
    }
    ...
    public boolean acceptSession(MuxStream stream, int sessid, int protid) {
        ...
        if (o == null) {
            if (debug) System.out.println("Rejecting " + protid + " on " + sessid + ".");
            return false;
        } else {
            if (debug) System.out.println("Accepting " + protid + " on " + sessid + ".");
            return true;
        }
    }
    ...
    public void notifySession(MuxSession session) {
        ...
        if (o instanceof String) {
            ...
            try {
                ...
                log("Instantiating handler for " + protid + " of class \"" + strcls + "\" failed.");
                ex.printStackTrace();
                try {
                    session.shutdown();
        ...
        try {
            ...
            log("Launching handler for " + protid + " of class \"" + handler.getClass() + "\" failed.");
            ex.printStackTrace();
            try {
                session.shutdown();
            } catch (IOException exex) {
