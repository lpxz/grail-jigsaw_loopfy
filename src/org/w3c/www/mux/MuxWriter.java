...
class MuxWriter implements MUX {
    ...
    private static final byte padbytes[] = { (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0 };
    ...
    private final void encodeWord(int word) {
        ...
        buflen += 4;
        if (debug) System.out.println("[encodeWord] 0x" + Integer.toString(buffer[bufptr], 16) + Integer.toString(buffer[bufptr + 1], 16) + Integer.toString(buffer[bufptr + 2], 16) + Integer.toString(buffer[bufptr + 3], 16));
    }
     ...
     */
    private final void encodeMessage(int flags, int sessid, int length) throws IOException {
        ensureCapacity(4);
        int word = (flags | ((sessid & 0xff) << 18) | length);
        if (debug) System.out.println("sending h=" + Integer.toString(word, 16));
        encodeWord(word);
    }
     ...
     * @param len The message length.
     */
    private final void encodeLongMessage(int flags, int sessid, int protocol, int length) throws IOException {
        ensureCapacity(8);
        int word = (flags | ((sessid & 0xff) << 18) | protocol);
        if (debug) System.out.println("sending h=" + Integer.toString(word, 16) + ", l=" + Integer.toString(length, 16));
        encodeWord(word);
        encodeWord(length);
    }
     ...
     */
    private final void emitData(byte data[], int off, int len) throws IOException {
        if (len <= 0) return;
     ...
     * yet).
     */
    private final void emitData(String str, int len) throws IOException {
        if (len < 0) len = str.length();
        if (!ensureCapacity(len)) throw new RuntimeException("String to big to hold in buffer !");
     ...
     */
    private final void emitShortArray(int a[]) throws IOException {
        if (!ensureCapacity(a.length << 1)) throw new RuntimeException("Array to bug to hold in buffer.");
        edu.hkust.clap.monitor.Monitor.loopBegin(887);
for (int i = 0; i < a.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(887);
encodeShort((short) (a[i] & 0xffff));} 
edu.hkust.clap.monitor.Monitor.loopEnd(887);
     ...
     */
    protected synchronized void writeMessage(int sessid, int flags, int protocol, byte b[], int o, int l) throws IOException {
     ...
     * @param protid The protocol identifier.
     */
    protected synchronized void writeMessage(int sessid, int flags, int protocol) throws IOException {
     ...
     * @param l Length of data to be written.
     */
    protected synchronized void writeData(int sessid, byte b[], int o, int l) throws IOException {
        encodeMessage(0, sessid, l);
        if (l > 0) {
            emitData(b, o, l);
            int padlen = ((l & 0x3) != 0) ? (4 - (l & 0x3)) : 0;
            if (padlen != 0) emitData(padbytes, 0, padlen);
        }
    }

    protected void ctrlDefineString(int stackid, String id) throws IOException {
        int word = ((MUX.LONG_LENGTH | MUX.CONTROL) | (MUX.CTRL_DEFINE_STRING << 26) | (stackid & MUX.LENGTH));
        int len = id.length();
        synchronized (this) {
            encodeWord(word);
            encodeWord(len);
    ...
    protected void ctrlDefineStack(int id, int stack[]) throws IOException {
        int word = ((MUX.LONG_LENGTH | MUX.CONTROL) | (MUX.CTRL_DEFINE_STACK << 26) | (id & MUX.LENGTH));
    ...
    protected void ctrlMuxControl(int sessid, int fragsz) throws IOException {
        int word = ((MUX.LONG_LENGTH | MUX.CONTROL) | (MUX.CTRL_MUX_CONTROL << 26) | (sessid << 18) | (fragsz & MUX.LENGTH));
        synchronized (this) {
            encodeWord(word);
            encodeWord(0);
        }
    ...
    protected void ctrlSendCredit(int sessid, int credit) throws IOException {
        int word = ((MUX.LONG_LENGTH | MUX.CONTROL) | (MUX.CTRL_SEND_CREDIT << 26) | (sessid << 18));
        synchronized (this) {
            encodeWord(word);
            encodeWord(credit);
        }
    ...
    MuxWriter(MuxStream stream, OutputStream out) throws IOException {
        this.stream = stream;
        this.out = out;
