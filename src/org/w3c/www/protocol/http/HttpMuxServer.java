...
public class HttpMuxServer extends HttpServer {
    ...
    protected synchronized void acquireConnection() throws IOException {
        ...
        if (conn.incrUseCount()) manager.notifyUse(conn);
    }

    protected synchronized void releaseConnection() {
        if ((conn != null) && conn.decrUseCount()) manager.notifyIdle(conn);
    }
    ...
    public Reply runRequest(Request req) throws HttpException {
        ...
        try {
            acquireConnection();
            s = conn.connect(80);
            OutputStream os = (new DataOutputStream(new BufferedOutputStream(s.getOutputStream())));
            MimeParser parser = new MimeParser(s.getInputStream(), manager.getReplyFactory());
            if (isTwoStage(req)) {
                req.emit(os, Request.EMIT_HEADERS);
                os.flush();
                if (o != null) notifyObserver(o, new ConnectedEvent(this, req, os));
                rep = (Reply) parser.parse();
                if ((rep.getStatus() / 100) == 1) {
                    if (o != null) notifyObserver(o, new ContinueEvent(this, req, rep));
                ...
                if (o != null) notifyObserver(o, new ConnectedEvent(this, req, os));
                rep = (Reply) parser.parse();
                edu.hkust.clap.monitor.Monitor.loopBegin(1066);
while ((rep.getStatus() / 100) == 1) { 
edu.hkust.clap.monitor.Monitor.loopInc(1066);
{
                    if (o != null) notifyObserver(o, new ContinueEvent(this, req, rep));
                    req.emit(os, Request.EMIT_BODY | Request.EMIT_FOOTERS);
                    os.flush();
...
edu.hkust.clap.monitor.Monitor.loopEnd(1066);
     ...
     */
    public void initialize(HttpManager manager, HttpServerState state, String host, int port, int timeout) {
     ...
     * @param timeout The socket's timeout in millisec
     */
    public void initialize(HttpManager manager, HttpServerState state, String host, int port, int timeout, int conn_timeout) {
        this.state = state;
