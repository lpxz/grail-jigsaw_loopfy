...
public class CacheGeneration implements LRUAble {
    ...
    public long deleteStored(CachedResource cr) {
        if (!loaded) throw new UnloadedGenerationException("generation " + id);
        if (debug) {
            System.out.println("Deleting " + cr.getIdentifier() + "from generation: " + id);
        }
    ...
    public synchronized Enumeration getFiles() {
        ...
        if (loaded) {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(198);
while (fenum.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(198);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(198);

        } else {
            Enumeration fenum = lookupTable.elements();
            edu.hkust.clap.monitor.Monitor.loopBegin(199);
while (fenum.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(199);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(199);
     ...
     */
    public synchronized boolean addResource(CachedResource cr, long size, long oldsize) {
        if (!loaded) throw new UnloadedGenerationException("generation " + id);
        if (canStore(cr, size)) {
            CachedResource old_cr = null;
            old_cr = (CachedResource) lookupTable.get(cr.getIdentifier());
    ...
    protected void loadCachedResource(CachedResource cr) {
        File file = cr.getFile();
        if ((file != null) && ((!file.exists()) || (file.length() != cr.getCurrentLength()))) {
            return;
        }
        lookupTable.put(cr.getIdentifier(), cr);
        cr.generation = this;
     ...
     * generation     
     */
    public synchronized long removeResource(CachedResource cr) throws NoSuchResourceException {
        if (!loaded) throw new UnloadedGenerationException("generation " + id);
     ...
     * generation     
     */
    public synchronized long removeResource(String url) throws NoSuchResourceException {
        if (!loaded) throw new UnloadedGenerationException("generation " + id);
     ...
     * generation
     */
    private CachedResource _removeResource(String url) throws NoSuchResourceException {
        if (debug) {
    ...
    public long collectSpace(long size, boolean check) {
        if (!loaded) {
            try {
                ...
                if (debug) {
                    System.err.println("*** Collecting " + "Unable to load generation [" + getId() + "]");
        ...
        if (debug) {
            System.err.println("*** Collecting " + size + " bytes " + ((check) ? "with" : "without") + " checking");
        }
        if (size <= 0) return 0;
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(200);
while (cr != null) { 
edu.hkust.clap.monitor.Monitor.loopInc(200);
{
            if (check) {
...
edu.hkust.clap.monitor.Monitor.loopEnd(200);

        synchronized (this) {
            bytecount -= collected;
        }
        if (debug) {
            System.err.println("*** Collected " + collected + " bytes from generation " + id);
        }
        return collected;
    }
    ...
    protected long emptyGeneration() {
        ...
        long collected;
        if (debug) {
            System.err.println("*** Deleting Generation " + id + " (" + bytecount + ")");
        }
        synchronized (this) {
            collected = bytecount;
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(201);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(201);
{
            CachedResource cr = (CachedResource) e.nextElement();
            toDel.addElement(cr);
            store.getState().notifyResourceToBeDeleted(cr);
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(201);
    ...
    protected void setDescription(LookupTable tables[]) {
        ...
        bytecount = 0;
        cr_count = 0;
        edu.hkust.clap.monitor.Monitor.loopBegin(202);
for (int i = 0; i < tables.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(202);
{
            LookupTable table = tables[i];
            try {
                String stored = (String) table.get(CachedResource.NAME_CURRENT_LENGTH);
                String id = (String) table.get(CachedResource.NAME_IDENTIFIER);
                String file = (String) table.get(CachedResource.NAME_FILE);
                file = (file == null) ? "" : file;
                bytestored += Integer.parseInt(stored);
                lookupTable.put(id, file);
            } catch (Exception ex) {
                if (debug) {
                    System.err.println("Unable to load description in [" + getId() + "] " + ex.getMessage());
                }
            }
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(202);
    ...
    public void unload() {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(203);
while (kenum.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(203);
{
            lookupTable.put(kenum.nextElement(), Boolean.TRUE);
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(203);
    ...
    protected synchronized void copyInto(CacheGeneration gen) {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(204);
while (cr != null) { 
edu.hkust.clap.monitor.Monitor.loopInc(204);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(204);

        gen.setSaved(false);
        Enumeration e = gen.toDel.elements();
        edu.hkust.clap.monitor.Monitor.loopBegin(205);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(205);
{
            toDel.add(e.nextElement());
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(205);
