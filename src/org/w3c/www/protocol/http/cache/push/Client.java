...
public class Client {
     ...
     * Version Control Id 
     */
    public static final String VC_ID = "$Id: Client.java,v 1.1 2010/06/15 12:25:44 smhuang Exp $";
...
edu.hkust.clap.monitor.Monitor.loopEnd(752);
    ...
    protected void readReply() throws IOException {
        ...
        if (rv < PushCacheProtocol.PACKET_LEN) {
            throw new IOException("read returned less than PACKET_LEN bytes " + rv);
        }
        if (!PushCacheProtocol.instance().isValidProtocolTag(_buffer)) {
            throw new IOException("Bad protocol tag");
        }
        _data_stream = new DataInputStream(new ByteArrayInputStream(_buffer));
        ...
        _reply_command = new String(_buffer, PushCacheProtocol.HEADER_LEN, PushCacheProtocol.COMMAND_LEN);
