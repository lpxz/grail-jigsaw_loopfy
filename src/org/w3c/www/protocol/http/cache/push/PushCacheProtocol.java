...
public class PushCacheProtocol {
     ...
     */
    public static final int NO_SUCH_COMMAND = -1, ERR = 0, ADD = 1, DEL = 2, CLN = 3, PRS = 4, BYE = 5, OK = 6, NO = 7, NOP = 8;
    ...
    public byte[] errorPacket(String message) {
        try {
            java.io.ByteArrayOutputStream baos = new java.io.ByteArrayOutputStream(16);
    ...
    public boolean isValidProtocolTag(byte[] packet) {
        return (packet[0] == (byte) 'P' && packet[1] == (byte) 'C' || packet[2] == (byte) 'P' && packet[3] == (byte) 'P');
    }
