...
public class CacheStore implements PropertyMonitoring {
    ...
    public boolean propertyChanged(String name) {
        ...
        if (name.equals(CacheFilter.CACHE_SIZE_P)) {
            ...
            if ((dval <= (double) 0.00001) || (dval >= (double) 1.0)) return false;
            threshold = dval;
            return true;
    ...
    public CacheGeneration getLRULoadedGeneration() {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(808);
while ((gen != null) && (!gen.isLoaded())) { 
edu.hkust.clap.monitor.Monitor.loopInc(808);
{
            gen = (CacheGeneration) generations.getPrev(gen);
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(808);
    ...
    protected CacheGeneration addNewGeneration() throws InvalidCacheException {
        ...
        if ((state.getNbGeneration() < max_generation) && (byteleft > generationlimit)) {
            CacheGeneration newgen = new CacheGeneration(this, generationlimit);
     ...
     * @exception InvalidCacheException if the cache is corrupted
     */
    protected CacheGeneration loadGeneration(CacheGeneration cg) throws InvalidCacheException {
        CacheGeneration cglru = getLRULoadedGeneration();
        synchronized (this) {
            if (getCachedByteFree() < generationlimit) {
                if (cglru != null) unloadGeneration(cglru);
            }
     ...
     * @exception InvalidCacheException if the cache is corrupted
     */
    private CacheGeneration _loadGeneration(CacheGeneration cg) throws InvalidCacheException {
        try {
            Reader reader = new BufferedReader(new FileReader(cg.getGenerationFile()));
            cg = getSerializer().readGeneration(cg, reader);
            state.notifyGenerationLoaded(cg);
            return cg;
        } catch (FileNotFoundException ex) {
            String msg = "Generation file does not exists: " + cg.getGenerationFile().getAbsolutePath();
            throw new InvalidCacheException(msg);
        } catch (IOException ex) {
            String msg = "IOError reading " + cg.getGenerationFile().getAbsolutePath();
            throw new InvalidCacheException(msg);
        }
    }
     ...
     * @exception InvalidCacheException if the cache is corrupted
     */
    protected void unloadGeneration(CacheGeneration cg) throws InvalidCacheException {
        try {
            ...
            String msg = "IOError writing on " + cg.getGenerationFile().getAbsolutePath();
            throw new InvalidCacheException(msg);
        }
    }
     ...
     * @exception InvalidCacheException if the cache is corrupted
     */
    protected void saveGeneration(CacheGeneration cg) throws InvalidCacheException {
    ...
    protected synchronized void loadGenerations() throws InvalidCacheException {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(809);
for (int i = 0; i < len; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(809);
{
            Sorter.orderedFileInsert(genfiles[i], sorted);
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(809);

        sorted.copyInto(genfiles);
        edu.hkust.clap.monitor.Monitor.loopBegin(810);
for (int i = 0; i < len; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(810);
{
            int id = -1;
            try {
                ...
                if (id != -1) {
                    CacheGeneration gen = new CacheGeneration(this, generationlimit);
                    Reader reader = new BufferedReader(new FileReader(genfile));
                    ...
                    System.err.println("Invalid generation filename : " + genfiles[i].getName());
                }
            } catch (FileNotFoundException ex) {
                if (debug) {
...
edu.hkust.clap.monitor.Monitor.loopEnd(810);
    ...
    public synchronized void sync() {
        ...
        do {
            try {
                ...
                if (debug) {
                    System.err.println("Unable to save generation [" + cg.getId() + "] " + ex.getMessage());
                }
            }
     ...
     * @exception InvalidCacheException if the cache is corrupted
     */
    public CachedResource getCachedResource(String url) throws InvalidCacheException {
        CacheGeneration cg = (CacheGeneration) generations.getHead();
     ...
     */
    protected synchronized void resizeGeneration(CacheGeneration cg, CachedResource cr) {
        if (debug) {
            System.out.println("Resizing generation " + cg.getId());
        }
        long real_size = Math.max(cr.getContentLength(), cr.getCurrentLength());
        if (real_size > generationlimit) {
            cg.setByteLimit(real_size);
            generationlimit = (bytelimit - real_size) / (max_generation - 1);
        } else if (debug) {
     ...
     * @exception InvalidCacheException if the cache is corrupted
     */
    public CachedResource getCachedResourceReference(String url) throws InvalidCacheException {
        CacheGeneration cg = (CacheGeneration) generations.getHead();
        if (cg == null) return null;
        do {
            CachedResource cr = null;
     ...
     */
    public boolean storeCachedResource(CachedResource cr) throws InvalidCacheException {
     ...
     * @exception InvalidCacheException if the cache is corrupted
     */
    public boolean storeCachedResource(CachedResource cr, long oldsize) throws InvalidCacheException {
        CacheGeneration cg = (CacheGeneration) generations.getHead();
        ...
        if (cg != null) {
            ...
            if (new_cg != null) {
                if (!new_cg.addResource(cr, size, oldsize)) {
                    ...
                    if (!new_cg.addResource(cr, size, oldsize)) {
                        String msg = "Unable to add a cachedResource in a " + "resized generation!!";
                        throw new InvalidCacheException(msg);
                    }
                }
            } else {
                if ((state.getStoreCount() < (storelimit - generationlimit)) && (state.getNbGeneration() < max_generation)) {
                    edu.hkust.clap.monitor.Monitor.loopBegin(811);
while (new_cg == null) { 
edu.hkust.clap.monitor.Monitor.loopInc(811);
{
                        CacheGeneration older_cg = getLRULoadedGeneration();
                        if (older_cg == null) {
                            String msg = "No Generation Loaded but store limit reached";
                            throw new InvalidCacheException(msg);
                        }
                        synchronized (this) {
...
edu.hkust.clap.monitor.Monitor.loopEnd(811);
                    ...
                    synchronized (this) {
                        ...
                        generationlimit = ((generationlimit * (max_generation - 1)) + cg_limit) / max_generation;
    ...
    private String getFileNumber(int nb, int size) {
        ...
        if (nb2 == 0) {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(812);
while (nb2 > 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(812);
{
                nb2 = nb2 / 10;
                cpt++;
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(812);

        }
        int zero2add = size - cpt;
        StringBuffer buffer = new StringBuffer();
        edu.hkust.clap.monitor.Monitor.loopBegin(813);
for (int i = 0; i < zero2add; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(813);
{
            buffer.append("0");
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(813);
     ...
     * @exception InvalidCacheException if the cache is corrupted
     */
    private synchronized void setGenerationFile(CacheGeneration gen) throws InvalidCacheException {
        int current_generation = state.incrCurrentGeneration();
        File file = new File(cache_dir, GENERATION_FILENAME + getFileNumber(current_generation, 4));
        if (debug) {
    ...
    public String toString() {
        StringBuffer buffer = new StringBuffer();
        buffer.append(">>> CacheStore [").append(cache_dir.getAbsolutePath()).append("] <<<");
    ...
    protected void checkDirs() {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(814);
for (int i = 0; i < nb_dir; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(814);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(814);
    ...
    protected int cleanCacheDir() {
        ...
        CacheGeneration cg = (CacheGeneration) generations.getHead();
        edu.hkust.clap.monitor.Monitor.loopBegin(815);
while (cg != null) { 
edu.hkust.clap.monitor.Monitor.loopInc(815);
{
            Enumeration fenum = cg.getFiles();
            edu.hkust.clap.monitor.Monitor.loopBegin(816);
while (fenum.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(816);
{
                entryfiles.put(fenum.nextElement(), Boolean.TRUE);
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(816);

            cg = getNextGeneration(cg);
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(815);

        int cpt = 0;
        edu.hkust.clap.monitor.Monitor.loopBegin(817);
for (int i = 0; i < dirs.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(817);
{
            File files[] = dirs[i].listFiles();
            if (files != null) {
                edu.hkust.clap.monitor.Monitor.loopBegin(818);
for (int j = 0; j < files.length; j++) { 
edu.hkust.clap.monitor.Monitor.loopInc(818);
{
                    if (entryfiles.get(files[j]) == null) {
                        if (files[j].delete()) {
                            cpt++;
                            if (debug) {
                                System.out.println(files[j] + " not used, removed");
                            }
                        }
                    }
                }} 
edu.hkust.clap.monitor.Monitor.loopEnd(818);

            }
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(817);
    ...
    protected void compactGenerations() {
        if (debug) System.out.println("*** trying to compact generations");
        if (state.getNbGeneration() < max_generation) return;
        if (debug) System.out.println("*** compact: Max reached, compacting...");
        CacheGeneration gen = getMRUGeneration();
        CacheGeneration nextGen;
        gen = getNextGeneration(gen);
        edu.hkust.clap.monitor.Monitor.loopBegin(819);
while (gen != null) { 
edu.hkust.clap.monitor.Monitor.loopInc(819);
{
            if (debug) {
                System.out.println("*** compact: working on generation " + gen.getId());
            }
            nextGen = getNextGeneration(gen);
            if (nextGen == null) {
                break;
            }
            if ((gen.getCachedByteCount() + nextGen.getCachedByteCount()) < gen.getByteLimit()) {
                synchronized (generations) {
                    if (debug) {
                        System.out.println("*** compact: merging (" + gen.getId() + ") and (" + nextGen.getId() + ")");
                    }
...
edu.hkust.clap.monitor.Monitor.loopEnd(819);
    ...
    protected synchronized void checkState() {
        ...
        long bc = 0;
        long sc = 0;
        edu.hkust.clap.monitor.Monitor.loopBegin(820);
while (cg != null) { 
edu.hkust.clap.monitor.Monitor.loopInc(820);
{
            ...
            if (pc.length() > 5) pc = pc.substring(0, 5);
...
edu.hkust.clap.monitor.Monitor.loopEnd(820);

        System.out.println(">>> Check State <<<");
        System.out.println("  Byte Count  <= Byte Limit    : " + (byteCount <= bytelimit));
        System.out.println("  Store Count <= Store Limit   : " + (storeCount <= storelimit));
        System.out.println("  CR Count    <= CR Limit      : " + (crCount <= cr_limit));
        System.out.println("  Byte Count  <= Store Count   : " + (byteCount <= storeCount));
        System.out.println("  CR Count    <= Entry Num     : " + (crCount <= entryNum));
        System.out.println("  Current gen >= Number of gen : " + (currentGeneration >= nbGeneration));
        System.out.println("  Generations SC == Store SC   : " + (sc == storeCount));
        System.out.println("  Generations BC == Store BC   : " + (bc == byteCount));
    }
    ...
    public void initialize(CacheFilter filter) throws InvalidCacheException {
        ...
        garbageCollectionEnabled = props.getBoolean(GARBAGE_COLLECTION_ENABLED_P, true);
