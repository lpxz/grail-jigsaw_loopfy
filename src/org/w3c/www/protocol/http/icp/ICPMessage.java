...
class ICPMessage implements ICP {
     ...
     * protocol.
     */
    protected static ICPMessage parse(DatagramPacket p) throws ICPProtocolException {
        byte buf[] = p.getData();
        int len = p.getLength();
        if (len < 4) throw new ICPProtocolException("Invalid ICP datagram length");
        InetAddress addr = p.getAddress();
        int port = p.getPort();
        int opcode = buf[0];
        int version = buf[1];
        int plen = (((buf[2] & 0xff) << 8) + (buf[3] & 0xff));
        if (plen != len) throw new ICPProtocolException("Didn't got full message: " + len + "/" + plen);
        ICPMessage msg = null;
        switch(opcode) {
            case ICP_OP_QUERY:
    ...
    protected int parse(byte buf[], int off, int len) throws ICPProtocolException {
        this.id = (((buf[off] & 0xff) << 24) + ((buf[off + 1] & 0xff) << 16) + ((buf[off + 2] & 0xff) << 8) + ((buf[off + 3] & 0xff) << 0));
        off += 4;
        this.options = (((buf[off] & 0xff) << 24) + ((buf[off + 1] & 0xff) << 16) + ((buf[off + 2] & 0xff) << 8) + ((buf[off + 3] & 0xff) << 0));
        off += 4;
        off += 4;
        return off;
    }
    ...
    protected int toByteArray(byte buf[]) {
        ...
        if (length > buf.length) return -length;
        buf[0] = (byte) opcode;
