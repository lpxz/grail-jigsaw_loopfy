...
public class ICPFilter implements PropRequestFilter, PropertyMonitoring {
    ...
    protected void createICPSender(String host, int dstport, String http) throws UnknownHostException, MalformedURLException, SocketException {
        ...
        Long key = new Long((((long) dstport) << 32) + ((baddr[0] & 0xff) << 24) + ((baddr[1] & 0xff) << 16) + ((baddr[2] & 0xff) << 8) + (baddr[3] & 0xff));
        friends.put(key, sender);
        if (debug) System.out.println("icp: friend " + key + " http=" + http);
    }
    ...
    protected void parseConfiguration() {
        ...
        try {
            edu.hkust.clap.monitor.Monitor.loopBegin(549);
for (String line = null; (line = in.readLine()) != null; ) { 
edu.hkust.clap.monitor.Monitor.loopInc(549);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(549);

        } catch (Exception ex) {
            System.out.println("*** ICP, unable to create " + host + "@" + port + "[" + http + "]: " + ex.getMessage());
        } finally {
            try {
                if (in != null) in.close();
            } catch (IOException ex) {
    ...
    public ICPSender getSender(InetAddress addr, int port) {
        byte baddr[] = addr.getAddress();
        Long key = new Long((((long) port) << 32) + ((baddr[0] & 0xff) << 24) + ((baddr[1] & 0xff) << 16) + ((baddr[2] & 0xff) << 8) + (baddr[3] & 0xff));
    ...
    protected URL locateProxy(ICPReply reply) {
        ICPSender sender = getSender(reply.getSenderAddress(), reply.getSenderPort());
    ...
    protected int sendQuery(ICPQuery query) {
        int count = 0;
        if (senders != null) {
            edu.hkust.clap.monitor.Monitor.loopBegin(550);
for (int i = 0; i < senders.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(550);
{
                if (debug) System.out.println("icp: query@" + senders[i].getAddress() + "/" + senders[i].getPort() + " for " + query.getURL());
                if (senders[i].send(query)) count++;
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(550);
    ...
    protected URL runQuery(ICPQuery query) {
        ...
        try {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(551);
while ((sent > 0) && (timeout > 0)) { 
edu.hkust.clap.monitor.Monitor.loopInc(551);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(551);
    ...
    public void initialize(HttpManager manager) throws PropRequestFilterException {
        ...
        if (debug) System.out.println("icp: listening on port " + port);
