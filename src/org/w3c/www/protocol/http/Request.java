...
public class Request extends HttpRequestMessage {
    ...
    protected void endEmit(OutputStream out, int what) throws IOException {
        if ((what & EMIT_BODY) != EMIT_BODY) return;
        if (output != null) {
            byte buf[] = new byte[1024];
            int cnt = 0;
            int total = 0;
            edu.hkust.clap.monitor.Monitor.loopBegin(749);
while ((cnt = output.read(buf)) >= 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(749);
{
                total += cnt;
                out.write(buf, 0, cnt);
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(749);
    ...
    public synchronized void interruptRequest() {
        interrupted = true;
        if (server != null) server.interruptRequest(this);
    }
