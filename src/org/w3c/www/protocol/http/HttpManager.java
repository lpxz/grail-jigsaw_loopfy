...
class ManagerDescription {
    ...
    final boolean sameProperties(Properties props) {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(332);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(332);
{
            String name = (String) e.nextElement();
            String prop = properties.getProperty(name);
            if ((prop == null) || (!prop.equals(props.getProperty(name)))) return false;
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(332);
...
public class HttpManager implements PropertyMonitoring {
    ...
    protected Hashtable _tmp_servers = null;
    ...
    protected boolean updateProxy() {
        ...
        if (set) {
            ...
            if ((proxy != null) && (proxy.getHost() != null)) template.setProxy(proxy);
        } else {
    ...
    public boolean propertyChanged(String name) {
        ...
        if (name.equals(ONLY_IF_CACHED_P)) {
            ...
            tpl.setValue("user-agent", props.getString(USER_AGENT_P, DEFAULT_USER_AGENT));
            return true;
        } else if (name.equals(ACCEPT_P)) {
            tpl.setValue("accept", props.getString(ACCEPT_P, DEFAULT_ACCEPT));
            return true;
        } else if (name.equals(ACCEPT_LANGUAGE_P)) {
            String sval = props.getString(ACCEPT_LANGUAGE_P, null);
            if (sval != null) tpl.setValue("accept-language", sval);
            return true;
        } else if (name.equals(ACCEPT_ENCODING_P)) {
            String sval = props.getString(ACCEPT_ENCODING_P, null);
            if (sval != null) tpl.setValue("accept-encoding", sval);
            return true;
        } else if (name.equals(PROXY_SET_P) || name.equals(PROXY_HOST_P) || name.equals(PROXY_PORT_P)) {
            return updateProxy();
        } else {
            return true;
        }
    ...
    protected static synchronized HttpManager getManager(Class managerclass, Properties p) {
        edu.hkust.clap.monitor.Monitor.loopBegin(333);
for (int i = 0; i < managers.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(333);
{
            if (managers[i] == null) continue;
            if (managers[i].sameProperties(p)) return managers[i].getManager();
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(333);

        ObservableProperties props = null;
        if (p instanceof ObservableProperties) props = (ObservableProperties) p; else props = new ObservableProperties(p);
        HttpManager manager = null;
        ;
        try {
        ...
        if (filters != null) {
            edu.hkust.clap.monitor.Monitor.loopBegin(334);
for (int i = 0; i < filters.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(334);
{
                try {
                    ...
                    System.out.println("Couldn't initialize filter \"" + filters[i] + "\" init failed: " + ex.getMessage());
                } catch (Exception ex) {
                    System.err.println("Error initializing prop filters:");
                    System.err.println("Coulnd't initialize [" + filters[i] + "]: " + ex.getMessage());
                    ex.printStackTrace();
                    System.exit(1);
                }
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(334);
        ...
        try {
            ...
            System.err.println("Class \"" + c + "\" not found, from property " + SERVER_CLASS_P);
            ex.printStackTrace();
            System.exit(1);
        }
        ...
        manager.conn_timeout = props.getInteger(CONN_TIMEOUT_P, manager.conn_timeout);
        props.registerObserver(manager);
        edu.hkust.clap.monitor.Monitor.loopBegin(335);
for (int i = 0; i < managers.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(335);
{
            if (managers[i] == null) {
...
edu.hkust.clap.monitor.Monitor.loopEnd(335);
    ...
    public final String getServerKey(Request request) {
        ...
        String key = null;
        if (proxy != null) {
            return ((proxy.getPort() == 80) ? proxy.getHost().toLowerCase() : (proxy.getHost().toLowerCase() + ":" + proxy.getPort()));
        } else {
            return ((target.getPort() == 80) ? target.getHost().toLowerCase() : (target.getHost().toLowerCase() + ":" + target.getPort()));
        }
    }
     ...
     */
    protected HttpServer lookupServer(String host, int port) throws HttpException {
        int p = (port == -1) ? 80 : port;
        String id = ((p == 80) ? host.toLowerCase() : (host.toLowerCase() + ":" + p));
        ...
        synchronized (_tmp_servers) {
            if (_tmp_servers.containsKey(id)) {
                ...
                synchronized (server) {
                    edu.hkust.clap.monitor.Monitor.loopBegin(336);
while (server.state == null || server.state.state != HttpServerState.PREINIT) { 
edu.hkust.clap.monitor.Monitor.loopInc(336);
{
                        try {
                            wait(100);
...
edu.hkust.clap.monitor.Monitor.loopEnd(336);
                ...
                try {
                    ...
                    String msg = ("Unable to create an instance of \"" + serverclass.getName() + "\", invalid config, check the " + SERVER_CLASS_P + " property.");
        ...
        try {
            synchronized (server) {
                server.initialize(this, new HttpServerState(server), host, p, timeout, conn_timeout);
                try {
                    notifyAll();
    ...
    public synchronized void notifyUse(HttpConnection conn) {
        if (debug) System.out.println("+++ connection used");
    ...
    public synchronized void notifyIdle(HttpConnection conn) {
        if (debug) System.out.println("+++ connection idle");
    ...
    protected synchronized void notifyConnection(HttpConnection conn) {
        if (debug) System.out.println("+++ notify conn_count " + (conn_count + 1) + " / " + conn_max);
        if (++conn_count > conn_max) closeAnyConnection();
    }
    ...
    protected synchronized void deleteConnection(HttpConnection conn) {
        --conn_count;
        connectionsLru.remove(conn);
        if (debug) System.out.println("+++ delete conn_count: " + conn_count);
        notifyAll();
    }
    ...
    protected synchronized boolean closeAnyConnection() {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(337);
for (int i = 0; i < max; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(337);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(337);

        synchronized (servers) {
            Enumeration e = servers.keys();
            if (debug) {
                System.out.println("+++ hashtable purge starting: " + servers.size() + " entries");
            }
            int nbconn = 0;
            int rnbconn = 0;
            int nbkept = 0;
            edu.hkust.clap.monitor.Monitor.loopBegin(338);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(338);
{
                ...
                if (id != null) {
                    ...
                    if (conn_count <= 0) {
                        ...
                        if (debug) {
                            ...
                            System.out.println("+++ hashtable keep: " + id + " ( " + server.state.getConnectionCount() + " )");
                        }
                    }
                }
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(338);

            if (debug) {
                System.out.println("+++ hashtable purge done, keeping " + servers.size() + " entries");
                System.out.println("+++ hashtable stats, keeping " + nbconn + " ( " + rnbconn + " ) connections for " + nbkept + " servers ( " + ((float) nbconn / (float) nbkept) + " )");
            }
        }
        return saved;
    }
    ...
    protected final synchronized void incrConnCount(HttpServer server) {
        if (++conn_count > conn_max) closeAnyConnection();
        if (debug) System.out.println("+++ incr conn_count: " + conn_count);
    }
    ...
    protected final synchronized void decrConnCount(HttpServer server) {
        --conn_count;
        if (debug) System.out.println("+++ decr conn_count: " + conn_count);
        if (conn_count < 0) {
    ...
    public Reply runRequest(Request request) throws HttpException {
        ...
        if (filters != null) {
            edu.hkust.clap.monitor.Monitor.loopBegin(339);
for (int i = 0; i < filters.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(339);
{
                if ((reply = filters[fcalls].ingoingFilter(request)) != null) break;
                fcalls++;
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(339);
        ...
        if (reply == null) {
            ...
            do {
                ...
                try {
                    URL proxy = request.getProxy();
                    if (proxy != null) srv = lookupServer(proxy.getHost(), proxy.getPort()); else srv = lookupServer(target.getHost(), target.getPort());
                    request.setServer(srv);
                    reply = srv.runRequest(request);
                } catch (HttpException ex) {
                    edu.hkust.clap.monitor.Monitor.loopBegin(340);
for (int i = 0; i < fcalls; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(340);
rtry = rtry || filters[i].exceptionFilter(request, ex);} 
edu.hkust.clap.monitor.Monitor.loopEnd(340);
        ...
        if (filters != null) {
            edu.hkust.clap.monitor.Monitor.loopBegin(341);
while (--fcalls >= 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(341);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(341);
    ...
    public void setFilter(URL incs[], URL exs[], RequestFilter filter) {
        if (incs != null) {
            edu.hkust.clap.monitor.Monitor.loopBegin(342);
for (int i = 0; i < incs.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(342);
filteng.setFilter(incs[i], true, filter);} 
edu.hkust.clap.monitor.Monitor.loopEnd(342);

        }
        if (exs != null) {
            edu.hkust.clap.monitor.Monitor.loopBegin(343);
for (int i = 0; i < exs.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(343);
filteng.setFilter(exs[i], false, filter);} 
edu.hkust.clap.monitor.Monitor.loopEnd(343);
    ...
    public synchronized void setTimeout(int timeout) {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(344);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(344);
{
            ((HttpServer) e.nextElement()).setTimeout(timeout);
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(344);
    ...
    public synchronized void setConnTimeout(int conn_timeout) {
        ...
        Enumeration e = servers.elements();
        edu.hkust.clap.monitor.Monitor.loopBegin(345);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(345);
{
            ((HttpServer) e.nextElement()).setConnTimeout(conn_timeout);
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(345);
    ...
    public synchronized String toString() {
        ...
        if (hcn != null) {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(346);
while (hcn != null) { 
edu.hkust.clap.monitor.Monitor.loopInc(346);
{
                sb.append("      ");
...
edu.hkust.clap.monitor.Monitor.loopEnd(346);
    ...
    public static void main(String args[]) {
        try {
            ...
            PropRequestFilter filter = new org.w3c.www.protocol.http.cookies.CookieFilter();
            filter.initialize(manager);
            PropRequestFilter pdebug = new org.w3c.www.protocol.http.DebugFilter();
            ...
            int cnt = 0;
            edu.hkust.clap.monitor.Monitor.loopBegin(347);
while ((cnt = in.read(buf)) >= 0) { 
edu.hkust.clap.monitor.Monitor.loopInc(347);
{
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(347);
