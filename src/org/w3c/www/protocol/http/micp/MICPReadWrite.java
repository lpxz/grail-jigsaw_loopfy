...
public class MICPReadWrite implements MICP {
     ...
     */
    public MICPMessage decode(byte buf[], int len, MICPMessage into) throws MICPProtocolException {
        if (len < 12) throw new MICPProtocolException("invalid buffer length " + len);
        int packlen = decodeShort(buf, 2);
        if (len < packlen) throw new MICPProtocolException("invalid length " + packlen + "/" + len);
    ...
    public int encode(int op, int src, int id, String url, byte buf[]) {
        ...
        buf[0] = (byte) (MICP_VERSION & 0xff);
        buf[1] = (byte) (op & 0xff);
        encodeShort(buf, 2, len);
        encodeInt(buf, 4, src);
        encodeInt(buf, 8, id);
        encodeString(buf, 12, url);
        return len;
    }
}
