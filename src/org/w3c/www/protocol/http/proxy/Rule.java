...
class ForbidRule extends Rule {
    ...
    public Reply apply(Request request) {
        ...
        reply.setContent("<h1>Access forbidden</h1>" + "<p>Access to " + request.getURL() + " is forbidden by proxy dispatcher rules.");
     ...
     * from given tokens.
     */
    public void initialize(String tokens[], int offset, int length) throws RuleParserException {
...
class ProxyRule extends Rule {
     ...
     */
    public void initialize(String tokens[], int offset, int length) throws RuleParserException {
        if (offset + 1 != length) throw new RuleParserException("No target proxy.");
        try {
            args = tokens[offset];
            proxy = new URL(args);
        } catch (MalformedURLException ex) {
            throw new RuleParserException("Invalid target proxy \"" + tokens[offset] + "\".");
        }
        host = tokens[0];
    }
...
class RedirectRule extends Rule {
     ...
     */
    public Reply apply(Request request) {
        if (redirect != null) request.setURL(redirect);
        return null;
    }
     ...
     */
    public void initialize(String tokens[], int offset, int length) throws RuleParserException {
        if (offset + 1 != length) throw new RuleParserException("No target redirect URL.");
        try {
            args = tokens[offset];
            redirect = new URL(args);
        } catch (MalformedURLException ex) {
            throw new RuleParserException("Invalid target redirect URL \"" + tokens[offset] + "\".");
        }
        host = tokens[0];
    }
...
class AuthorizationRule extends Rule {
     ...
     */
    public Reply apply(Request request) {
        if (!request.hasHeader(HttpRequestMessage.H_AUTHORIZATION)) request.setHeaderValue(HttpRequestMessage.H_AUTHORIZATION, credential);
        return null;
    }
     ...
     */
    public void initialize(String tokens[], int offset, int length) throws RuleParserException {
        if (offset + 2 != length) throw new RuleParserException("Invalid authorization rule: " + " should be authorization " + " <user> <password>.");
        credential = HttpFactory.makeCredential("Basic");
        user = tokens[offset];
        password = tokens[offset + 1];
        args = user + " " + password;
        Base64Encoder base64 = new Base64Encoder(user + ":" + password);
...
class ProxyAuthRule extends Rule {
    ...
    public Reply apply(Request request) {
        if (proxy != null) request.setProxy(proxy);
        if (!request.hasHeader(HttpRequestMessage.H_PROXY_AUTHORIZATION)) request.setProxyAuthorization(credential);
        return null;
    }
     ...
     */
    public void initialize(String tokens[], int offset, int length) throws RuleParserException {
        if (offset + 3 != length) throw new RuleParserException("Invalid proxyauth rule: " + " should be authorization " + " <user> <password> <proxy>.");
        try {
            user = tokens[offset];
            password = tokens[offset + 1];
            credential = HttpFactory.makeCredential("Basic");
...
public class Rule implements Comparable {
     ...
     * from given tokens.
     */
    protected void initialize(String tokens[], int offset, int length) throws RuleParserException {
        if (offset != length) throw new RuleParserException("Unexpected token: " + tokens[offset]);
        host = tokens[0];
    }
     ...
     * tokens.
     */
    public static Rule createRule(String tokens[], int offset, int length) throws RuleParserException {
        Rule rule = null;
        if ((tokens == null) || (length - offset == 0)) return null;
