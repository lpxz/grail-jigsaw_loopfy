...
public class ProxyDispatcher implements PropRequestFilter, PropertyMonitoring {
    ...
    protected void parseRules() {
        if (debug) System.out.println("PARSING RULES...");
        ...
        try {
            ...
            try {
                in = (new BufferedInputStream(new FileInputStream(new File(ruleurl))));
            } catch (Exception nex) {
                System.err.println("* ProxyDispatcher: unable to open rule " + "file \"" + ruleurl + "\"");
                rules = null;
                return;
            }
        }
        ...
        if (rules != null) {
            ...
            if (rule != null) {
                if (debug) {
                    ...
                    System.out.println("[" + getClass().getName() + "]: applying rule <" + rule.getRuleName() + args + "> to " + request.getURL());
                }
    ...
    public boolean exceptionFilter(Request request, HttpException ex) {
        if (request.hasProxy()) {
            ...
            if (debug) System.out.println("[" + getClass().getName() + "]: direct fetch " + "for " + request.getURL());
            return true;
        }
        return false;
    }
    ...
    public void initialize(HttpManager manager) {
        ...
        parseRules();
        if (debug = props.getBoolean(DEBUG_P, false)) System.out.println("[" + getClass().getName() + ": debuging on.");
        check_rules = props.getBoolean(CHECK_RULES_LAST_MODIFIED_P, false);
