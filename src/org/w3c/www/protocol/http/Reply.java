...
public class Reply extends HttpReplyMessage {
    ...
    public boolean keepsAlive() {
        if (major >= 1) {
            if (minor >= 1) return ((getContentLength() >= 0) || hasTransferEncoding("chunked"));
            return (hasConnection("keep-alive") || hasProxyConnection("keep-alive"));
        }
        return false;
    }
    ...
    protected void matchesRequest(Request request) {
        ...
        if (mth.equals("HEAD")) setStream(null);
    }
    ...
    public InputStream getInputStream() throws IOException {
        ...
        if (len >= 0) {
            input = new ContentLengthInputStream(observer, parser.getInputStream(), len);
            definesInput = true;
            return input;
        }
        if (observer != null) observer.notifyFailure(parser.getInputStream());
    ...
    public void setContent(String msg) {
        if (!hasHeader(H_CONTENT_TYPE)) setHeaderValue(H_CONTENT_TYPE, DEFAULT_TYPE);
