...
public class ResourceTreeBrowser extends JTree implements DropTargetListener, ResourceActionListener {
    ...
    MouseAdapter mouseAdapter = new MouseAdapter() {
        ...
        private void maybeShowPopup(MouseEvent e) {
            if (e.isPopupTrigger()) {
                ...
                if (rrw != null) getPopupMenu(rrw).show(e.getComponent(), e.getX(), e.getY());
            }
        }
    };
    ...
    protected boolean dropResource(ResourceCell cell) {
        ...
        try {
            if (!pm.isExtensible(rrw)) {
                return false;
            } else if ((cell.isFrame() || cell.isFilter()) && (pm.isEditable(rrw))) {
                popupResource(rrw);
                return true;
            } else if (((cell.isContainer() || cell.isResource()) && (!rrw.getResource().isIndexersCatalog())) || (rrw.getResource().isIndexersCatalog() && cell.isIndexer())) {
                if (rrw.getResource().isContainer()) {
                    String identifier = getIdentifier(cell, rrw);
                    if (identifier != null) {
                        TreePath path = getSelectionPath();
                        addResource(identifier, cell.toString(), rrw, path);
                        return true;
                    }
                }
            }
     ...
     */
    protected void addResource(String identifier, String classname, RemoteResourceWrapper rrwf, TreePath fpath) throws RemoteAccessException {
        RemoteResource rc = rrwf.getResource().registerResource(identifier, classname);
        RemoteResourceWrapper nrrw = new RemoteResourceWrapper(rrwf, rc);
        RemoteResourceWrapperNode parent = (RemoteResourceWrapperNode) fpath.getLastPathComponent();
        RemoteResourceWrapperNode child = new RemoteResourceWrapperNode(parent, nrrw, identifier);
     ...
     */
    protected void addResourceToSelectedContainer(String classname, String identifier) throws RemoteAccessException {
        RemoteResourceWrapper rrw = getSelectedResourceWrapper();
        if (rrw == null) return;
     ...
     * @see org.w3c.jigadmin.editors.ResourceCell
     */
    protected String getIdentifier(ResourceCell cell, RemoteResourceWrapper rrw) throws RemoteAccessException {
        ...
        int cpt = 0;
        int i = 0;
        edu.hkust.clap.monitor.Monitor.loopBegin(1020);
        loop: 
while (i < names.length) { 
edu.hkust.clap.monitor.Monitor.loopInc(1020);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(1020);
    ...
    protected RemoteResourceWrapper getSelectedResourceWrapper() {
        RemoteResourceWrapperNode node = (RemoteResourceWrapperNode) getLastSelectedPathComponent();
    ...
    protected RemoteResourceWrapper getSelectedResourceWrapper(TreePath path) {
        ...
        RemoteResourceWrapperNode node = (RemoteResourceWrapperNode) path.getLastPathComponent();
        return node.getResourceWrapper();
    }
     ...
     */
    protected AddResourcePanel getAddResourcePanel(String title, RemoteResourceWrapper rrw) throws RemoteAccessException {
     ...
     * @param rrw The wrapper of the father RemoteResource
     */
    protected void popupAddResourceDialog(String title, RemoteResourceWrapper rrw) {
        try {
            AddResourcePanel arp = getAddResourcePanel(title, rrw);
            JFrame frame = rootNode.getResourceWrapper().getServerBrowser().getFrame();
            ...
            popup.show();
            arp.getFocus();
            edu.hkust.clap.monitor.Monitor.loopBegin(1021);
while (!arp.waitForCompletion()) { 
edu.hkust.clap.monitor.Monitor.loopInc(1021);
;} 
edu.hkust.clap.monitor.Monitor.loopEnd(1021);
    ...
    private void performAddResourceToSelectedContainer() {
        ...
        if ((resIdentifier != null) && (resClassname != null)) {
            try {
                addResourceToSelectedContainer(resClassname, resIdentifier);
            } catch (RemoteAccessException ex) {
                Message.showErrorMessage(this, ex);
            }
        }
    ...
    protected void addResourceToSelectedContainer() {
        ...
        try {
            if (selected == null) {
                JOptionPane.showMessageDialog(this, "No Container selected", "Error", JOptionPane.ERROR_MESSAGE);
                return;
            } else if (!pm.isExtensible(selected)) {
                JOptionPane.showMessageDialog(this, "The resource selected is not " + "extensible.", "Error", JOptionPane.ERROR_MESSAGE);
                ...
                JOptionPane.showMessageDialog(this, "The resource selected is not " + "container.", "Error", JOptionPane.ERROR_MESSAGE);
            }
        } catch (RemoteAccessException ex) {
            Message.showErrorMessage(this, ex);
        }
    ...
    protected TreePath[] removeDescendants(TreePath[] paths) {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(1022);
for (int i = 0; i < paths.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1022);
{
            TreePath currentp = paths[i];
            boolean hasParent = false;
            edu.hkust.clap.monitor.Monitor.loopBegin(1023);
for (int j = 0; j < paths.length; j++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1023);
{
                if ((!(j == i)) && (paths[j].isDescendant(currentp))) hasParent = true;
            }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1023);

            if (!hasParent) newpaths.addElement(currentp);
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(1022);
    ...
    protected void reindexSelectedResources(boolean rec) {
        ...
        if (path.length > 0) {
            int result = JOptionPane.showConfirmDialog(this, "Reindex selected resource(s)?", "Reindex Resource(s)", JOptionPane.YES_NO_OPTION);
            if (result == JOptionPane.YES_OPTION) {
                edu.hkust.clap.monitor.Monitor.loopBegin(1024);
for (int i = 0; i < path.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1024);
{
                    RemoteResourceWrapper rrw = getSelectedResourceWrapper(path[i]);
                    if (rrw == null) continue;
                    try {
...
edu.hkust.clap.monitor.Monitor.loopEnd(1024);
     ...
     */
    protected void reindexResource(RemoteResourceWrapper rrw, boolean rec) throws RemoteAccessException {
        RemoteResource rr = rrw.getResource();
        if (rr.isContainer()) {
            rr.reindex(rec);
        } else {
            JOptionPane.showMessageDialog(this, rr.getValue("identifier") + " is not a container.", "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    protected void deleteSelectedResources() {
        ...
        TreePath path[] = removeDescendants(getSelectionPaths());
        if (path == null) {
            JOptionPane.showMessageDialog(this, "No Resource selected", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        if (path.length > 0) {
            int result = JOptionPane.showConfirmDialog(this, "Delete selected resource(s)?", "Delete Resource(s)", JOptionPane.YES_NO_OPTION);
            if (result == JOptionPane.YES_OPTION) {
                DefaultTreeModel model = (DefaultTreeModel) getModel();
                edu.hkust.clap.monitor.Monitor.loopBegin(1025);
for (int i = 0; i < path.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1025);
{
                    RemoteResourceWrapper rrw = getSelectedResourceWrapper(path[i]);
                    if (rrw == null) continue;
                    try {
                        if (pm.isEditable(rrw)) {
                            ...
                            String name = (String) rrw.getResource().getValue("identifier");
...
edu.hkust.clap.monitor.Monitor.loopEnd(1025);
    ...
    protected void showReferenceDocumentation() {
        try {
            ...
            if (selected == null) {
                ...
                String url = (String) selected.getResource().getValue("help-url");
                MiniBrowser.showDocumentationURL(url, "Reference documentation");
            }
    ...
    protected void doubleClick(TreePath path) {
        ...
        PropertyManager pm = PropertyManager.getPropertyManager();
        if (pm.isEditable(rrw)) popupResource(node.getResourceWrapper()); else {
            try {
                String name = (String) rrw.getResource().getValue("identifier");
                Message.showInformationMessage(this, name + " is not editable");
            } catch (RemoteAccessException ex) {
     ...
     */
    public static ResourceTreeBrowser getResourceTreeBrowser(RemoteResourceWrapper rrw, String rootName) {
        RemoteResourceWrapperNode rnode = new RemoteResourceWrapperNode(rrw, rootName);
