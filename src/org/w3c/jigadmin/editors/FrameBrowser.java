...
public class FrameBrowser extends ResourceTreeBrowser {
     ...
     */
    public static FrameBrowser getFrameBrowser(RemoteResourceWrapper rrw, String name) {
     ...
     */
    protected void addFrame(String classname, RemoteResourceWrapper rrwf, TreePath path) throws RemoteAccessException {
        RemoteResource newframe = rrwf.getResource().registerFrame(null, classname);
        RemoteResourceWrapper nrrw = new RemoteResourceWrapper(rrwf, newframe);
        RemoteFrameWrapperNode parent = (RemoteFrameWrapperNode) path.getLastPathComponent();
        RemoteFrameWrapperNode child = new RemoteFrameWrapperNode(parent, nrrw);
    ...
    protected void deleteSelectedResources() {
        ...
        if (path == null) return;
        if (path.length > 0) {
            int result = JOptionPane.showConfirmDialog(this, "Delete Selected Frame(s)?", "Delete Frame(s)", JOptionPane.YES_NO_OPTION);
            if (result == JOptionPane.YES_OPTION) {
                DefaultTreeModel model = (DefaultTreeModel) getModel();
                edu.hkust.clap.monitor.Monitor.loopBegin(982);
for (int i = 0; i < path.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(982);
{
                    ...
                    if (rrw != rootNode.getResourceWrapper()) {
                        ...
                        JOptionPane.showMessageDialog(this, "You can't remove the " + "resource here, please use " + "the resource tree", "Information", JOptionPane.INFORMATION_MESSAGE);
                    }
                }} 
edu.hkust.clap.monitor.Monitor.loopEnd(982);
