...
public class FramedResourceHelper extends ResourceHelper {
    ...
    class ResourceMenu extends JMenuBar implements ActionListener {
        ...
        public void actionPerformed(ActionEvent evt) {
            ...
            if (command.equals(CLOSE_AC)) {
                ...
                try {
                    String url = (String) selected_rrw.getResource().getValue("help-url");
                    showReference(url);
        ...
        ResourceMenu(Window parent) {
            ...
            add(resource);
            JMenuItem add = new JMenuItem("Add frame to selected resource/frame", Icons.addIcon);
            add.setActionCommand(ADD_AC);
            add.addActionListener(this);
            resource.add(add);
            JMenuItem del = new JMenuItem("Delete selected frame(s)", Icons.deleteIcon);
            del.setActionCommand(DEL_AC);
            del.addActionListener(this);
            resource.add(del);
            resource.addSeparator();
            JMenuItem quit = new JMenuItem("Close Resource window", Icons.closeIcon);
            quit.setActionCommand(CLOSE_AC);
            quit.addActionListener(this);
            resource.add(quit);
            JMenu help = new JMenu("Help");
            add(help);
            JMenuItem ref = new JMenuItem("Show reference documentation", Icons.infoIcon);
        ...
        public void valueChanged(TreeSelectionEvent e) {
            if (e.isAddedPath()) {
                if (!browser.isDragging()) {
                    RemoteFrameWrapperNode node = (RemoteFrameWrapperNode) e.getPath().getLastPathComponent();
     ...
     * @exception RemoteAccessException if a remote access error occurs.
     */
    public void initialize(org.w3c.jigadm.RemoteResourceWrapper rw, Properties p) throws RemoteAccessException {
    ...
    protected void build(boolean framed) {
        ...
        if (framed) {
            ...
            JSplitPane split = new JSplitPane(JSplitPane.VERTICAL_SPLIT, true, scroll, attrs);
