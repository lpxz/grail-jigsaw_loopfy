...
public class MiniBrowser extends ClosableFrame {
    ...
    class History {
        ...
        protected synchronized Object getPrev(Object current) throws NoSuchElementException {
            Object prev = prevurls.lastElement();
            prevurls.removeElementAt(prevurls.size() - 1);
            nexturls.addElement(current);
            if (prevurls.size() > 0) prev_B.setEnabled(true); else prev_B.setEnabled(false);
            next_B.setEnabled(true);
            return prev;
        }

        protected synchronized Object getNext(Object current) throws NoSuchElementException {
            Object next = nexturls.lastElement();
            nexturls.removeElementAt(nexturls.size() - 1);
            prevurls.addElement(current);
            prev_B.setEnabled(true);
            if (nexturls.size() > 0) next_B.setEnabled(true); else next_B.setEnabled(false);
            return next;
        }
    ...
    HyperlinkListener hll = new HyperlinkListener() {
        ...
        public void hyperlinkUpdate(HyperlinkEvent he) {
            ...
            if (type == HyperlinkEvent.EventType.ENTERED) {
                ...
                try {
                    if (currentURL != null) history.add(currentURL);
                    setPage(he.getURL());
                    if (urlField != null) {
     ...
     */
    public MiniBrowser(String url, String title) throws MalformedURLException, IOException {
        super(title);
        history = new History();
        build(url);
        setPage(url);
     ...
     * @exception IOException if an IO error occurs.
     */
    public MiniBrowser(String title) throws MalformedURLException, IOException {
        super(title);
        history = new History();
        build("");
    }
     ...
     * @exception IOException if an IO error occurs.
     */
    public static void showDocumentationURL(String url, String title) throws MalformedURLException, IOException {
        if (mbrowser == null) {
    ...
    public static void main(String args[]) {
        try {
            ...
            if (args.length > 0) browser = new MiniBrowser(args[0], "Mini Browser"); else browser = new MiniBrowser("Mini Browser");
