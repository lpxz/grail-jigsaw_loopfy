...
public class AddResourcePanel extends JPanel {
    ...
    ActionListener al = new ActionListener() {
        ...
        public void actionPerformed(ActionEvent ae) {
            if (ae.getActionCommand().equals("Ok") || ae.getSource().equals(identifierTF)) {
                if (!classSC.getText().equals("")) {
                    if (!identifierTF.getText().equals("")) {
                        browser.setResourceToAdd(classSC.getText(), identifierTF.getText());
     ...
     */
    protected Hashtable getResources(RemoteResourceWrapper rrw) throws RemoteAccessException {
        PropertyManager pm = PropertyManager.getPropertyManager();
        RemoteResource rr = rrw.getResource();
        if (rr.isIndexersCatalog()) return pm.getIndexers(); else return pm.getResources();
    }
     ...
     * @exception RemoteAccessException is some remote error occurs
     */
    protected void initializeStringChoice(RemoteResourceWrapper rrw) throws RemoteAccessException {
        classSC = new EditableStringChoice();
        Hashtable resources = getResources(rrw);
        Enumeration e = (Sorter.sortStringEnumeration(resources.keys())).elements();
        Vector cells = new Vector(10);
        edu.hkust.clap.monitor.Monitor.loopBegin(235);
while (e.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(235);
{
            String name = (String) e.nextElement();
            ResourceCell cell = new ResourceCell(name, (String) resources.get(name));
            cells.addElement(cell);
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(235);
     ...
     * @exception RemoteAccessException is some remote error occurs
     */
    protected void build(RemoteResourceWrapper rrw, String title) throws RemoteAccessException {
     ...
     * @exception RemoteAccessException is some remote error occurs
     */
    protected AddResourcePanel(String title, RemoteResourceWrapper rrw, ResourceTreeBrowser browser) throws RemoteAccessException {
        this.ok = false;
