...
public class RealmsServerHelper extends JPanel implements ServerHelperInterface {
    ...
    ActionListener al = new ActionListener() {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (obj == combo) {
                ...
                if (realms.contains(realm)) selectRealm(realm); else if (realm.length() > 0) addRealm(realm);
            } else if (e.getActionCommand().equals(ADD_USER_AC) || (obj == userT)) {
                String user = userT.getText();
                if ((user.length() > 0) && (!users.contains(user))) addUser(user);
    ...
    protected void build() {
        ...
        try {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(1067);
while (root.getServerBrowser().shouldRetry(ex)) { 
edu.hkust.clap.monitor.Monitor.loopInc(1067);
{
                try {
                    srealms = root.getResource().enumerateResourceIdentifiers();
                    break;
...
edu.hkust.clap.monitor.Monitor.loopEnd(1067);

        }
        if (srealms == null) return;
        Sorter.sortStringArray(srealms, true);
        realms = new Vector(srealms.length);
        edu.hkust.clap.monitor.Monitor.loopBegin(1068);
for (int i = 0; i < srealms.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1068);
realms.addElement(srealms[i]);} 
edu.hkust.clap.monitor.Monitor.loopEnd(1068);
    ...
    protected void selectRealm(String realm) {
        ...
        try {
            ...
            edu.hkust.clap.monitor.Monitor.loopBegin(1069);
while (root.getServerBrowser().shouldRetry(ex)) { 
edu.hkust.clap.monitor.Monitor.loopInc(1069);
{
                try {
                    realmrrw = root.getChildResource(realm);
                    break;
...
edu.hkust.clap.monitor.Monitor.loopEnd(1069);

        }
        if (realmrrw == null) return;
        try {
            String susers[] = realmrrw.getResource().enumerateResourceIdentifiers();
            Sorter.sortStringArray(susers, true);
            users = new Vector(susers.length);
            edu.hkust.clap.monitor.Monitor.loopBegin(1070);
for (int i = 0; i < susers.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(1070);
users.addElement(susers[i]);} 
edu.hkust.clap.monitor.Monitor.loopEnd(1070);
        ...
        if (usersList != null) usersList.removeListSelectionListener(lsl);
        ...
        p2.add(p1);
        p2.add(delRealmB);
        TitledBorder border = BorderFactory.createTitledBorder("Realm: " + realm);
        border.setTitleFont(Utilities.mediumBoldFont);
        usersPanel.setBorder(border);
    ...
    protected void selectUser(String user) {
        try {
            ...
            userPanel.removeAll();
            userPanel.invalidate();
            TitledBorder border = BorderFactory.createTitledBorder("User: " + user);
            border.setTitleFont(Utilities.mediumBoldFont);
            userPanel.setBorder(border);
    ...
    protected void addUser(String user) {
        try {
            if (realmrrw == null) return;
            realmrrw.getResource().registerResource(user, "org.w3c.jigsaw.auth.AuthUser");
    ...
    protected void addRealm(String realm) {
        try {
            root.getResource().registerResource(realm, "org.w3c.jigsaw.auth.AuthRealm");
            combo.invalidate();
            combo.addItem(realm);
            combo.validate();
            selectRealm(realm);
