...
class WindowCloser extends WindowAdapter {
    ...
    public static synchronized void close(JFrame frame) {
        windows--;
...
class ServerMenu extends JMenuBar implements ActionListener {
    ...
    protected String getAdminURL() {
        String url = JOptionPane.showInputDialog(this, "Admin server URL", "Open", JOptionPane.PLAIN_MESSAGE);
    ...
    public void actionPerformed(ActionEvent evt) {
        ...
        if (command.equals("open")) {
            ...
            try {
                ...
                JOptionPane.showMessageDialog(this, adminurl + " is not a valid URL", "Invalid URL", JOptionPane.ERROR_MESSAGE);
            }
...
public class ServerBrowser extends JPanel implements ServerListListener {
    ...
    protected void setAdminURL(URL adminurl) {
        try {
            ...
            initialize();
            build();
            RemoteResourceWrapper rrw = serverList.getModel().getServer(ServerListModel.ADMIN_SERVER_NAME);
            serverSelected(ServerListModel.ADMIN_SERVER_NAME, rrw);
            frame.validate();
            setCursor(Cursor.DEFAULT_CURSOR);
            frame.setTitle("Server Browser : " + adminurl);
        } catch (RemoteAccessException ex) {
            JOptionPane.showMessageDialog(this, ex.getMessage(), "RemoteAccessException", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    protected void initialize() {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(320);
while (!authorized) { 
edu.hkust.clap.monitor.Monitor.loopInc(320);
{
            try {
                authorized = true;
                admin.initialize();
...
edu.hkust.clap.monitor.Monitor.loopEnd(320);
    ...
    protected void build() {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(321);
for (int i = 0; i < servers.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(321);
{
            String server = servers[i];
            RemoteResourceWrapper rrw = serverList.getModel().getServer(server);
            ServerEditorInterface editor = ServerEditorFactory.getServerEditor(server, this, rrw);
            serverPanel.add(server, editor.getComponent());
        }} 
edu.hkust.clap.monitor.Monitor.loopEnd(321);
    ...
    protected ServerList getServerList() {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(322);
while (!authorized) { 
edu.hkust.clap.monitor.Monitor.loopInc(322);
{
            try {
                authorized = true;
...
edu.hkust.clap.monitor.Monitor.loopEnd(322);
...
edu.hkust.clap.monitor.Monitor.loopEnd(323);
    ...
    public static void main(String args[]) {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(324);
for (int i = 0; i < args.length; i++) { 
edu.hkust.clap.monitor.Monitor.loopInc(324);
{
...
edu.hkust.clap.monitor.Monitor.loopEnd(324);

        if (baseURL == null) baseURL = "http://localhost:8009/";
        URL bu = null;
        try {
            bu = new URL(baseURL);
