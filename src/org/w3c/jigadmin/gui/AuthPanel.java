...
public class AuthPanel extends JPanel {
    ...
    protected AuthPanel(ServerBrowser sb, String name) {
            ...
            public void actionPerformed(ActionEvent ae) {
                if (ae.getActionCommand().equals("Ok") || ae.getSource().equals(passwd)) {
                    if (!user.getText().equals("")) {
                        ...
                        Base64Encoder encoder = new Base64Encoder(user.getText() + ":" + password);
                        credential.setAuthParameter("cookie", encoder.processString());
                        AuthPanel.this.sb.getAdminContext().setCredential(credential);
        ...
        mgbl.setConstraints(p, mgbc);
        add(p);
        if (name != null) setBorder(BorderFactory.createTitledBorder("Realm: " + name)); else setBorder(BorderFactory.createTitledBorder("Authentication"));
    }
