...
edu.hkust.clap.monitor.Monitor.loopEnd(17);
    ...
    String getEditorClass() {
        if (editorProperties == null) return null;
...
edu.hkust.clap.monitor.Monitor.loopEnd(18);
    ...
    protected void initialize() {
        ...
        edu.hkust.clap.monitor.Monitor.loopBegin(485);
while (entries.hasMoreElements()) { 
edu.hkust.clap.monitor.Monitor.loopInc(485);
{
            ZipEntry entry = (ZipEntry) entries.nextElement();
...
edu.hkust.clap.monitor.Monitor.loopEnd(485);
...
edu.hkust.clap.monitor.Monitor.loopEnd(20);
        ...
        try {
            ...
            InputStream in = (new BufferedInputStream(zip.getInputStream(res)));
        ...
        try {
            ...
            InputStream in = (new BufferedInputStream(zip.getInputStream(fr)));
        ...
        try {
            ...
            InputStream in = (new BufferedInputStream(zip.getInputStream(idx)));
...
edu.hkust.clap.monitor.Monitor.loopEnd(28);
    ...
    public static PropertyManager getPropertyManager() {
        if (propertyManager == null) {
            ...
            if (base == null) {
                propertyManager = new PropertyManager(new File(p.getProperty("user.dir") + File.separator + "config" + File.separator + "jigadmin.zip"), new File(p.getProperty("user.dir") + File.separator + "config"));
            } else propertyManager = new PropertyManager(new File(base + File.separator + "config" + File.separator + "jigadmin.zip"), new File(base + File.separator + "config"));
        }
